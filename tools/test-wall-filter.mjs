...
/* ==> THE FIGURE COUNTS STORMS, NOT LANDFALLS, AND THIS IS THE GUARD ON THAT.
 * <== §57.7a. The column carried a real landfall count for about a day and
 * Aaron reverted it on glass: a count ranks archipelagos, because Irene
 * crossed five Bahamian islands and scores 10 while Katrina scores 1. If a
 * future change re-derives the count as an improvement, this goes red. */
ok('the figure equals the number of storms shown, never a sum of their landfalls',
  lfRows.every((r) => r.landfalls === r.shown.length));

const y2005 = lfRows.find((r) => r.year === 2005);
ok(`2005 counts storms ashore, not its 30 landfalls — got ${y2005?.landfalls}`,
  y2005?.landfalls === y2005?.shown.length && y2005.landfalls < 20);
