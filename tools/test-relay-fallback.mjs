 ...
 * refreshes it every successful cycle, changed bytes or not, so `ageSeconds`
 * is genuinely how long ago the cron last confirmed this key with its source.
 * A second "when did the content change" stamp was built and removed the same
 * day: nothing read it, and routing the header at it put a false "feed
 * delayed" banner over a healthy relay. */
...
for (const route of ROUTES) {
  ...
  /* ==> AND THE STAMP IT HANDS BACK IS THE ONE THE CLIENT JUDGES DELAY ON. <==
   * `ui/status.js` reads `X-Landfall-Fetched-At` and says "feed delayed" past
   * 90 minutes. Serving a warm copy stamped seconds ago must therefore NOT look
   * delayed — the regression that put a false banner over a healthy relay was
   * exactly this header carrying a content-change time instead.
   *
   * SKIPPED WHERE THE HEADER IS ABSENT BY DESIGN. `/api/jtwc/storms` carries
   * its `fetchedAt` inside the JSON body rather than a header, which is also
   * why it shows up as `bypassUnknown` in the cron summary. Asserting on a
   * header it never sets would be testing the test. */
  const rawStamp = res.headers.get('X-Landfall-Fetched-At');
  if (rawStamp) {
    const servedStamp = Date.parse(rawStamp);
    ok(`${route.name}: fresh KV -> stamp is recent, not a content-change time`,
      Number.isFinite(servedStamp) && Date.now() - servedStamp < 90 * 60 * 1000,
      'the client reads this header to decide whether to cry "delayed"');
  }
