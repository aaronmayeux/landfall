#!/usr/bin/env node
/**
 * test-seasons-bar.mjs — the words along the bottom of the archive globe.
 * §57.21b item 8.
 *
 *   node tools/test-seasons-bar.mjs
 *
 ...
 *
 * `barDetail` is pure — facts in, a sentence out — which is why this suite
 * needs no DOM at all. The board reports what is true and the bar owns how it
 * is said, so there is exactly one place to change the wording.
 *
 * Zero dependencies, plain node.
 */

import { barDetail } from '../seasons/bar.js';
 ...
 *
 * The bar mounts before the board has read a byte. An invented sentence in
 * that gap would be the app claiming a year it has not loaded.
 * ------------------------------------------------------------------------ */

eq('no season yet means no sentence', barDetail(null), '');
eq('and neither does a where with no label', barDetail({ shown: 3 }), '');
...
eq('==> ZERO DRAWN GETS ITS OWN WORDS, OR THE BAR IS A TITLE BAR <==',
  barDetail(WHERE), '2005 · Atlantic · tick a storm to draw it');

ok('and it never reads as a count of nothing',
  !/0 shown/.test(barDetail(WHERE)));
...
eq('the open storm is named',
  barDetail({ label: '2005 · Atlantic', shown: 4, openName: 'KATRINA' }),
  '2005 · Atlantic · KATRINA');

ok('and the hint stands down, because it has been taken',
  !/tap a track/.test(barDetail({ label: '2005 · Atlantic', shown: 4, openName: 'KATRINA' })));

ok('the count stands down with it',
  !/shown/.test(barDetail({ label: '2005 · Atlantic', shown: 4, openName: 'KATRINA' })));
 ...
 * roster row takes — a bar calling a storm something the row beside it does
 * not is the panel disagreeing with itself. */
eq('an unnamed storm reads the way its row does',
  barDetail({ label: '1935 · Atlantic', shown: 1, openName: 'Storm 3' }),
 ...
 * on a closed board this bar is the only thing on screen that can.
 * ------------------------------------------------------------------------ */
...
for (const w of [
  ...
  ok(`the year and basin survive every state (shown=${w.shown}, open=${w.openName || 'none'})`,
    barDetail(w).startsWith('2005 · Atlantic'));
}
...
console.log(`✓ ${pass} assertions pass — the archive bar says which state it is in`);
