...
/* =========================================================================
 * 8. A WIND BAND IS ONE FIELD, NOT TWO HALVES (§49.15)
 *
 * The fault was seen on Lala against a Hawaii home: her 34 and 50 kt bands
 * ended at 2:00 PM with `now` eight hours to the right of them. Lala's
 * forecast wind radii are not in this repo and were not in the archive at the
 * time, so the SHAPE is reproduced out of Ida's real bytes instead — a
 * measured field that crossed the house, and a forecast field that never comes
 * near it again.
 *
 * ==> THE HOME IS IN OPEN WATER, AND THAT IS THE POINT. <== Every inhabited
 * place tried — Port Fourchon, Grand Isle, Houma, Golden Meadow, Cocodrie,
 * Venice, Boothville, Biloxi, Gulfport, Bay St Louis, Mobile, Pensacola — sits
 * inside Ida's forecast 34 kt field at advisory 018, because that field is
 * enormous. The one place in her own data where the measured field crossed and
 * the forecast field stays beyond `nearRingNm` is offshore. The app takes a
 * home coordinate, not an address, so this is a coordinate; what is under test
 * is geometry, and the geometry is NHC's.
 * ====================================================================== */
section('§49.15 — one field, not two halves');

{
  const { normalizePastPoints } = await import('../lib/track-point.js');
  const { HOME_DASH } = await import('../config/constants.js');

  /** The same build the earlier sections use, re-declared because theirs is
   *  block-scoped to them. One advisory, its past track and its past wind
   *  field, all off the replay relay. */
  const buildAt = async (nnn, home = HOME, clockMs = null) => {
    const a = parseTcm(readAdv(nnn), { sourceId: 'al092021' });
    const iso = new Date(a.issuedMs).toISOString().replace(/\.\d+Z$/, 'Z');
    const pts = normalizePastPoints(JSON.parse(
      await (await call(iso, 'nhc/mapserver', q(SUMMARY_LAYER.pastPoints))).text()
    ).features);
    const prad = normalizePastRadii(JSON.parse(
      await (await call(iso, 'nhc/mapserver', q(SUMMARY_LAYER.windPast))).text()
    ));
    return {
      a, past: pts, pastRadii: prad,
      dash: buildHomeDashboard({
        storm: { ...a.storm, category: categoryFromKt(a.storm.windKt) },
        forecast: a.forecast.map((p) => ({ ...p, category: categoryFromKt(p.windKt) })),
        past: pts, radii: a.radii, pastRadii: prad, home,
        now: clockMs ?? a.issuedMs, trackState: 'ok',
      }),
    };
  };

  /* 28.10 N, 90.10 W — about 60 nm south of Port Fourchon, in Ida's wake. */
  const GULF = { lon: -90.1, lat: 28.1, label: 'Gulf of Mexico', source: 'address' };
  const A18 = parseTcm(readAdv('018'), { sourceId: 'al092021' });
  const iso18 = new Date(A18.issuedMs).toISOString().replace(/\.\d+Z$/, 'Z');
  const past18 = normalizePastPoints(JSON.parse(
    await (await call(iso18, 'nhc/mapserver', q(SUMMARY_LAYER.pastPoints))).text()
  ...
  const gulfDash = buildHomeDashboard({
    storm: { ...A18.storm, category: categoryFromKt(A18.storm.windKt) },
    forecast: A18.forecast.map((p) => ({ ...p, category: categoryFromKt(p.windKt) })),
    past: past18, radii: A18.radii, pastRadii: pr18, home: GULF,
    now: A18.issuedMs, trackState: 'ok',
  });
  const gco = gulfDash.corridor;

  /* --- the configuration is real, and it is asserted before it is used --- */
  ok(gco?.ok === true, 'the offshore home has a corridor at advisory 018');
  const gFwd = gco.published.includes(34) ? gco.forecast[34] : null;
  const gPast = gco.past?.published.includes(34) ? gco.past.cross[34] : null;
  ok(gPast?.everInside === true,
     'her MEASURED 34 kt field crossed this point — the past half qualifies');
  ok(gFwd && gFwd.everInside === false,
     'while her FORECAST 34 kt field never reaches it again');
  near(gFwd.closestGapNm, 107.5, 0.1,
     'and stays 107.5 nm off at its closest, past the 86.9 nm near ring');
  ok(gFwd.closestGapNm > HOME_DASH.nearRingNm,
     'so under the old per-half rule the forecast half was dropped outright');

  /* --- and the band does not stop dead in the middle of the frame -------- */
  const gSvg = homeChart(gulfDash, 'imperial');
  const nowXg = +(gSvg.match(
    /<line x1="([\d.]+)" y1="6" x2="[\d.]+" y2="[\d.]+" stroke="var\(--text-muted\)"/
  ) || [])[1];
  ok(Number.isFinite(nowXg), 'the chart draws a `now` line to measure the band against');

  /** The x coordinates of a band's TOP edge, in the order the path states
   *  them — parsed off the rendered SVG, so none of this can pass by
   *  repeating the chart's own arithmetic back at it. The top edge is the
   *  first half of the polygon, before it turns and comes back along the
   *  centre track. */
  const bandXs = (svg, tok) => {
    const m = svg.match(
      new RegExp(`<path d="M([^"]*)" fill="color-mix\\(in srgb, var\\(--kt${tok}\\)`)
    );
    if (!m) return null;
    const pts = m[1].split(/[ LZ]+/).filter(Boolean).map((p) => Number(p.split(',')[0]));
    return pts.slice(0, Math.ceil(pts.length / 2));
  };

  const gTop34 = bandXs(gSvg, 34);
  ok(gTop34 && gTop34.length > 1, 'the 34 kt band is drawn at all');
  near(gTop34[gTop34.length - 1], nowXg, 0.6,
     'and it runs all the way to `now` rather than stopping at the last analysis');

  /* THE MEASURE OF THE BUG, NOT ONLY OF THE FIX. The last measured hour is
   * three hours behind the advisory here, and the old band ended there. If
   * the near test is put back on the forecast half alone, this is the
   * assertion that goes red — the one above it could still pass on a chart
   * whose window happened to end near `now`. */
  const lastPastH = (gco.past.samples || []).filter((s) => s.gap[34] != null).at(-1)?.h;
  near(lastPastH, -3, 0.01,
     'her measured 34 kt field ends three hours before the advisory it is read with');
  ok(gTop34[gTop34.length - 1] > nowXg - 0.6,
     'so the band reaches past the hole rather than ending on the near side of it');

  /* --- the merged series never doubles back ------------------------------
   * The invariant the polygon needs and nothing was checking. Asserted on
   * every band of every advisory that draws one, not only on the offshore
   * case, because a sawtooth is silent — it draws, it just draws a lie. */
  {
    let checked = 0;
    let bad = '';
    for (const nnn of ['012', '014', '016', '017', '018']) {
      const { dash: d } = await buildAt(nnn);
      const s = homeChart(d, 'imperial');
      for (const kt of [34, 50, 64]) {
        const xs = bandXs(s, kt);
        if (!xs) continue;
        checked++;
        for (let i = 1; i < xs.length; i++) {
          if (xs[i] < xs[i - 1] - 0.01) bad ||= `adv ${nnn}, ${kt} kt at index ${i}`;
        }
      }
    }
    ok(checked >= 8, `enough bands to be worth asserting on (${checked})`);
    ok(!bad, `no band doubles back on itself in time (${bad || 'none'})`);
  }

  /* --- an advisory whose tau 0 PREDATES the last analysis -----------------
   * The opposite cadence case, and Ida never publishes it on her own: in
   * every fixture here the measured arm ends BEFORE tau 0. It is reachable in
   * the live app whenever the MapServer's past layer warms after the advisory
   * text was cached, so it is built by pairing advisory 017's forecast with
   * the past arm read at 018's clock — the exact bundle that situation hands
   * over. Merged naively the two arms would zigzag between measured and
   * forecast reach for the same hour.
   *
   * The clock is 018's, not 017's: read at 017's own issue time the past arm
   * is truncated at the present and the overlap cannot exist. Six hours later
   * — an advisory old, its geometry current — 017's tau 0 sits at -6h and the
   * analyses run to -3h, and the two arms genuinely cross. */
  {
    const A17 = parseTcm(readAdv('017'), { sourceId: 'al092021' });
    const mixed = buildHomeDashboard({
      storm: { ...A17.storm, category: categoryFromKt(A17.storm.windKt) },
      forecast: A17.forecast.map((p) => ({ ...p, category: categoryFromKt(p.windKt) })),
      past: past18, radii: A17.radii, pastRadii: pr18, home: HOME,
      now: A18.issuedMs, trackState: 'ok',
    });
    const mco = mixed.corridor;
    const lastP = (mco.past?.samples || []).filter((s) => s.gap[34] != null).at(-1)?.h;
    const firstF = (mco.samples || []).filter((s) => s.gap[34] != null)[0]?.h;
    ok(Number.isFinite(lastP) && Number.isFinite(firstF) && lastP > firstF,
       `the arms genuinely overlap here (measured to ${lastP?.toFixed(2)}h, forecast from ${firstF?.toFixed(2)}h)`);
    const xs = bandXs(homeChart(mixed, 'imperial'), 34);
    ok(xs && xs.length > 1, 'and a 34 kt band is still drawn across the overlap');
    let mono = true;
    for (let i = 1; i < xs.length; i++) if (xs[i] < xs[i - 1] - 0.01) mono = false;
    ok(mono, 'with the forecast half cut at the last measured hour, so it cannot zigzag');
  }

  /* --- the hedge never draws without the thing it hedges ------------------
   * Panama City at advisory 012: Ida's 34 kt field is 102.7 nm off in the
   * forecast and was 101.5 nm off in the analyses, so NEITHER half comes near
   * and no 34 kt band is drawn — but the corridor still publishes 34 kt and
   * still computes an earliest-arrival edge for it, 37 samples of it. The old
   * condition drew that dashed line onto a frame with no band under it. */
  {
    const PC = { lon: -85.66, lat: 30.16, label: 'Panama City, Florida', source: 'address' };
    const A12 = parseTcm(readAdv('012'), { sourceId: 'al092021' });
    const iso12 = new Date(A12.issuedMs).toISOString().replace(/\.\d+Z$/, 'Z');
    const past12 = normalizePastPoints(JSON.parse(
      await (await call(iso12, 'nhc/mapserver', q(SUMMARY_LAYER.pastPoints))).text()
    ).features);
    const pr12 = normalizePastRadii(JSON.parse(
      await (await call(iso12, 'nhc/mapserver', q(SUMMARY_LAYER.windPast))).text()
    ));
    const pcDash = buildHomeDashboard({
      storm: { ...A12.storm, category: categoryFromKt(A12.storm.windKt) },
      forecast: A12.forecast.map((p) => ({ ...p, category: categoryFromKt(p.windKt) })),
      past: past12, radii: A12.radii, pastRadii: pr12, home: PC,
      now: A12.issuedMs, trackState: 'ok',
    });
    const pco = pcDash.corridor;
    ok(pco.published.includes(34), 'the corridor still publishes 34 kt at Panama City');
    ok(!!pco.earliest?.[34], 'and still computes its earliest-arrival edge');
    const lit = (pco.samples || []).filter((s) => s.h >= 0 && s.gapEarly?.[34] != null);
    ok(lit.length === 37, `with 37 samples the old code would have drawn (${lit.length})`);
    const pcSvg = homeChart(pcDash, 'imperial');
    ok(pcSvg !== '', 'the chart itself is drawn');
    ok(!bandXs(pcSvg, 34), 'no 34 kt band, because neither half comes near');
    ok(!/stroke-dasharray="4 3"/.test(pcSvg),
       'and no orphan dashed hedge floating over a band that is not there');
  }
}
