...
toggle's note names the limit that would otherwise read as a fault — **US
only**, because NWS is and no global equivalent has been found. *(The note used
to name a second limit, that watches have no shape to draw. §56.4 resolved
their boundaries, so the note now says the narrower true thing: any that could
not be resolved are listed and not drawn.)*

**==> AND THE LAYER IS NO LONGER NATIONAL. <==** §56.5 replaced the whole-country
draw with a per-storm one on 2026-08-23. What that layer is now is below.

### 56.5 The map — icon and polygon, each doing the job the other cannot

**Landed 2026-08-23.** `map/layers/flood.js` rewritten, `lib/interior-point.js`
and `lib/flood-features.js` new, `FLOOD` in `config/constants.js` and
`FLOOD_GEO` in `config/tokens.js`.

**==> THE NATIONAL DRAW IS GONE. <==** §56.1's first fault was a layer
contradicting its own manifest entry: the toggle sat in the `Storm detail`
group and the layer painted every flood alert in the country. It now draws the
alerts inside the **selected storm's corridor** and nothing else, which is the
question the group it lives in actually asks. `update(map, storm, bundle)` and
`clear(map)` were deliberate no-ops and are real hooks now — the selection is
the layer's whole subject.

**==> THE RISK THIS ACCEPTS, WORDED HERE SO A LATER SESSION FINDS THE DECISION
RATHER THAN THE SMELL. <==** Green shapes drawn only inside one storm's
corridor say *this storm did this*, in pictures, where there is no sentence to
hedge with — and an NWS flood alert names no storm (§48.21, §50.3). The
mitigation is that the layer draws only while that storm is **selected**, so
the drawer's careful wording is on screen at the same moment as the shapes.
**Aaron made this call knowingly on 2026-08-22.**

**WITH NO STORM SELECTED THE LAYER DRAWS NOTHING AND THE STATUS ROW SAYS WHY.**
A toggle that appears to do nothing is its own bug. Both halves shipped; neither
is optional.

#### Two sources, because MapLibre clusters points only

**ONE LIST IN, TWO COLLECTIONS OUT, BUILT BY ONE FUNCTION.** `floodSources()` in
`lib/flood-features.js` walks the matched alerts once and emits the polygon
source and the clusterable point source together. Shapes in one place and points
in another is the split that drifts — a shape with no chip over it, or a chip
over a county with nothing painted, and neither surface knowing it disagrees.

**THE CHIP'S POINT IS A POLE OF INACCESSIBILITY, NOT A CENTRE.**
`lib/interior-point.js`, about 300 lines with its reasoning, no dependency.
§56.2 measured the cheap answer failing: **re-measured on the frozen national
snapshot plus the two archived zone boundaries — 35 shapes — the bounding-box
centre falls outside its own polygon 6 times**, every one a river corridor,
which is exactly the shape being clustered. A hazard icon in the wrong county is
this app telling somebody the water is somewhere it is not.

**THE AREA-WEIGHTED CENTROID WAS ALSO MEASURED, AND IT DID NOT FAIL.** It landed
inside all 35. What it lacks is a **guarantee** — a crescent or a ring-shaped
county puts a centroid in the hole by construction, and none of those happened
to be in one quiet day's capture. `interiorPoint` enforces containment on the
way out instead of hoping for it, and returns **null** rather than guessing.
Null is a real answer: the alert keeps its polygon, loses its chip, and is
**counted** as `iconless` so the status row can say so.

**A MULTIPOLYGON RESOLVES TO ITS LARGEST MEMBER BY AREA.** Measured on HIZ023, a
real captured Hawaii zone: it has **three** members — one island of 1,959
vertices and two slivers of six and five vertices with effectively zero area,
which NWS ships. Flattening them into one ring set returns a point inside a
*sliver*, about 0.001° — roughly a hundred metres — off the island the alert is
about. It passes a naive "inside any member" check and puts the chip on a speck
offshore.

#### Clustering replaces collision

**§56.2 IS WHY IT HAD TO.** Measured in chromium on a 390×700 phone with
`icon-allow-overlap: false`, MapLibre's own collision drew **7 icons for 25
alerts at z3 — and 11 for the 14 on screen sitting right on top of the cluster
at z7.** It does not resolve by zooming in. Three warnings with no icon and
nothing saying so is §5's silence with a map over it.

A cluster never drops anything: two alerts that cannot both be drawn become one
chip reading "2". `FLOOD.clusterRadiusPx` is **44** — the touch target (§10).
Icons closer together than a fingertip cannot be tapped apart, so merging them
is not a loss of information, it is an accurate statement about what the screen
can resolve. `clusterProperties` accumulates `warnings`, so a cluster holding
even one warning takes the warning chip: the more urgent member is what the
reader has to know is in there.

#### The chip is a rounded square, and must never become a circle

**`GENESIS_GEO`'s RULE, INHERITED.** That block refuses a centroid marker
outright, because a storm in this app **is** a filled dot with a spiral and a
halo and that equation is the whole legibility of the globe. Genesis obeys by
having no point marker at all. A flood alert cannot — a chip at a point is the
only thing that survives §56.2's pixel table at planet distance — so it obeys
the other way, by not being round. Separated from a storm by **shape**, not by
hue, which still holds for a reader who cannot tell the green from the orange.

Four canvas images: warning and watch, times two themes, pre-added under stable
names so a theme flip is a layout write and never a texture upload on the frame
the reader is looking at. **No DOM, no chip** — the headless suites drive the
engine with a stub map and no `document`, and a throw in `ensure` takes the
whole layer engine down. The polygons still draw, so an alert is still a green
shape.

**THE COUNT'S INK IS PER CHIP, AND ONE INK FAILS AA ON A REACHABLE CASE.**
Computed, not eyeballed — the theme's dark ink (`#0B1420`) and light ink
(`#F6F6F4`) against each fill:

| chip | dark ink | light ink |
|---|---|---|
| dark theme / warning `#3FBF6F` | **7.83** | 2.18 |
| dark theme / watch `#2A7A4A` | 3.51 | **4.87** |
| light theme / warning `#1E7A45` | 3.46 | **4.94** |
| light theme / watch `#14532E` | 2.03 | **8.41** |

The first draft used the theme's label halo everywhere — 3.51 on a watch-only
cluster, under AA's 4.5 for text this size, and a watch-only cluster is an
ordinary thing to have on screen. Picking per chip clears 4.5 on all four.

#### The polygons are zoom-gated and the chips are not

**§56.2's PIXEL TABLE IS WHERE BOTH ENDS COME FROM.** The median polygon is
6.1 px at z5 and 12.3 at z6. `FLOOD.polygonMinZoom` is **6** and
`FLOOD.polygonSolidZoom` is **7**, with the fill and outline interpolating
between them — a ramp and not a switch, because `minzoom` alone pops the whole
set on mid-pinch and reads as a rendering fault. Nothing is lost by gating,
because the chips carry the layer at distance: a scatter of clustered counts is
...
row was composed from the **national** slot, which was honest while the layer
drew the nation and became a sentence about a different set the moment it did
not. Five outcomes, every one of them producing the same empty globe, and only
one is an all-clear: the feed errored, no storm is selected, the storm has no
track, nothing came near it, or something came near it and could not be placed.

---

### 56.6 Tapping an alert

**Landed 2026-08-23.** `ui/view-flood-alert.js` new; `ui/rain-alerts.js` rows
became buttons; one delegated listener in `app/views.js`.

**TAP A SINGLE CHIP → THAT ALERT'S DETAIL. TAP A CLUSTER → THE MAP ZOOMS UNTIL
IT SPLITS.** The second is MapLibre's own `getClusterExpansionZoom`, which knows
the exact zoom this particular pile comes apart at because it built the pile.
Adding a fixed number of levels instead would overshoot a sparse cluster and
undershoot the Wabash valley, where §56.2 measured fifteen of twenty-five alerts
in one line. **It resolves a Promise** — in MapLibre 5 the answer comes from the
source's worker, and a synchronous read hands `easeTo` a Promise for a zoom and
the camera silently does nothing.

**THE HIT TEST IS A 44 px BOX**, the same one `map/markers.js` builds, over the
chip first and the fill second — what is drawn on top is tested first. Flood is
tested **last** in `main.js`'s click dispatcher, after storms and genesis: the
layer sits at order 5 under the storm and its cone, and the county-sized polygon
it also hit-tests would otherwise swallow a storm dot standing on it.

**THE KEYBOARD PATH IS THE ROWS, AND WITHOUT IT THIS IS A GESTURE-ONLY
FEATURE.** §10. An icon reachable only by tapping the globe does not exist for a
keyboard user. Each row in the `Flooding` section is a real `<button>` carrying
the alert's id, opening the same panel the chip does — which also means Enter
and Space work with no keydown handler anywhere. `lib/rainfall.js` `floodAlerts()`
now carries `id`; that was the one hop where it used to be dropped. A row without
an id stays inert rather than becoming a button that opens nothing.

**ONE DELEGATED LISTENER, NOT TWO WIRINGS.** The row builder is shared by both
screens' sections and both mount inside the drawer root. Threading an `onAlert`
dependency through two controllers to reach one shared builder is two places for
the same handler to drift; §12's rule is that a pattern used twice gets
extracted before the second use, and a single delegated listener **is** that
extraction.

**IT IS A DRAWER PANEL AND NOT AN IN-ROW EXPANDER**, because a chip on the globe
and a row in a list have to arrive at the same thing. An expander cannot be
opened from the map and a map popup cannot be opened from a keyboard; a panel is
the one surface both reach, and the one this app already knows how to focus,
dismiss and navigate back from.

**==> §56.6's PLAN NAMED A FIELD THE RELAY DOES NOT CARRY, AND THE SPEC IS
CORRECTED RATHER THAN THE RELAY WIDENED. <==** The plan listed *the issuing
office* among what was already in hand. It is not: `functions/api/nws/flood.js`
projects id, event, areaDesc, severity, urgency, onset, expires, ends, geometry,
drawable, zones and counties, and **`senderName` is not among them.** The panel
prints no office. Widening the projection inside a phase that was not about the
relay is the "it was right there" work §12 forbids — **adding it is a small,
separate change and it is Aaron's to call.**

The panel shows what the relay does carry: event, the whole area list
unshortened, when it began, when it ends, how long is left, and whether it is
drawn on the globe at all. **Do not widen the projection to bring NWS's
`description` and `instruction` across** — if the panel reads thin on glass,
fetch that one alert on demand.

**THE WORDING IS `lib/rainfall.js`'s, NOT A SECOND COPY.** The row and the panel
run the same alert through the same `floodAlerts()`, so *"in force until
6:00 PM"* cannot say one thing in the list and another in the detail. That
function also filters by expiry, so an alert that runs out while the panel is
open renders as **ended** rather than as a live warning nobody revisited — said,
not blanked (§5).
