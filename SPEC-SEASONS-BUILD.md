...
#### 57.67o Slice E — the head and the colour — as built

**The last slice, and it landed as ONE commit rather than two.** Aaron's call,
2026-08-31, asked for the split as two: the colour is a change to a file every
archive screen already draws through, and the head is a new layer that draws
nothing until the clock is engaged, so they fail in completely different ways.
He took one. **Both halves are behind the same switch — a cut in hand — so a
revert of this commit takes the whole of call 3 and call 4 and leaves slices A
through D standing.**

`map/layers/season-head.js` is new. `map/layers/season-tracks.js`,
`map/layers/season-cut.js`, `lib/season-clock.js`, `config/constants.js` and
`main.js` all move. No CSS: there is no chrome in this slice.

**==> AARON'S THREE CALLS THAT SHAPED IT, 2026-08-31. <==**

1. **One commit, not two.**
2. **The head draws only from the mesh's own handoff zoom up, and the 3D cage is
   left alone.** The consequence is stated below and was accepted before the
   work started rather than discovered after it.
3. **The head spins.** §57.67a call 4 said it *may* rotate; the recommendation
   put to him was mirrored-and-still, on the grounds that the mark has four arms
   and repeats every 90°. He took the spin.

##### The colour — call 3

**==> A MAPLIBRE LINE CARRIES ONE COLOUR, SO A TRACK COLOURED AT THE CORRECT
TIMESTAMPS IS SEVERAL LINES. <==** `line-gradient` was the obvious alternative
and it cannot be driven per feature — one storm's ramp would be every storm's
ramp — so the trail is cut into one feature per grade the storm held.

**THE COST WAS MEASURED BEFORE THE CODE WAS WRITTEN, ACROSS THE WHOLE SHIPPED
ARCHIVE.** 6,532 storms, 175,262 fixes. **Merging neighbouring fixes that
resolve to the same hex, the average storm breaks into 4.93 pieces — median 4,
95th percentile 12, worst 24 (AL131967).** A fully ticked 2005 Atlantic goes
from 31 features to 198, with the same vertex count plus one duplicated join
vertex per cut, 167 of them. Cutting at every fix instead would have been 175,262
features for the same picture.

**The runs are memoised beside the curve**, because a storm's grades are a
property of the finished record and cannot change while the clock runs. The
boundaries are read through the same fix-to-vertex map the cut uses
(`smoothPathIndexed`), so a colour change lands ON the vertex its fix landed on
rather than near it — and the dedupe trap §57.67e records is paid for once, here
as well, because it is the same index.

**==> EVERY PIECE SHARES ITS END VERTEX WITH THE NEXT ONE'S START. <==** The
obvious spelling leaves a gap one leg wide at every category change, which is
precisely where the reader is looking. Sharing it means the two lines meet under
their round caps.

**==> THE LABEL RIDES THE FIRST PIECE AND ONLY THE FIRST, AND THE COST IS
STATED. <==** The name is set along the line by MapLibre off this same source, so
a label on all five pieces prints the storm's name five times where it printed it
once. The first piece is chosen because it stops changing earliest — its shape is
fixed the moment the storm first changes grade, so a playing season cannot make
the name jitter — and because it is the origin end, where §57.21a already puts
the direction ring. **A storm whose opening run is short in screen pixels can
therefore lose its name while the clock is engaged**, because MapLibre will not
set text along a line shorter than the word. **If that reads wrong on glass the
dial is a sixth feature carrying the whole trail and the label alone**, at the
price of pushing every storm's geometry twice on every step — which is why it
was not done up front.

**Peak is untouched with no cut in hand.** §57.67a's reversal stands as
§57.67c wrote it: peak is the default, per-fix is what the clock switches on, and
the byte-identity guarantee slice B earned is unchanged because nothing goes near
the split without a state to split on.

##### The head — call 4

**==> IT STANDS ON THE TRAIL'S OWN LAST VERTEX, WHICH IS §57.67e's INSTRUCTION
AND IS NOW MEASURED. <==** `lib/season-clock.js` also answers a lon/lat,
interpolated straight between the two fixes either side of the moment, and the
drawn curve bends between them. **Measured across 200 moments of Katrina's life:
the two answers are the same point at every recorded fix and part company by up
to 7.6 km in between.** A head on the clock's own point looks perfect at every
fix and floats off its track on a recurve.

**`setSeasonTracks` HANDS ITS TIPS BACK AS A RETURN VALUE**, and `main.js` passes
them straight into `setSeasonHead`. A `seasonTrackTips()` export would answer
whatever the last push left behind, so a caller asking in the wrong order gets a
head one step behind its own trail — visible only as a mark that lags slightly,
which is the silent kind of wrong. Carried forward in one expression it cannot be
stale. **The fallback is the clock's own position** and only for a storm with no
drawn trail at all, where the record's position is the only answer there is.

**==> AND IT ONLY DRAWS FROM z3.8 UP, WHICH IS THE ONE THING IN THIS SLICE THAT
WOULD BE A BUG IF IT WERE DELETED. <==** `SPEC-MAP.md` §9.13 is blunt: ONE engine
draws the spiral and it is the 3D mesh. MapLibre's copy was deleted on
2026-07-24 because the two overlapped for 1.6 zoom levels and drew one mark twice
at slightly different projected positions — a smear that was structural rather
than tunable. This puts a MapLibre spiral back on the map, so it starts ABOVE
that band: below it the mesh is still stamping every archive storm's mark on its
BIRTHPLACE (`map/season-mesh.js`), and a head down there would be two spirals per
storm meaning two different things with nothing to tell them apart.
`SEASONS.clockHeadMinZoom` is the arithmetic — `zSpace + (zHandoff − zSpace) ×
fade.nodes[1]` = 3.8 — and not the number, so a hand retuning the dive band moves
it too.

**THE COST, ACCEPTED BY AARON BEFORE THE WORK STARTED:** out at the space floor a
playing season has no head. The tracks still grow; the cage still shows every
ticked storm's mountains and birthplace mark whole. **That is the same gap
§57.67h names against `buildSeasonMeshPoints` and it is unchanged by this
slice** — cutting the cage is a different layer and would have widened this
commit in exactly the way slice B refused to.

**==> THE MARK IS A PRE-COLOURED IMAGE PER CATEGORY, NOT A TINTED ONE. <==**
MapLibre can only tint an SDF icon and an SDF is a single channel, which would
throw away the baked halo `map/glyph.js` spends a paragraph explaining is the
only thing holding the mark off the sea at the paler end of §6's fixed palette —
measured there at 1.03–1.87:1 against the light ocean. So there is one image per
colour per hemisphere, built on demand. `SEASONS.clockHeadTexturePx` is 128
rather than the sprite's 256 because these live in the sprite ATLAS and a season
can ask for eight of them: 512 KB rather than 2 MB, at a 3.2× downscale into
`SIZE.stormDot3dPx`, which is still above the 3× a phone screen asks for.
**The pixel ratio is the downscale**, so the head and the mesh's own spiral are
the same size on glass.

**==> THE SPIN IS A LEGIBILITY DEVICE AND THE SPEC SAYS SO RATHER THAN IMPLYING
A MEASUREMENT. <==** A real eyewall goes round in an hour or two and the clock
plays a storm day a second, so an honest rate would be twenty-odd turns a second.
`SEASONS.clockGlyphSpinSeconds` is 6 real seconds a revolution, which with the
mark's four-fold symmetry is an arm passing every 1.5 seconds. **It is spent in
STORM time through `toStormMs`**, so dragging the scrubber turns the head and the
angle at a given moment is the same however the reader reached it — a rotation
paced on `performance.now()` would spin a paused globe. **`lib/season-clock.js`
answers `spinDeg`** because it is the only file that knows what time it is; the
layer is handed a state, never a moment. Positive is clockwise, which is what
`icon-rotate` means, so the sign is the negative of `spin` — a northern cyclone
turns counter-clockwise. **This is the dial in either direction if it reads wrong
on glass.**

**The head dims with everything else.** A reader who opens a storm mid-playback
gets every other track dropped to a ghost, and a full-strength mark on the end of
a ghosted line reads as a rendering fault rather than as emphasis — which is the
argument `season-focus.js` exists to make, applied to a third layer.

**Nothing here is a tap target and that is not an omission.** The head stands on
the trail, and `seasonStormAtPoint` queries the trail's line layer inside a 44 px
box — so a finger aimed at the mark selects that storm already, without a second
hit path that could disagree with the first.

#### 57.67p Slice E's gate

**`tools/test-season-head.mjs` is new — 43 assertions**, zero dependencies.
`tools/test-season-tracks.mjs` goes from 57 to 77, `tools/test-season-clock.mjs`
from 88 to 94, and `tools/test-season-cut.mjs` from 58 to 74. All 147 suites pass
under `node tools/run-suites.mjs`.

**==> THE CUT SUITE'S BYTE-IDENTITY ASSERTION MOVED, AND THE MOVE IS THE
INTERESTING PART. <==** §57.67f's section 1 compared a season pushed with an
end-of-season cut against one pushed with no cut, as strings. Those bytes are no
longer identical and should not be: the geometry is untouched, the payload gained
pieces. It now asserts the two things it was ever really claiming — **the same
storms, and every track rejoining to the identical line vertex for vertex** —
plus that the split really happened. **The byte-identity claim itself is not
lost**: the block above it still makes it between no cut and a null cut, which is
the case slice B's guarantee is actually about.

**`tools/test-season-head.mjs` CARRIES A CANVAS STAND-IN AND SAYS WHAT IT CANNOT
SEE.** `map/glyph.js` needs `Path2D` and a 2D context, and the layer correctly
answers "no mark" where there is no DOM. Every drawing call in the stand-in is a
no-op, so **nothing in this suite can tell a correct spiral from a blank square**
— the artwork belongs to the 3D engine, which has drawn it since July, and to
glass. What it buys is which image a feature names and what the layer was handed.

**TWENTY-NINE MUTATIONS WERE RUN AND TWENTY-SEVEN BITE.** On the colour: a piece
per fix instead of a run, each run wearing the next fix's grade, the pieces not
sharing their join vertex, the label on every piece, the label on the last piece
instead of the first, and the track splitting with no clock. On the head: standing
on the clock's own position, no fallback with no trail, an ended storm keeping its
head, an unborn one getting one, the zoom floor deleted, the floor set to the
space floor, the mark turning with the compass, a mark droppable to collision, the
longitude unwrapped, one wrap instead of a modulo, the grade taken as the peak,
the image rebuilt every push, the mark handed over at 1×, focus not reaching it,
and leaving the archive leaving the marks up. On the spin: the sign not flipped,
the angle never wrapped, the period in real milliseconds, counting from the epoch
instead of from the storm's first fix, and an unborn storm carrying an angle.

**==> FOUR SURVIVED THE FIRST RUN. ONE WAS A DEAD LINE, TWO WERE FAULTS IN THE
SUITE, AND TWO ARE EQUIVALENT — WHICH IS THE FIFTH RUN IN FIVE SLICES WHERE THE
EXERCISE CAUGHT SOMETHING THAT WAS NOT IN THE FEATURE. <==**

1. **`season-head.js` held a `Set` of image names beside the map's own atlas and
   it could not change any answer**, because it was ANDed with `map.hasImage`.
   It could go wrong, though: a style install takes MapLibre's images with it and
   a module-level Set would still have been saying yes, which is a missing mark
   with nothing logged. **Deleted rather than covered** — one owner, the same
   call slice D made about `disengage` resetting the position.
2. **The atlas assertion counted images rather than uploads.** The stub kept them
   in a `Map` keyed on the name, so re-adding the same mark sixty times looked
   identical to adding it once — it read "how many marks exist" while claiming to
   read "how much work was done", and a texture upload per push on the frame the
   reader is watching is the whole thing it exists to catch. The stub counts adds
   now.
3. **The no-canvas section could not tell the `document` guard from the `try`
   around the canvas**, because both answer "no mark". The difference is volume —
   without the guard every ticked storm warns on every step, ten times a second —
   so the section now asserts that nothing was warned, and both paths are covered.
4. **And the sibling suite's tip assertion was driven at the halfway mark of
   Katrina's timeline, where the trail's tip and the clock's own position are the
   SAME POINT.** It compared a value with itself and would have stayed green with
   the whole rule deleted. Both suites now FIND the moment by scanning 200 of them
   for the largest disagreement and report the distance. **That is §57.67d,
   §57.67f and §57.67i's finding for the fourth time, on the fourth different
   rule.**

**Two mutations survive the fixed suites and both are equivalent rather than
uncovered**, and both the code and this section say so plainly. Removing the
`>= last` half of `cutSegments`' clamp changes no output — a stop past the end
slices to the end anyway and leaves a one-vertex tail, which is dropped — and it
is kept because a cut is INSIDE a line, and because without it a playing season
builds and discards a slice per unreached grade per storm on every step. Removing
`setSeasonHead`'s `if (cut)` changes no output either, because every storm then
answers a null state and only a `running` one gets a mark; it is kept because it
skips a walk over every ticked storm on every push the archive makes with the
clock down, which is all of them until somebody presses play.

**WHAT THE SUITES CANNOT SAY.** Whether the colours read as a story or as a
rainbow at basin zoom; whether a storm's name survives on its first run; whether
a mark turning four times a revolution reads as a cyclone or as a pinwheel;
whether ten pushes a second across four archive layers still holds frame rate on
a phone. Those are Aaron's, on glass.
