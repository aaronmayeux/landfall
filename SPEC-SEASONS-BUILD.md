...
### 57.23a The season clock, as built

**The clock runs over the TICKED STORMS' own span, not the season's.** Aaron's
call, 2026-08-26. A season runs June to January and four ticked storms may all
live inside three weeks of September, so a calendar-year clock would be mostly
empty ocean with a date crawling across it. The cost, accepted: ticking a fifth
storm can move both ends of the timeline, so the clock resets rather than
carrying a moment that now means a different date.

**The controls fold into `seasons/bar.js` rather than taking a strip of their
own.** Aaron's call, same day. The archive globe's only chrome is that bar
(§57.16a), and a second horizontal band on a 390 px phone is globe the reader
does not get back. The bar wraps; the clock is `flex-basis: 100%` and breaks to
a second row. It is appended AFTER the Leave button in the DOM and moved
visually, so a keyboard reader never has a play button and a slider between
them and the way out.

**Nothing is special-cased for reduced motion, and that is a deliberate
departure from §57.23's own wording.** That setting exists to stop motion
nobody asked for; nothing here moves until somebody presses play. Hiding the
control from a reader who turned it on would delete the whole point of the
screen for them. Aaron's call, 2026-08-26.

**The clock SHORTENS the existing track line. There is no second line layer.**
`setSeasonTracks` takes an optional map of storm id to cut. The names along the
line, the focus dimming, the peak-category ink and the 44 px tap target all
already live on that feature, and a parallel clock layer would mean each of
them either being duplicated or quietly ceasing to work the moment somebody
pressed play. A storm the cut says is UNBORN is dropped from the collection
entirely rather than pushed as an empty line, because MapLibre renders a
one-coordinate LineString as nothing while still answering taps on it.

**The moving heads are the one separate source** (`map/layers/season-clock.js`),
which is §57.35 fault 3's split: the dots move every step, the trail does not.

**A head is coloured by the storm's strength AT THAT MOMENT while the track
behind it keeps its peak.** The one place on this globe where the two disagree,
and it is the feature: a finished track's hue answers *how bad did this get*,
a moving head answers *how bad is it right now*. Both are §6's fixed ramp.
**Strength is never interpolated between two fixes even though position is** —
a position between fixes is a claim the record supports, a wind speed between
them is a number NOAA never wrote, and rendering it would put an invented
Cat 4 on screen for a storm that went 3 to 5 between observations.

**A storm with no duration keeps its whole track at every moment.** A single
19th-century sighting has a position and no life; `buildTimeline` answers null
and `seasons/index.js` leaves it drawn as it always was. Deleting it from the
globe to serve a feature it is not part of would be the §5 silence.

#### The two dials, and why they are two

`SEASONS.clockDaysPerSecond` is **pace** — taste, Aaron's. `SEASONS
.clockStepsPerSecond` is **cost** — the number §57.35 fault 3 caps, shipped at
10. `lib/season-clock.js` `msPerStep` multiplies them, so **raising the step
rate makes playback more expensive and not faster.**

**The engine paces itself on WALL time and advances by STORM time, and
conflating the two shipped once.** The first version divided elapsed real
milliseconds by the storm-time step — 8,640,000 of them — so a step was owed
only after two and a half hours of real time and the clock never visibly moved.
Both are milliseconds, both come out of the same constants block, and the
arithmetic is correct in isolation. `tools/test-season-clock.mjs` caught it by
driving the engine on an injected clock. This is CLAUDE.md's fluent-wrong-number
rule one level up.

**A gap longer than three steps is skipped, never caught up.** A backgrounded
tab or a sleeping device leaves `requestAnimationFrame` unpaid for minutes;
advancing by the missed time fast-forwards past everything the reader wanted to
see, and running the missed steps one by one locks the phone on the frame it
woke up. A shorter stutter IS paid in full, or the clock silently runs slow on
any device that drops the odd frame.

#### What it costs, measured

`tools/season-clock-cost.mjs`, against the real `samples/seasons/seasons/
al-2005.txt`. **Every figure here is reproduced by running it; none is
recalled.**

| ticked | curve build | vertices held | trail pushes | cut arithmetic |
|---|---|---|---|---|
| 1 storm | 1.6 ms | 325 | 100% of steps | 0.008 ms/step |
| 4 storms | 3.9 ms | 1,073 | 55% | 0.007 ms/step |
| whole season (31) | 10.7 ms | 7,985 | 69% | 0.014 ms/step |

**The fingerprint guard is a saving on QUIET steps, not a general reduction in
push rate**, and an earlier draft of this section claimed otherwise. A single
325-vertex track crosses about six vertices per step at a day a second, so it
always changes; the saving comes entirely from storms that are unborn or
already over.

**==> EVERY NUMBER ABOVE IS A NODE NUMBER AND NONE OF THEM IS EVIDENCE THE APP
IS FAST. <==** CLAUDE.md is explicit about this and it applies here exactly.
They measure our own arithmetic, which is not where a phone spends its time —
MapLibre parsing and re-indexing the pushed geometry is, and this sandbox
cannot measure that at all because the basemap is blocked. **The useful half is
the vertex count: a whole ticked season hands MapLibre ~8,000 vertices about
seven times a second.** Whether that holds frame rate is step 10's
done-condition and it is Aaron's glass call. If it does not, the dial is
...
drawn through — which `spline` already computed and `smoothPath` discarded at
the door. Cutting a track at a moment needs to know which vertices belong to
which recorded fix, and matching coordinates back afterwards is arithmetic that
goes subtly wrong on a storm that loops over itself. `kept` indexes the
CALLER's array: two filters run inside (non-points dropped, then consecutive
duplicates merged), and a caller holding a timestamp per point that ignores
them lines the times up against the wrong vertices — silently, and only on
storms that stall. `smoothPath` is now this function with the map discarded, so
the two can never draw different curves.

#### The suite, and what a mutation run found

`tools/test-season-clock.mjs`, 112 assertions, driven off the real archived
storms — KATRINA for a long life with landfalls, CP011957 (Della) for the
antimeridian, AL021971 for a fix with no assigned intensity, AL011851 for
fourteen rows from the first storm in the record.

**Twenty-two mutations were run and five survived the first pass.** All five
are recorded because each was a different kind of finding:

1. **The head detaching from its own trail.** The assertion sampled the
   halfway point only, and that cut happened to land ON a vertex — where the
   interpolated head and the last vertex are the same coordinate by definition.
   One sample of a per-step property proves nothing about the steps it did not
   sample. It now walks the storm's whole life.
2. **Reading fix times without going through `kept`.** No archived storm drops
   a fix to `dedupe` — measured, all seven are the identity — so the
   indirection was never exercised. Fixed with a synthetic STALLED storm, the
   same move §57.30 step 2 had to make for the same reason.
3. **The monotonic-times guard.** Same shape: no fixture has a backwards
   timestamp. Fixed with a synthetic.
4. **The final-vertex anchor was DEAD CODE and is deleted** (§12). `spline`
   already anchors the last vertex; the tail gap measured zero on all seven
   storms. It was a defensive branch that could never execute.
5. **`usable.length < 2` is belt-and-braces**, not a hole: loosening it
   survives because the `curve.length < 2` gate after smoothing still answers
   null. Written down in the suite so the next mutation run does not spend an
   hour rediscovering it.

The remaining seventeen bite, including the wall/storm unit conflation above,
the dateline (`lonU` swapped for `lon` turns Della's head dot into a
planet-crossing jump), the unborn-storm drop, and the `['==', ['get','ended'],
true]` form — a bare `['get','ended']` is a `case` condition that can be null,
which MapLibre refuses at `addLayer`, throwing the whole line layer away for
every reader who never presses play. That is the same class as §48.21's third
bug, which shipped.
