...
/* ---------------------------------------------------------------------------
 * WIND ARRIVAL — FETCHED, NEVER COMPUTED (SPEC §4, §8)
 *
 * ==> THESE ARE NOT IN THE SELECTION BUNDLE, AND THAT IS DELIBERATE. <==
 * `fetchStormGeometry` above is what the MAP needs the moment a storm is
 * tapped: nine layers, in parallel, on the critical path of a tap. Arrival
 * isochrones draw nothing on the globe today — they answer one question, in
 * one block of the home panel, for a reader who has set a home. Adding them to
 * the bundle would put two more queries on every tap by every reader,
 * including the majority who have no home set, to fetch geometry nobody looks
 * at. So they are their own fetch, made only when there is a home and a storm
 * close enough for the answer to mean anything.
 *
 * THE TWO LAYERS ARE NOT INTERCHANGEABLE AND BOTH ARE SHOWN. "Earliest
 * reasonable" is the plan-by time — NHC's own guidance is to prepare for the
 * earliest, not the likeliest — and "most likely" is the expectation. Showing
 * only the likely one quietly deletes the safety margin the product exists to
 * publish; showing only the earliest reads as alarmism. They are two different
 * true numbers and the panel says both.
 *
 * THEY ARE LINES, NOT AREAS. Confirmed off the service 2026-08-09: layer 19 is
 * `esriGeometryPolyline` with an `arrival_time` string field, parented to a
 * group layer 17 called "Arrival Time of TS Winds". A point-in-polygon test
 * against these would return false everywhere and read as "no arrival data",
 * which is why this is written down here rather than discovered twice.
 * ------------------------------------------------------------------------- */

/** The two arrival isochrone layers on the summary service (§4.5). */
export const ARRIVAL_LAYER = Object.freeze({
  arrivalEarliest: 18,
  arrivalLikely: 19,
});

/**
 * Fetch both arrival isochrone sets for one storm.
 *
 * Same slot contract as the bundle — `ok` / `none` / `unavailable` per layer,
 * one failing never blanks the other (§5) — so `lib/home-exposure.js` reads
 * these and the bundle's watch/warning slot through one code path.
 *
 * @returns {Promise<{layers: Record<string, {status, fc, error}>, fetchedAt: string}>}
 */
export async function fetchWindArrival(storm) {
  if (!storm || storm.source !== 'nhc') throw new Error('arrival: NHC storms only');

  const bin = String((storm.raw && storm.raw.binNumber) || '').toUpperCase();
  if (!BIN_RE.test(bin)) throw new Error(`arrival: unusable binNumber "${storm.raw?.binNumber}"`);

  const layers = {};
  await Promise.all(
    Object.entries(ARRIVAL_LAYER).map(async ([key, layerId]) => {
      try {
        const fc = await queryLayer(layerId, bin);
        const clean = scrubSentinels(fc);
        layers[key] = { status: clean.features.length ? 'ok' : 'none', fc: clean, error: null };
      } catch (e) {
        console.warn(`[landfall] arrival layer '${key}' (id ${layerId}) failed:`, e?.message || e);
        layers[key] = { status: 'unavailable', fc: null, error: e?.message || 'failed' };
      }
    })
  );

  return { layers, fetchedAt: new Date().toISOString() };
}
