...
export function storyClauses(facts, {
  name, points = null, places = null, system = null, retirement = null,
} = {}) {
  ...
  /* ==> THE APOLOGY THAT USED TO SIT HERE IS DELETED, NOT MOVED. <== §57.7c.
   * It read "NOAA also marks a landfall for this storm after it had already
   * lost its tropical structure … so it is not in the list above", and it was
   * an honest sentence about a wrong answer. The landfall is in the list now,
   * so the sentence would be false rather than apologetic. */
  ...
  /* --- 7. THE NAME. §57.52. `name` rather than `who`: a storm with no name
   * cannot have had one withdrawn, and the fallback `This storm` must never
   * end up inside the sentence as though it were one. */
  out.push(...retirementClauses(retirement, name));

  return out;
}

/**
 * What the paragraph says about the storm's name, if anything. §57.52.
 *
 * ==> IT SPEAKS ONLY WHEN THE ANSWER IS YES, AND THAT IS THE §5 DESIGN RATHER
 * THAN AN OMISSION. <== There is no "this name was never retired" sentence and
 * there must not be one. Below a basin's derivation floor the frozen historic
 * block is what answers (§57.51), and nothing in the data separates "no name
 * from this year was withdrawn" from "this era was never assessed" — so a
 * negative sentence would be a claim we cannot stand behind, printed over
 * exactly the years least able to argue with it. Silence makes the two look
 * identical because neither is spoken about at all, which is the honest
 * outcome: the reader is never told something false, and the only thing they
 * lose is a sentence that would have said nothing.
 *
 * ==> A NAME RETIRED FOR A DIFFERENT STORM GETS NOTHING EITHER. <== Ida 2009
 * carries a name that was withdrawn for Ida 2021, and 208 storms in the record
 * are in that position. The join is name PLUS year, so they simply do not
 * arrive here. A sentence about them is available and accurate and was not
 * built; it is a product call rather than a technical one.
 *
 * ==> AND NO CLAUSE USES AN EM DASH. <== §57.41. `lib/units.js` returns a bare
 * em dash as its MISSING sentinel, so a paragraph containing one is the
 * cheapest available signal that a figure failed to resolve and was printed
 * anyway. `tools/test-season-story.mjs` bans the character across every
 * fixture, and that ban only works if nothing here uses one decoratively.
 *
 * @param {{kind:string, datedToThisStorm:boolean}|null} retirement
 *   from `data/retired-lookup.js`
 * @param {string|null} name
 */
function retirementClauses(retirement, name) {
  if (!retirement || !name) return [];

  /* ==> THE GREEK PAIR NEEDS TWO SENTENCES AND CANNOT BORROW THE FIRST. <==
   * §57.51. The 2021 WMO session ended the use of the Greek alphabet rather
   * than withdrawing nine letters, and where a Greek-designated storm was
   * retired it is the year of occurrence that was recorded. So *"the name Eta
   * was retired and will never be used again"* is false twice over: the letter
   * was not withdrawn as a name, and the retirement attaches to the storm.
   *
   * Aaron's call, 2026-08-30: the accurate longer version over the short wrong
   * one. It is two plain sentences rather than one long one because that is
   * the voice §57.41 settled on — one fact per sentence, repetitive on
   * purpose — and because a single sentence carrying both halves needs a
   * conjunction this paragraph does not otherwise use.
   *
   * Two storms in 3,266 reach this branch. */
  if (retirement.kind === 'description') {
    return [
      `${name} was retired after this storm.`,
      'The Greek alphabet is no longer used for storm names, so what was recorded'
      + ' is the storm and its year rather than the letter.',
    ];
  }

  /* ==> AN UNCERTAIN YEAR DROPS THE CLAUSE THAT CARRIES IT RATHER THAN
   * HEDGING IT. <== §57.41's rule, applied to a fact instead of a figure.
   * `RETIRED_UNSURE` flags three entries where the name's withdrawal is
   * settled but WHICH storm earned it is not: Carol and Edna were each retired
   * after 1954, brought back, and retired again, and NOAA's own table lists
   * the later year. *"After this storm"* is the part that might be wrong, so
   * it is the part that goes. What is left is true whichever storm earned it,
   * and no word in it is softer than the ordinary version. */
  return retirement.datedToThisStorm
    ? [`The name ${name} was retired after this storm and will never be used again.`]
    : [`The name ${name} was retired and will never be used again.`];
}
...
function landfallClauses(facts, { places, system }) {
  ...
  /* ==> A POST-TROPICAL LANDFALL SAYS WHAT IT WAS INSTEAD OF WHAT IT SCORED.
   * <== §57.7c. `lib/landfall.js` withholds a Saffir-Simpson number from a
   * crossing made after the storm lost its tropical structure, so
   * `categoryPhrase` returns nothing for it and the sentence would have gone
   * quiet on the single most interesting thing about Sandy's hardest landfall.
   *
   * ==> IT DESCRIBES A STATE AND NEVER DATES THE TRANSITION, AND THAT IS NOT
   * A STYLE CHOICE. <== The first wording was `after it had lost its tropical
   * structure`, which collided with the closing sentence six words later:
   * `ENDINGS.extratropical` is stamped with `facts.lastTime`, the end of the
   * RECORD, not the moment of transition. On Ophelia that printed October 16
   * and October 17 for one event. The closing sentence's looseness is older
   * than this clause and is left alone; this clause simply must not restate
   * it. */
  const hardCat = list[worstAt].nature === 'post-tropical'
    ? 'by then a post-tropical storm'
    : categoryPhrase(list[worstAt].category);
...
export const __internals = { isCyclone, joinList, landfallClauses, retirementClauses };
