...
/**
 * The box or boxes radar is allowed to fetch inside, from the live storms.
 *
 * ==> THIS IS THE FUNCTION THAT STOPS THE APP TAKING ITSELF DOWN. <== MapLibre
 * only requests tiles inside a source's `bounds`, and the first tile build had
 * none. On a globe that means the whole world pyramid, re-requested on every
 * pan, until Cloudflare answers 429 to everything on the origin — including
 * satellite, which is why an unrelated layer broke too.
 *
 * ==> IT RETURNS UP TO TWO BOXES, AND THE SECOND ONE IS THE DATELINE. <==
 * MapLibre's `bounds` is a plain [west, south, east, north] and cannot express
 * a box that crosses ±180. A naive min/max over longitudes turns two storms
 * either side of the dateline into a box spanning the entire planet — which is
 * exactly the flood again, in the basin Landfall watches most. So the span is
 * found by locating the widest EMPTY gap in longitude and taking its
 * complement, and if that complement crosses the antimeridian it is split in
 * two. A caller that installs one source per box gets a tight fit either way.
 *
 * Returns `[]` for no usable storms, which the caller must treat as "draw no
 * radar" rather than "draw it everywhere".
 */
export function radarBounds(storms, padDeg = IMAGERY.radar.boundsPadDeg) {
  const pts = (storms || []).filter((s) => Number.isFinite(s?.lat) && Number.isFinite(s?.lon));
  if (!pts.length) return [];

  const south = clamp(Math.min(...pts.map((s) => s.lat)) - padDeg, -MERC_MAX_LAT, MERC_MAX_LAT);
  const north = clamp(Math.max(...pts.map((s) => s.lat)) + padDeg, -MERC_MAX_LAT, MERC_MAX_LAT);

  const lons = [...new Set(pts.map((s) => wrapLon(s.lon)))].sort((a, b) => a - b);

  /* THE WIDEST GAP, MEASURED ROUND THE CIRCLE. The pair with the most empty
   * longitude between them is where the box should be OPEN, so the span runs
   * the other way round. With one storm the gap is the whole circle and the
   * span collapses to a point, which the padding then opens up. */
  let gapStart = lons[lons.length - 1];
  let gapEnd = lons[0];
  /* ONE STORM IS A FULL CIRCLE OF EMPTY, NOT ZERO. The modulo spelling of this
   * wrap-around gap reads 0 for a single longitude, which made the span the
   * whole planet — a global box for one storm, which is the exact flood this
   * function exists to prevent. Caught by walking the cases rather than by
   * reading the arithmetic. */
  let widest = lons.length === 1 ? 360 : (lons[0] + 360 - lons[lons.length - 1]) % 360;
  ...
  /* The span is the complement of the gap, opened by the padding at both ends.
   * `west` may come out below -180 and `east` above +180; that is the
   * antimeridian case and is resolved below rather than wrapped away here. */
  const west = gapEnd - padDeg;
  /* The span runs EASTWARD from the far side of the gap. It needs the extra
   * turn only when that walk crosses ±180 — which is exactly when the gap's end
   * sits east of its start. The comparison was the other way round at first and
   * turned two Atlantic storms into a global box. */
  const east = gapStart + padDeg + (gapEnd > gapStart ? 360 : 0);

  /* Padded past a full circle — the storms ring the planet. One global box is
   * the honest answer, and the zoom floor is what keeps it affordable. */
  if (east - west >= 360) return [[-180, south, 180, north]];

  const w = wrapLon(west);
  const e = wrapLon(east);
  /* `w > e` after wrapping means the span crosses ±180. Two boxes, meeting at
   * the antimeridian, which together cover exactly the same ground. */
  if (w > e) {
    return [
      [w, south, 180, north],
      [-180, south, e, north],
