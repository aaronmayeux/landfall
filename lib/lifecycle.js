...
export function endedExpired(storm, now = Date.now()) {
  ...
  /* ==> MEASURED FROM THE LAST PUBLISHED FIX, NOT FROM WHEN WE NOTICED. <==
   *
   * `observedAt` is the newest analysis anybody published for this storm — the
   * last time a source said anything about where it was and how strong. That is
   * the clock a reader is actually keeping: "NHC's last word on Faustob was
   * 10 AM Wednesday, so it should be gone by 10 AM Thursday."
   *
   * `ended.at` is a different moment and it drifts, sometimes by days. A storm
   * whose ending we read the instant it was declared has the two within an hour
   * of each other; a storm we only confirmed dead a week later, because nobody
   * opened the app or because the evidence took that long to arrive, would get a
   * FULL FRESH WINDOW starting from the day we worked it out. That is the bug
   * that kept a storm on the globe three and a half days after its last
   * transmission — the hold was being measured from the wrong end.
   *
   * Reading the fix time also makes the two death routes agree. A declared
   * ending and a confirmed absence stamp `at` from different places, so anchored
   * on `at` the same storm gets a different lifetime depending on how we found
   * out it was over, which is not a distinction a reader can see or should feel.
   *
   * FALLS BACK, rather than trusting one field. `ageMs` returns null for a
   * missing or unparseable stamp, so a storm with no readable fix time is judged
   * on `ended.at` instead of being thrown away for a parse failure.
   *
   ...
   * furniture. */
  const age = ageMs(storm.observedAt, now) ?? ageMs(storm.ended.at, now);
