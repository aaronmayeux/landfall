 ...
 * unthemeable for every storm that has a category to show. An ended storm has
 * none — nobody is publishing a wind for it — so this returns the absence of a
 * severity claim rather than a different one.
 */
export function stormSwatch(storm) {
  return isEnded(storm)
    ? palette().stormEnded
