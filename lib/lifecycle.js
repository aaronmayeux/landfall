...
/**
 * The source ids of every storm somebody is STILL PUBLISHING a wind for.
 *
 * ==> THIS EXISTS FOR THE ARCHIVE, AND IT LIVES HERE SO THE ARCHIVE DOES NOT
 * GET A SECOND OPINION. <== §57.21c. Aaron's call, 2026-08-25: a storm greyed
 * out on the live globe is finished as far as the sepia globe is concerned,
 * and one still in colour is still happening and stays off the historical
 * record. `noCurrentReading` directly above is the rule that does the greying,
 * so this is the same question asked of a list instead of a storm — not a
 * reimplementation of it, which is the thing that would drift.
 *
 * ==> IT IS IN `lifecycle.js` AND NOT IN `season-facts.js`, AND THAT IS ABOUT
 * THE BOOT PATH RATHER THAN ABOUT TIDINESS. <== `lib/season-facts.js` is
 * reached only through the archive's dynamic import and never ships to a
 * reader who does not open Past storms (§57.35 fault 4). `main.js` is the
 * caller, because `main.js` holds the live storm list — so putting these ten
 * lines in that file would have put its other three hundred on every boot
 * forever, to answer a question about a feature most sessions never open.
 *
 * ==> LOWERCASED, BECAUSE THE TWO SIDES OF THE JOIN ARE CASED DIFFERENTLY AND
 * BOTH ARE RIGHT. <== Measured on real archived bytes rather than assumed:
 * `/api/seasons/live` lists `ep092026` off NOAA's b-deck filenames, NHC's own
 * CurrentStorms carries `"id": "ep092026"` for the same storm, and NHC's KMZ
 * filenames carry `EP092026`. `sourceId` is whatever the source wrote; the
 * comparison should not depend on which.
 *
 * A GDACS storm's `sourceId` is a numeric event id, so it can never collide
 * with an ATCF id and needs no filtering out — it simply never matches, which
 * is the correct answer for a basin the provisional roster does not cover.
 *
 * @param {Array|null|undefined} storms
 * @param {number} [now]
 * @returns {Set<string>}
 */
export function reportingStormIds(storms, now = Date.now()) {
  const out = new Set();
  for (const s of storms || []) {
    const id = String(s?.sourceId || '').toLowerCase();
    if (!id || noCurrentReading(s, now)) continue;
    out.add(id);
  }
  return out;
}
