...
/**
 * ==> VOLCANOES THAT ARE BELOW SEA LEVEL AND STILL ON DRY LAND, BY GVP NUMBER.
 * <== The catalog gives a summit ELEVATION and has no field anywhere that says
 * whether a volcano is under water. `elev < 0` was standing in for that and it
 * is wrong for continental depressions, where the ground itself is below sea
 * level. Dallol sits in Ethiopia's Danakil Depression 78 km inland; Salton
 * Buttes is 155 km inside California. Both were being handed a seafloor, a
 * sunken foot and a sheet of ocean painted over the desert.
 *
 * ==> IT IS A LIST RATHER THAN A TEST, BECAUSE EVERY TEST AVAILABLE GETS THE
 * MARGIN WRONG. <== No depth threshold separates them: Kuwae is genuinely under
 * water at 2 m down and Dallol is dry at 48 m down. Testing the position against
 * `map/coastline.js` looks tempting and fails — that outline's points sit a
 * median 63 km apart, so it puts the Tjornes Fracture Zone (really offshore
 * Iceland) 8 km INLAND, and a test with a margin wide enough to ignore that
 * error is a test that no longer catches anything near a coast.
 *
 * So: two entries, each one checked by hand and each one obvious. Both are far
 * enough inland that no plausible refinement of the coastline changes the
 * answer. `tools/check-dry-volcanoes.mjs` re-derives the candidates from the
 * shipped catalog and fails if a new one appears, so this stays honest across a
 * catalog refetch instead of quietly going stale.
 */
const DRY_BELOW_SEA_LEVEL = Object.freeze(new Set([
  221041, // Dallol — Danakil Depression, Ethiopia, -48 m, 78 km inland
  323200, // Salton Buttes — Salton Trough, California, -40 m, 155 km inland
]));

/**
 * Is this volcano under water?
 *
 * ==> THIS IS ASKED ONCE, WHEN A MARK IS BUILT. EVERYTHING DOWNSTREAM READS
 * `mark.submarine`. <== It was re-derived as `elev < 0` in three separate
 * places — the field builder, `volcanoRelief()` and `volcanoBaseM()` — and
 * three copies of one decision is three chances to disagree. Flipping a volcano
 * in only some of them is worse than not flipping it at all: it would put
 * Dallol on the map at zero with a 3 km-deep foot underneath it.
 *
 * ==> SO CALL THIS WITH CATALOG PROPERTIES, NOT WITH A MARK. <== It needs the
 * GVP number, and a mark built somewhere careless may not carry one — at which
 * point every exception above silently stops applying. The dimension functions
 * read the stored flag precisely so they cannot make that mistake.
 *
 * @param {object} v raw catalog properties; needs `n` and `elev`
 * @returns {boolean}
 */
export function isSubmarine(v) {
  if (!v) return false;
  const elev = Number(v.elev);
  if (!isFinite(elev) || elev >= 0) return false;
  return !DRY_BELOW_SEA_LEVEL.has(Number(v.n));
}
 ...
 * map lathes real-scale geometry with it on the CPU (`proto/volcano-3d.js`,
 * which calls this function directly and has no copy). That is the same trap
 * `map/plate-seams.js` exists to close: two independent constructions of one
 * shape look right until they quietly disagree, and nothing tells you when.
 *
