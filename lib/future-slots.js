...
export const FUTURE_SLOTS = Object.freeze([
  ...
  'windCurrent',
  'windSwath',
  /* ==> `windAhead` IS THE ONE SLOT ON THIS LIST THAT IS PURE FUTURE BY
   *  DEFINITION. <== It is the forecast half of the envelope with the past
   *  deliberately withheld (§54), built so the headcount can say "still to
   *  come". A storm whose publisher has gone quiet or issued its last bulletin
   *  has no "still to come" that anybody is standing behind, and this slot
   *  cannot be clipped back to anything true — there is nothing in it but
   *  claims about next. It goes whole, with its siblings. */
  'windAhead',
]);
