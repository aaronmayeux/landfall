...
import {
  ACE, CAT, DAYS, LANDFALL, NAME, PRESSURE_MB, SATELLITE_ERA_FROM,
} from './wall-index.js';
...
/**
 * One filter, as a predicate over a raw storm row AND the year it belongs to.
 *
 ...
 *
 * ==> THE PREDICATE TAKES A YEAR AS ITS SECOND ARGUMENT, AND EVERY CALLER HAS
 * TO PASS IT DELIBERATELY. <== A storm row is a bare array of seven values and
 * carries no year of its own — the year is the key of the object it was found
 * under. Retirement is the first filter that needs it, because a name is
 * retired FOR A STORM rather than forever: Ida 2021 is retired and Ida 2009 is
 * not. **`list.filter(keep)` would hand this function an ARRAY INDEX as the
 * year**, which is not an error, does not throw, and would mark storms at
 * random — so both call sites wrap it in an arrow that names the year.
 *
 * ==> THE RETIRED-NAMES INDEX IS INJECTED RATHER THAN IMPORTED. <== §12: this
 * file is pure and may not reach into `data/`. `data/retired-lookup.js` owns
 * the join and the view binds it to the basin on screen. With no predicate
 * supplied the chip simply matches nothing, which is the honest failure — the
 * wall empties loudly rather than marking the wrong storms quietly.
 *
 * @param {object} f
 * @param {object} [opts]
 * @param {(name:string, year:number) => boolean} [opts.isRetired]
 */
export function keepFor(f, { isRetired = null } = {}) {
  if (!isFiltered(f)) return null;

  const cats = categoriesNarrowed(f) ? f.cats : null;
  const { landfall } = f;
  const retired = !!f.retired;
  ...
  return (s, year) => {
    /* ==> AN UNNAMED STORM FAILS THE RETIRED CHIP RATHER THAN JOINING IT. <==
     * §5, and it is the same shape as the ungraded-category rule below. Two
     * thirds of the Atlantic record carries no name at all — 1,139 of 2,004 —
     * and a name that was never given cannot have been withdrawn. It is still
     * on an unfiltered wall, because there the question is not being asked. */
    if (retired && !(isRetired && isRetired(s[NAME], year))) return false;
...
export function filterPhrase(f, { catLabel }) {
  ...
  /* ==> `retired` IS AN ADJECTIVE ON THE FRONT, NOT A CLAUSE ON THE BACK. <==
   * The trailing clauses are all measurements of the storm — how long, how low,
   * how much ACE — and they read as a list. Retirement is a fact about the
   * NAME, so `retired landfalling Category 5` reads the way a person would say
   * it while `Category 5 whose name was retired and lasting 10 days or more`
   * reads as two grammars stapled together. */
  const ashore = f.landfall ? `landfalling ${noun}` : noun;
  const head = f.retired ? `retired ${ashore}` : ashore;
...
export function sortFigure(row, key, { catLabel }) {
  ...
  /* ==> LANDFALLS DRAWS NO EXTRA COLUMN, AND THAT IS THE WHOLE POINT OF
   * WHERE ITS NUMBER MOVED TO. <== Aaron on glass, 2026-08-28. It had its own
   * figure and then, briefly, its own figure carrying a denominator — which
   * clipped to `18 of 3` in a 2.6em column and, worse, printed a total the
   * count column beside it was already showing.
   *
   * The ratio he asked for belongs IN the count column, because the
   * denominator IS that column's own number. `seasons-wall-markup.js` builds
   * it there, and this returns null the way `year` and `count` do — so the
   * strip keeps its full width on this sort rather than giving up 2.6em to
   * repeat a figure. */
  if (key === 'landfalls') return null;
