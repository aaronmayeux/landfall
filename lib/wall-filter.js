...
export function sortFigure(row, key, { catLabel }) {
   ...
   *
   * ==> AND IT CARRIES ITS DENOMINATOR, BECAUSE A BARE 18 IS NOT AN ANSWER.
   * <== Aaron on glass, 2026-08-28: he liked the count column's `8 of 18` and
   * asked for the same on this sort. He is right that the bare figure was the
   * weaker of the two — `18` next to a strip of 31 dots invites the reader to
   * work out the ratio themselves, and 18 out of 22 is a different season from
   * 18 out of 31 while both print `18`.
   *
   * ==> THE DENOMINATOR IS `shown`, NOT `total`. <== It has to agree with the
   * dots actually on screen. Under a Category 5 filter the strip is the majors
   * and the landfall count is already computed over the same subset
   * (`rowsFor` re-derives it), so quoting the season's full size beside it
   * would pair a filtered numerator with an unfiltered denominator — a ratio
   * that is simply wrong, in the direction that flatters the old years. */
  if (key === 'landfalls') {
    return {
      value: String(row.landfalls),
      sub: String(row.shown.length),
      unit: row.landfalls === 1 ? 'storm ashore' : 'storms ashore',
    };
  }
