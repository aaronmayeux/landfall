 ...
 * §9 says elevation and color are one signal from one number. The number was
 ...
 *
 *   - NO GEOMETRY IN THE PRODUCT THIS READS. The plain warning text is text:
 *     no cone polygon, no wind-band footprints, no past track. That is most of
 *     what the app draws outside the NHC basins.
 *
 *     ==> DO NOT READ THAT AS A STATEMENT ABOUT JTWC. IT IS A STATEMENT ABOUT
 *     ONE FILE. <== Measured 2026-08-19 off `latest/jtwc/`: the per-storm
 *     `.tcw` carries forecast wind radii per quadrant at 34/50/64 kt out to
 *     120 hours, plus a best track of 39 six-hourly positions back nine days
 *     with intensity on every one, and the `.kmz` draws the same track and
 *     forward radii as shapes. None of it carries PAST wind extent — that gap
 *     is real and global — but a cone, a footprint and a past track are all
 *     things JTWC publishes somewhere, and this file said for weeks that they
 *     were not.
 *
 ...
 * A WRONG wind is a §5 lie on the channel that drives height, color and the
 ...
 * wrong is also different in kind: a bad wind drives height, color and the
 * category badge and is a §5 lie on three channels at once; a bad final flag
 * greys one storm out for 24 hours, and the storm reappearing in either feed
 ...
 * the code is redundant for color and label — both prefer the index — but it
...
export function applyJtwcWind(storm, entry) {
  ...
  return {
     ...
     * measurement lights up the list row, the detail panel, the marker color
...
export function joinJtwcWinds(storms, index, nowMs = Date.now()) {
  ...
  const out = list.map((s) => {
    ...
    /* ==> ROSTER MEMBERSHIP, WHICH IS NOT THE SAME QUESTION AS A WIND MATCH.
     * <==
     *
     * `matchJtwcEntry` below asks "is there a JTWC warning I can safely take a
     * wind number from", and it applies distance and age tests on top of the
     * name — a storm whose warning is stale or whose position has walked too
     * far fails it while JTWC is still very much warning on the system. Reading
     * that as "JTWC has dropped this storm" would kill live typhoons.
     *
     * This asks the narrower thing: is the storm on JTWC's active list AT ALL.
     * Name only, same rule the declaration uses.
     *
     * ONLY CLAIMED ON A CLEAN INDEX. `partial` means some warning products
     * would not read, so a storm missing from it may be missing because its own
     * product failed — indistinguishable from JTWC having stopped. That is the
     * failed-index-is-not-an-empty-index rule (data/jtwc-index.js) applied one
     * level down, and it matters more here than anywhere: this field is
     * evidence that a storm is over. On anything but `ok` the field is absent
     * entirely rather than false, so a reader cannot mistake "we could not ask"
     * for "the answer is no". */
    const roster =
      index.state === 'ok'
        ? { listed: !!matchStormByName(index.storms, s.name) }
        : null;
    ...
    if (declared) next = { ...next, jtwcFinal: declared };
    if (roster) next = { ...next, jtwcRoster: roster };
    return next;
  });
