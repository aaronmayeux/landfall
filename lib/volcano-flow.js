 ...
 */
function stepFlow(surf, startE, startN, footRadius, centreE, centreN, reachM, phase) {
  const pts = [];
  let e = startE;
  let n = startN;
  ...
  for (let i = 0; i < L.maxSteps; i++) {
    ...
    /* ==> THE MEANDER, AND WITHOUT IT A TRACED FLOW IS TOO STRAIGHT TO READ AS
     * LAVA. <== Aaron on glass: *"it needs more wiggle."* He is right, and the
     * cause is that our mountain is smooth — four low bearing harmonics and
     * nothing finer (§43.2) — so steepest descent down it is very nearly a
     * straight radial line. Real flows wander constantly, because a flow builds
     * its own levees, breaks through them, and re-occupies its channel.
     *
     * ==> THIS IS THE ONE FRANKLY DECORATIVE TERM IN THE FILE AND IT IS LABELLED
     * AS ONE. <== Everything else here is the terrain deciding. This is a
     * seeded wander laid on top, because the terrain has no detail at this
     * scale to decide WITH. It rotates the direction of travel rather than
     * displacing the position, so the path stays continuous and still obeys
     * gravity on average — a displacement would let a flow step sideways
     * across a ridge it never climbed.
     *
     * Two incommensurate wavelengths for the same reason the plate seams use
     * two: one sine is a regular S-bend and reads as a drawn squiggle. */
    const w = travelled / L.meanderM;
    const bend =
      (Math.sin(w + phase) + 0.6 * Math.sin(w * 2.37 + phase * 1.7)) * L.meander;
    const cb = Math.cos(bend);
    const sb = Math.sin(bend);
    const rve = ve * cb - vn * sb;
    const rvn = ve * sb + vn * cb;
    ve = rve;
    vn = rvn;
    ...
    for (let i = 0; i < L.launches; i++) {
      ...
      for (let attempt = 0; attempt < L.launchTries; attempt++) {
        ...
        pts = stepFlow(
          ...
          m.n,
          reachM,
          /* Each flow wanders differently, stably, from its own launch index. */
          (m.seed % 97) * 0.31 + i * 1.13
        );
 ...
 */
export function buildFlowRibbons(flows, surf) {
  ...
  for (const flow of flows) {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      /* ==> EACH EDGE SITS ON THE GROUND UNDER *ITSELF*, NOT ON THE GROUND
       * UNDER THE CENTRELINE. <== This is the clipping Aaron saw: *"it clips as
       * you rotate."* Both edge vertices used to take the centreline's height,
       * which makes the ribbon's cross-section a straight CHORD across a
       * CONVEX flank — and a chord across a convex surface passes underneath
       * it. So the margins of every flow were buried in the mountain, and
       * which parts showed depended on the viewing angle, which is exactly why
       * it looked like clipping that came and went as the globe turned.
       *
       * ==> AND MY OWN TEST WAVED IT THROUGH. <== `tools/test-volcano-flow.mjs`
       * asserted that FEWER THAN 25% of ribbon vertices were under the surface.
       * That tolerance was invented to make a failing test pass rather than to
       * describe anything true, and 25% of vertices buried is precisely the
       * defect. The assertion is now zero, with the lift as the only margin.
       *
       * Two extra surface lookups per path point, on a handful of volcanoes,
       * once per field change. */
      const zL = surf
        ? surfaceHeightAt(surf.local, surf.k, surf.anySubmarine, p.e + px, p.n + py)
        : p.z;
      const zR = surf
        ? surfaceHeightAt(surf.local, surf.k, surf.anySubmarine, p.e - px, p.n - py)
        : p.z;

      positions.push(p.e + px, p.n + py, zL + L.liftM);
      positions.push(p.e - px, p.n - py, zR + L.liftM);
      ts.push(t, t);
      us.push(1, -1);
      /* Distance along the flow in METRES, for the travelling pulse. Normalised
       * `t` would make a 3 km flow and an 8 km flow show the same number of
       * crests, so the pulse would run visibly faster on the long one. Same
       * reasoning as `aArc` on the plate seams (`proto/world-deep.js`). */
      arcs.push(p.travelled, p.travelled);
    }
