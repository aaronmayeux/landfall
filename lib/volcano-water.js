...
/**
 * Per-node sea fraction: 1 in open water, 0 on land, ramped across ONE cell.
 *
 ...
 *
 * ==> SUPERSAMPLED, NOT BLURRED, AND THE DIFFERENCE IS WHY THE FIRST VERSION
 * FAILED ON GLASS. <== A blur measured in cells spread the shore over ~7 km,
 * because a cell is 1,731 m and nobody checked. This takes
 * `water.shoreSamples` squared land tests inside each node's own cell and
 * averages them, so a node gets the FRACTION of its cell that is water. The
 * ramp cannot reach past the one cell that straddles the coast, and open ocean
 * one cell away is fully opaque. Still built once, still nothing per frame.
 */
function landFactors(landMask, g) {
  ...
  const ss = Math.max(1, Math.round(WATER.shoreSamples));
  const inv = 1 / (ss * ss);

  /* Sub-sample offsets, centred on the node: for ss = 3 these are −1/3, 0,
   * +1/3 of a cell. Symmetric, so a node in open water samples only open
   * water and its coverage is exactly 1 — an asymmetric set would bias every
   * node one way and shift the whole coastline by half a sample. */
  const off = new Float64Array(ss);
  for (let k = 0; k < ss; k++) off[k] = (k + 0.5) / ss - 0.5;
  ...
  for (let j = 0; j < ny; j++) {
    ...
    for (let i = 0; i < nx; i++) {
      ...
      for (let sj = 0; sj < ss; sj++) {
        const lat = lat0 + (y + off[sj] * dy) / M_PER_DEG_LAT;
        for (let si = 0; si < ss; si++) {
          const lon = lon0 + (x + off[si] * dx) / mPerDegLon;
          if (!landMask.landAt(lon, lat)) sea++;
        }
      }
      out[j * nx + i] = sea * inv;
    }
