 ...
 *
 * ==> IT DOES NOT KNOW WHERE THE SHORE IS, AND A TEXTURE DOES. <== A custom
 * layer paints over the basemap unconditionally, so a sheet near a coast would
 * run across whatever island MapLibre drew underneath. Two CPU cuts were tried
 * and both reverted; the cut that works is `map/coast-mask.js`, sampled per
 * fragment in the shader. What this file owes it is `aLonLat` — each vertex's
 * position in DEGREES, because the mask is a picture of the world in degrees.
 *
...
export function buildWater(subs, lon0, lat0) {
  ...
  /* ==> LON/LAT PER VERTEX, AND IT CANNOT BE DERIVED FROM `wave`. <== The shore
   * mask is a picture of the world in degrees, so a fragment has to know where
   * on the planet it is. `wave` looks like it already says that and does not:
   * it is metres scaled by THIS CLUSTER's own latitude, deliberately, because
   * all it owes the shader is a continuous phase. Two clusters at different
   * latitudes use two different scales, and one uniform cannot invert both.
   * Interpolation across a flat sheet 75 km wide is exact enough that the
   * difference from true geodesy is far under one texel. */
  const lonlat = new Float32Array(count * 2);
  ...
  for (let j = 0; j < ny; j++) {
    ...
    for (let i = 0; i < nx; i++) {
      ...
      /* Back to degrees about the cluster centre, on the same equirectangular
       * approximation the metres were built with, so the two are consistent. */
      lonlat[idx * 2] = lon0 + x / (M_PER_DEG_LAT * cosLat);
      lonlat[idx * 2 + 1] = lat0 + y / M_PER_DEG_LAT;
  ...
  return indices.length ? { positions, colors, wave, lonlat, indices } : null;
}
