...
import { APPROACH, RAIN, UNITS } from '../config/constants.js';
...
/**
 * How long a warning has left, in words. §48.20.
 *
 ...
 *
 * ==> `onset` IS CARRIED AND IT IS NOT THE SAME THING AS `urgency`. <== §48.20.
...
/**
 * Whether a point forecast at the house belongs beside THIS storm. §48.18.
 *
 * ==> THE PROBLEM THIS SOLVES IS NOISE, NOT ACCURACY. <== The house figure is
 * true for every storm on the globe, because it is a forecast about a PLACE and
 * has nothing to do with which storm is on screen. Printing it under a typhoon
 * 6,000 nm from home is not a wrong number — it is a true number in a position
 * that implies a connection nobody claimed, which is the same class of failure
 * as a confident wrong answer.
 *
 * ==> THE GATE IS THE WIND FIELD, NOT THE DISTANCE. <== It used to borrow
 * `APPROACH.relevanceNm` — 1,500 nm, roughly a basin's width — because that
 * constant was already answering a nearby-sounding question for the Home
 * block. It is not the same question. "Is this storm in the reader's world at
 * all" is about a whole ocean; "does this storm impact this house" is about
 * this house, and `data/home-corridor.js` measures exactly that: whether the
 * storm's own published 34/50/64 kt field ever crosses the point. That is a
 * measurement off the agency's numbers rather than a threshold of ours, and it
 * is the SAME measurement the home chart draws, so the two surfaces cannot
 * disagree about whether a storm reaches somebody.
 *
 * ==> AND AN UNMEASURED STORM FALLS BACK RATHER THAN FALLING SILENT. <== A
 * `reach` of null means nobody published a wind field to walk — the geometry is
 * still in flight, or the storm is too weak to have radii, or it has ended and
 * been rebuilt from a skeleton. Treating that as "misses" would hide a real
 * answer behind a missing one, which is §5 pointed at the reader's own house.
 * So distance answers instead, at `RAIN.houseFallbackNm` (300 nm) — anchored
 * just past the widest wind field this project has ever measured, because when
 * there is nothing to measure the honest assumption is something as large as
 * the largest we have seen and no larger.
 *
 * ==> BOTH DISTANCES ARE CHECKED AND THE NEARER ONE WINS. <== A storm currently
 * 900 nm out whose track closes to 200 is one the reader cares about now;
 * gating on where it happens to be at this moment would hide the figure for
 * exactly the days it matters most. `approachNm` is null until the geometry
 * bundle lands, so the current distance is what carries the early renders.
 *
 * Pure. `reach` is `data/home-corridor.js`'s `reachesHome()`; both distances
 * are nautical miles and either may be null.
 */
export function houseRainInRange({
  reach = null, distanceNm = null, approachNm = null,
} = {}) {
  if (reach === 'reaches') return true;
  /* MEASURED, AND IT MISSES. A real all-clear from the agency's own wind
   * fields, and the one case where distance must NOT get a second vote — a
   * storm passing 150 nm offshore whose field is measured to stop short is
   * precisely the storm this gate exists to keep the figure off. */
  if (reach === 'misses') return false;

  const near = (n) => Number.isFinite(n) && n <= RAIN.houseFallbackNm;
  return near(distanceNm) || near(approachNm);
}

/**
 * How much of the house block this storm earns. §48.20.
 *
 * ==> A FLOOD WARNING IS NOT THE STORM'S, AND THAT IS WHY IT OUTLIVES THE
 * FIGURE. <== The first cut of §48.18 gated the whole block on whether the
 * storm's wind reaches the house, warnings included. That deleted the one thing
 * on the screen that was never the storm's to begin with: a Flash Flood Warning
 * is an agency's statement about the reader's own address, in force right now,
 * true whichever storm they happened to tap. The rainfall TOTAL under a storm
 * that misses is noise, because its position implies a connection nobody
 * claimed. The WARNING under the same storm is not — it makes no claim about
 * the storm at all, and the storm drawer is routinely the only screen anybody
 * opens during a hurricane (§48.17).
 *
 * So two gates, asking two different questions:
 *
 * | | question | test |
 * |---|---|---|
 * | `full` | does this storm's weather reach the house? | the wind field, or `RAIN.houseFallbackNm` |
 ...
 * question about a house. That reasoning is exactly why it fits here: the
 * warnings tier IS the basin-sized question. One constant, one meaning, in the
 * one place it is true.
 *
 * A storm on the far side of the planet reaches neither tier, so a flood
 * warning at home does not appear under every cyclone on Earth.
 *
 * Pure.
 */
export function houseRainScope({
  reach = null, distanceNm = null, approachNm = null,
} = {}) {
  if (houseRainInRange({ reach, distanceNm, approachNm })) return 'full';
  const inWorld = (n) => Number.isFinite(n) && n <= APPROACH.relevanceNm;
  return inWorld(distanceNm) || inWorld(approachNm) ? 'alerts' : 'none';
}
