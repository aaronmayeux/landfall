...
export function floodAlerts(features, nowMs) {
  ...
  for (const f of features || []) {
    ...
    out.push({
      /* ==> THE KEY, AND IT IS WHAT MAKES THE ROW A KEYBOARD PATH. <== §56.6:
       * an alert icon reachable only by tapping the globe does not exist for a
       * keyboard user, so the rows in the `Flooding` section open the same
       * detail the chip does. That needs the alert's own id to travel from the
       * relay all the way to the row, and this is the one hop where it used to
       * be dropped. Null is a real answer — a row without one stays inert
       * rather than becoming a button that opens nothing. */
      id: p.id || null,
      event,
