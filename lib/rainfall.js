...
/**
 * Whether a point forecast at the house belongs beside THIS storm. §48.17.
 *
 ...
 *
 * ==> IT REUSES `APPROACH.relevanceNm` AND DOES NOT INVENT A SECOND LINE. <==
 * That constant already answers "is this storm in the reader's world at all"
 * for the Home block's three sentences. A second threshold would mean a storm
 * that is "never near home" three inches up the panel and near enough to earn
 * a rainfall figure three inches down.
 *
 * ==> BOTH DISTANCES ARE CHECKED AND THE NEARER ONE WINS. <== A storm currently
 * 1,800 nm out whose track closes to 400 is one the reader cares about now;
 ...
 *
 * Pure. Both arguments are nautical miles, and either may be null.
 */
export function houseRainInRange({ distanceNm = null, approachNm = null } = {}) {
  const near = (n) => Number.isFinite(n) && n <= APPROACH.relevanceNm;
