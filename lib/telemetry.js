 ...
 * coarsened to a city, not bucketed to a region, not hashed. No accounts, no
 * user id, no name, nothing that could be joined back to a PERSON.
 *
 * ==> THE ONE EXCEPTION, ADDED 2026-08-05: THERE IS NOW A CROSS-VISIT
 *     IDENTIFIER, AND IT IS A RANDOM NUMBER FOR THE DEVICE. <==
 * This paragraph used to say "no cross-visit identifier" and it was true for
 * as long as it was written. It is not any more. `lib/device-id.js` holds the
 * whole argument for the reversal and the exact shape of the field; the short
 * version is that without it the table cannot tell 267 strangers from one
 * person visiting 267 times, which is the only question anybody ever asks of
 * it. It identifies a BROWSER, it is made up rather than measured, and
 * clearing site data makes the device a new device.
 *
 * The rule the paragraph exists for is unchanged: EVERY FIELD IS GUILTY UNTIL
 * PROVEN IT CANNOT BE JOINED BACK TO A PERSON. This one was argued for
 * explicitly, in writing, and signed off — which is the bar. A field that
 * arrives without that conversation has not met it.
 *
 ...
 *
 * Imports: config/ and lib/device-id.js. Wired by main.js; never imported by
 * a render path.
 */
...
function flush() {
  ...
  try {
    const body = JSON.stringify({
      ...
      /* ==> IN THE ENVELOPE, NOT ON EVERY EVENT, AND THAT IS THE POINT. <==
       * A field repeated across the errors and source transitions in one
       * batch would be the same number several times over, and it would put
       * an identifier on rows that are read as a CROWD (`source_rollup` is a
       * counter, not a log — see _telemetry-store.js). The server takes it
       * from here and attaches it to the once-per-visit session row only.
       * See lib/device-id.js for what it is and what it is not. */
      device: deviceId(),
      events: batch,
    });
...
export function startTelemetry(opts = {}) {
  try {
    ...
    window.addEventListener('error', (e) => {
      try {
        /* ==> WITHOUT THIS, "Script error." IS UNATTRIBUTABLE. <==
         * A script from another origin that throws is reported to us with the
         * message blanked to the literal string "Script error.", no stack, no
         * filename and no line — that blanking is a browser privacy rule, not
         * a bug, and nothing we do to our own code changes it.
         *
         * Every script this app SERVES is same-origin (`./vendor/`, `./main.js`),
         * so our own failures always arrive with a real message and stack. The
         * one cross-origin script on the page is Cloudflare's RUM beacon, which
         * the platform injects and whose tag we do not control.
         *
         * So a blank filename next to "Script error." is the SIGNATURE OF
         * THIRD-PARTY NOISE, and recording it is what stops the next person
         * reading this table from mistaking it for one of ours. Both rows in
         * `events` on 2026-07-31 were exactly this and were briefly diagnosed
         * as a missing `crossorigin` attribute on script tags that do not
         * exist. */
        const where = e?.filename ? `${e.filename}:${e.lineno || 0}:${e.colno || 0}` : '';
        const stack = safeStack(e?.error);
        push('error', {
          message: clip(e?.message, TELEMETRY.maxMessageChars),
          stack: clip(stack || where || 'third-party (cross-origin, blanked)', TELEMETRY.maxStackChars),
        });
      } catch {
