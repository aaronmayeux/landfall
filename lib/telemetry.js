 ...
 * Three kinds of event, and the third is the reason this file exists:
 ...
 *  fact, not six hundred. Keyed by kind+message, cleared per flush. */
let seen = new Set();
...
function push(kind, fields) {
  ...
  const key = `${kind}:${fields.message || fields.source || ''}`;
