 ...
 *
 * 3. THE TITLE IS NHC'S WHEN NHC PUBLISHES ONE, AND OURS WHEN IT DOES NOT.
 *    GIS layer 3 published no name at all — a basin, four probability strings,
 *    a source and a date, and nothing else — so every title here used to be a
 *    description computed from the centroid. The KMZ the outlook now comes
 *    from carries the forecaster's own wording, attached to the polygon it
 *    describes: `South-Southwest of Mexico`, not `Eastern Pacific`.
 *
 *    So `nhcTitle` is preferred and `areaTitle()` is the fallback, which still
 *    runs whenever a document arrives without one. THE DISTINCTION IS CARRIED
 *    ON THE OBJECT as `titleIsOurs`, because the detail view says different
 *    things about the two: our description is checkable against the centroid
 *    printed underneath it, and NHC's name is not ours to caveat.
 *
 *    Do not let the fallback drift into anything that sounds like a
 *    designation. It is descriptive, not predictive.
 *
 ...
 * their point, their number. GIS layer 2 could not support it: five polygons
 * against three points, and anchor 1 carrying polygon 2's attributes while
 * sitting inside polygon 1's footprint, so attribute matching and
 * nearest-centroid matching disagreed on the very first feature. The full
 * measurement is in the `GENESIS` block in config/constants.js.
 *
 * ==> THE KMZ THE OUTLOOK NOW COMES FROM PUBLISHES THAT JOIN, so the reason
 * has changed and the code has not. <== Every placemark carries a disturbance
 * number and the pairing is stated rather than inferred. Drawing at our own
 * centroid is now a choice about placement rather than a workaround, and it
 * holds for a better reason than it used to: the centroid comes off the SAME
 * feature the probability came off, so it cannot belong to the wrong area
 * however NHC changes its label points.
 *
...
export function normalizeNhcAreas(collection) {
  ...
  for (const f of feats) {
    ...
    out.push({
      ...
      /* ==> NHC'S OWN WORDING WINS, AND ONLY A NON-EMPTY STRING COUNTS. <== A
       * document that arrives without one — an older snapshot, a template
       * change, a discussion that did not match the shape `titleFromDiscussion`
       * expects — falls back to the centroid description rather than showing a
       * blank heading. Both are honest; only one is NHC's, and `titleIsOurs`
       * is what lets the panel say which. */
      title: typeof p.nhcTitle === 'string' && p.nhcTitle.trim()
        ? p.nhcTitle.trim()
        : areaTitle(centroid.lon, centroid.lat),
      titleIsOurs: !(typeof p.nhcTitle === 'string' && p.nhcTitle.trim()),

      /* THE FORECASTER'S PARAGRAPH, verbatim, or null. Never trimmed to a
       * sentence and never summarised — it is the only prose in this feature
       * and the only place a reader learns WHY an area is on the board. */
      discussion: typeof p.discussion === 'string' && p.discussion.trim()
        ? p.discussion.trim()
        : null,
