...
/**
 * Relative wording in BOTH directions, for anything holding a moment:
 * "in 3 days", "in 14 hrs", "in 40 min", "now", "40 min ago", "14 hrs ago",
 * "3 days ago". Same coarseness as formatAge, same reason — it qualifies a
 * timestamp, it is not one.
 *
 * ==> IT CARRIES ITS OWN PREPOSITION IN EVERY DIRECTION. <== "in 9 hrs" and
 * "9 hrs ago", never a bare "9 hrs". Callers concatenate one string and NOTHING
 * downstream branches on tense (SPEC-NEXT §49.4). `ui/view-storms.js` has the
 * scar from the forward half of this: writing `in ${until}` produced "closest
 * 120 mi in in 9 hrs".
 *
 * ==> A PAST MOMENT USED TO COME BACK AS "now", ALL TEN CALL SITES, FOREVER.
 * <== The negative interval fell through the under-two-minutes case, so a
 * closest pass three hours behind the clock and a wind that eased yesterday
 * both printed the same word as a thing happening this second. Seen on glass
 * 2026-08-16 on Lala; §49.1 is the full account.
 *
 * THE DEAD ZONE IS TENSELESS ON PURPOSE, AND IT IS "now" ON BOTH SIDES. Inside
 * two minutes either way the event IS happening, and "just now" — formatAge's
 * word for the same span — leans past, so a forecast arrival ninety seconds out
 * would be described in the past tense. §49.2's whole rule is that the two
 * tenses never borrow each other's words, and the dead zone is the one place
 * where a word belonging to neither is the honest answer.
 *
 * THE PAST ARM IS formatAge, NOT A COPY OF IT. The boundaries (one hour,
 * 48 hours), the rounding and the hr/hrs plural are already there and already
 * match this function's forward arm exactly. Duplicating them would mean two
 * places to change and a silent way for the two directions to disagree; the
 * only thing this adds is skipping formatAge's "just now", which the dead zone
...
export function formatUntil(t, now = Date.now()) {
  ...
  if (ms < 2 * MINUTE) return 'now';
  return formatAge(t, now);
}
...
/**
 * The same instant as `formatClockDay`, with the day AFTER the time —
 * "6:20 PM Sat" rather than "Sat 6:20 PM".
 *
 * ==> IT EXISTS BECAUSE PROSE AND LABELS WANT DIFFERENT ORDERS. <== In a
 * column of timestamps the weekday leads, because that is the part the eye
 * scans down and the rail is read as a column. Spliced into a sentence it does
 * not: *reached your house at Sat 6:20 PM* reads as a stutter where *reached
 * your house at 6:20 PM Sat* reads as speech. Judged on glass 2026-08-16.
 *
 * BUILT WITH `formatToParts`, NOT BY SLICING A STRING. The locale decides
 * whether the weekday is separated by a comma, a space or nothing, and whether
 * it leads at all — a `split(' ')` here works in en-US and quietly mangles
 * every other locale. The parts are asked for by name and reassembled.
 */
export function formatTimeDay(t) {
  if (t == null) return null;
  const d = new Date(typeof t === 'number' ? t : Date.parse(t));
  if (!isFinite(d.getTime())) return null;
  ...
  /* Everything that is not the weekday, and not the separator that was only
   * there to hold it apart from the rest. */
  const clock = parts
...
/**
 * NHC synoptic stamp → epoch ms UTC, or null.
 *
 * TEN DIGITS OF UTC, `YYYYMMDDHH`. It is `synoptime` on the past and forecast
 * wind-radii layers and `dtg` on the past-points layer — a STRING on one and a
 * NUMBER on the other, which is why this takes either and stringifies.
 *
 * ==> THIS LIVES HERE BECAUSE TWO LAYERS NEED IT AND THEY SIT ON OPPOSITE
 * SIDES OF THE IMPORT ARROW. <== `data/nhc-mapserver.js` parses it to stamp an
 * ISO instant onto normalized radii; `lib/windswath.js` parses it to decide
 * where a ring sits on the timeline (§7.13). `lib/` may not import from
 * `data/`, and a second copy in the swath builder is exactly how the two would
 * later disagree about what "18Z" means.
 *
 * ROUND-TRIPPED, so 31 February is rejected rather than rolling into March and
 * placing a wind field a day away from the storm that made it.
 */
export function parseSynopticStamp(value) {
