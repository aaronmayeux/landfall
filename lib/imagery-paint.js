 ...
 *              already decided what is cold and coloured it; we keep what it
 *              coloured.
 ...
 * clear ocean has no cold tops and therefore no colour, and auto-switching THAT
...
export function paintDisc(img, sat, opts = {}) {
  ...
  for (let y = 0, i = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++, i += 4) {
      ...
      if (enhanced) {
         ...
         * `slope * signal + intercept` ramp the colour path uses, so the two
  ...
  console.info(
    `[landfall] imagery ${sat?.id || '?'} ${enhanced ? 'colour' : 'grey'} ` +
   ...
   * vendor that starts colourising would silently lose its warm-cloud knockout;
   ...
   * bird over genuinely clear ocean has no cold tops and therefore no colour,
   * which is not evidence of anything. */
  if (!enhanced && chromaMax >= IMAGERY.greyscaleChroma) {
    console.warn(
      `[landfall] ${sat?.id} is flagged greyscale but this frame has colour ` +
