...
/**
 * How a storm is CALLED, anywhere it is called anything.
 *
 * §57.14: an unnamed storm displays as its number, never as a blank and never
 * as the spelled-out number NOAA wrote in the name column (`lib/hurdat.js`
 * folds those into unnamed).
 *
 * ==> IT LIVES HERE RATHER THAN IN THE MARKUP BECAUSE IT NOW HAS A SECOND
 * CALLER. <== It was `ui/seasons-board-markup.js`'s private helper for as long
 * as the roster was the only place a storm was named. Step 6 puts names on the
 * GLOBE, and `map/` may not import `ui/` (§12) — so the choice was one line
 * duplicated in two files or one rule in one place. A roster row reading
 * `Storm 7` beside a track labelled blank is precisely the drift the
 * extract-before-the-second-use rule exists to stop.
 */
