 ...
 * inside a class run (spec-parameter.md §32.3). They only ever LOOKED like a
...
function runsFrom(fc) {
  ...
  return dropRepeats(runs);
}

/**
 * Drop a run that repeats one already taken, forwards or backwards.
 *
 * ==> NHC PUBLISHES THE SAME SEGMENT TWICE. <== Measured 2026-08-21 on BOTH
 * live NHC storms, from the archived bytes of MapServer layer 11: Lala's past
 * track arrived as 14 segments of which `objectid` 742 and 743 were coordinate-
 * for-coordinate identical, and Moke's arrived as 3 of which 745 and 746 were.
 * In each case it is the LAST leg, sent twice.
 *
 * `stitch` then does the only thing it can with the copy: the chain's tail no
 * longer matches the copy's head, so it matches the copy's TAIL instead and
 * glues it on reversed. The path walks out along the final leg and straight
 * back down it — a 180 degree fold at the second-to-last vertex, which `unfold`
 * catches and reports on every single load.
 *
 * ==> IT IS DROPPED HERE, NOT REPAIRED IN `stitch`. <== A repeated segment is a
 * fault in the SOURCE, and the honest description of it is "this run carries no
 * information the other one did not". `stitch`'s job is to chain runs, and
 * teaching it to recognise a copy would put a source quirk inside the geometry.
 * The same rule the module already applies to duplicate VERTICES, applied one
 * level up to duplicate RUNS.
 *
 * EXACT COORDINATE MATCH ONLY, at `joinEpsDeg` — the same tolerance `stitch`
 * uses to decide two ends are one end, and for the same reason. Two runs are
 * the same run when they are the same COORDINATES, never when they are merely
 * near each other. Near is how you delete a real leg of a track.
 */
function dropRepeats(runs) {
  if (runs.length <= 1) return runs;
  const eps2 = TRACK_LINE.joinEpsDeg * TRACK_LINE.joinEpsDeg;
  const same = (a, b) => {
    if (a.length !== b.length) return false;
    const fwd = a.every((p, i) => sep2(p, b[i]) <= eps2);
    if (fwd) return true;
    return a.every((p, i) => sep2(p, b[b.length - 1 - i]) <= eps2);
  };
  const kept = [];
  for (const r of runs) if (!kept.some((k) => same(k, r))) kept.push(r);
  return kept;
}
 ...
 */
/**
 * Append the storm's last known position to its past track, on the right end.
 *
 * WHICH END IS NOT KNOWABLE FROM THE CHAIN. `stitch` assembles runs into a
 * polyline without a direction — the same reason `orient` exists — so this
 * picks the end NEARER the anchor and treats that as where the storm got to.
 * On a track whose two ends straddle the anchor that is also the right choice:
 * the storm arrived at the near one.
 *
 * THE DISTANCE CAP IS THE GUARD, and it exists because the failure it prevents
 * is worse than the gap it fixes. A bad parse, a mismatched storm, or a
 * position read on the wrong side of the antimeridian would otherwise draw a
 * confident dotted leg straight across an ocean the storm never crossed —
 * INVENTING a path, which §5 forbids far more strongly than it forbids a
 * missing one. Past the cap the track is left short and the console says so,
 * because a gap a reader can see beats a line they would believe.
 *
 * NO EPSILON TEST for "the anchor is already the last point". `dedupe` runs
 * downstream on the combined run and drops a duplicate endpoint on its own,
 * against the same `joinEpsDeg` the seam logic uses. A second copy of that rule
 * here is a second place for it to drift.
 */
function extendToAnchor(past, anchor, label) {
  if (past.length < 2 || !isPt(anchor)) return past;

  const head = past[0];
  const tail = past[past.length - 1];
  const oriented = sep2(tail, anchor) <= sep2(head, anchor) ? past : [...past].reverse();
  const end = oriented[oriented.length - 1];

  const gap2 = sep2(end, anchor);
  if (gap2 > TRACK_LINE.anchorMaxDeg * TRACK_LINE.anchorMaxDeg) {
    console.info(
      `[landfall] ${label}: last known position is ${Math.sqrt(gap2).toFixed(1)}\u00b0 from ` +
      'the end of the past track — leaving the gap rather than drawing a leg ' +
      'across it (SPEC \u00a75)'
    );
    return oriented;
  }
  return [...oriented, anchor];
}

function orient(past, forecast, label = 'storm') {
  if (!past.length || !forecast.length) return { past, forecast };

  const opts = [    { p: false, f: false },
    { p: false, f: true },
    { p: true, f: false },
    { p: true, f: true },
  ].map((o) => {
  ...
  const nearestOf = (list) => list.reduce((m, o) => (o.gap < m.gap ? o : m), list[0]);

  const forward = opts.filter((o) => o.turn <= TRACK_LINE.maxTurnDeg);
  const pool = forward.length ? forward : opts;
  if (!forward.length) {
    console.warn(`[landfall] ${label} track join: every orientation doubles back; taking the nearest`);
  }
  let best = nearestOf(pool);

  /* ==> THE TURN TEST MAY NOT BUY A CORNER WITH A HOLE. <==
   *
   * `maxTurnDeg` exists because direction of travel outranks endpoint distance
   * at the seam — Genevieve's lens, above. But it was an absolute veto, and an
   * absolute veto has no way to say "the alternative is absurd".
   *
   * FOUND ON GLASS 2026-08-21 by Aaron, on BOTH live NHC storms at once: the
   * dotted past track ran the entire length of the forecast, and the solid
   * forecast line was drawn in reverse. Reproduced from the archived bytes.
   *
   * The cause is a CLOCK, not a geometry. NHC's past track (MapServer layer 11)
   * was current while the forecast geometry was two advisories behind — Lala's
   * history reached 28.1N while her forecast still began at 26.9N. So the storm
   * had already walked PAST where its own forecast starts, and joining the two
   * correctly means a near-180 degree hairpin at the seam. The turn test threw
   * that orientation out; a reversed forecast passed it, and won on a gap of
   * roughly 11 degrees against the correct answer's 1.3.
   *
   * A stale forecast makes a SHARP CORNER. It does not make an OCEAN-SIZED
   * HOLE. So the turn test keeps its veto only while the join it prefers stays
   * comparable to the closest one available; past `orientGapRatio` the gap is
   * the stronger evidence and the corner is real data. This cannot reach
   * Genevieve's case, where the two candidate joins were both tight.
   */
  const nearest = nearestOf(opts);
  if (best !== nearest) {
    const ceil = nearest.gap * TRACK_LINE.orientGapRatio * TRACK_LINE.orientGapRatio;
    if (best.gap > ceil) {
      console.warn(
        `[landfall] ${label} track join: the only orientation without a sharp corner leaves a far ` +
          'wider gap; taking the tight join and its corner. The past track has most likely ' +
          'overtaken a stale forecast.'
      );
      best = nearest;
    }
  }
...
/** One centripetal Catmull-Rom sample between p1 and p2, at this file's own
 *  curve settings. The maths moved to lib/catmullrom.js when the cone of
 *  uncertainty became its second caller (§12) — same curve, so a cone and the
 *  track running through it round identically. */
const sample = (p0, p1, p2, p3, t) =>
  crPoint(p0, p1, p2, p3, t, TRACK_LINE.alpha, TRACK_LINE.minKnotGap);
 ...
 */
function spline(pts, maxVertices = TRACK_LINE.maxVertices) {
  ...
  /* ==> THE BUDGET IS SPREAD ACROSS THE LEGS, NOT SPENT FRONT TO BACK. <==
   *
   * This was a running total each leg drew down until it hit zero, which meant
   * a path with more legs than budget was smooth at the start and DEAD STRAIGHT
   * at the end, with one hard corner where the money ran out. On a model
   * guidance run under its tighter cap that put the elbow squarely in the
   * middle of the track and left the whole outer half — the part that fans out
   * and is the entire reason the layer exists — drawn exactly as it was before
   * smoothing. Caught on glass by Aaron, who reported the tracks looked no
   * different: half of each one genuinely was not.
   *
   * A LATENT BUG IN THE OFFICIAL TRACKS TOO, not just the new caller. At the
   * full budget a mature storm — 45 past fixes plus a forecast, so ~51 legs at
   * up to 24 samples — overruns by enough to flatten its last leg or two, which
   * is the freshest end of the track.
   *
   * An even share is the honest division: every leg is equally entitled to the
   * curve, and a path that cannot afford full resolution should lose it evenly
   * rather than all in one place. Legs still take LESS than their share when
   * they are short enough not to need it (`spacingDeg`), so the total normally
   * lands under the cap rather than at it. */
  const legs = xy.length - 1;
  const perLegCap = Math.max(1, Math.floor(maxVertices / legs));
  const curve = [];
  const index = [];
  ...
  for (let i = 0; i < legs; i++) {
    ...
    n = Math.max(1, Math.min(n, perLegCap));

    index.push(curve.length);
    for (let k = 0; k < n; k++) {
      const s = sample(p0, p1, p2, p3, k / n);
...
/**
 * The same curve, for a path that is nobody's history and nobody's forecast.
 *
 * ==> WHY THIS IS EXPORTED SEPARATELY FROM `smoothTracks`. <== That function
 * exists to solve the SEAM: two slots, an unknown chain direction, a gap
 * between them, and a cut that has to land on the shared vertex. A model
 * guidance run has none of that — it is one run of points that begins where it
 * begins. Handing it through the seam machinery would mean inventing a
 * forecast slot to satisfy the orientation logic, and the smoothing is the only
 * part of that pipeline it actually wants.
 *
 * ==> LONGITUDE CONTINUITY IS THE CALLER'S. <== `smoothTracks` unwraps because
 * it is concatenating two independently-published slots. This does not, because
 * `lib/adeck.js` has already run `unwrapRun` against the storm's own longitude —
 * which is a BETTER anchor than the path's first point, and unwrapping twice
 * against a different anchor is how a track that crossed 180° cleanly ends up
 * drawn a world away.
 *
 * `maxVertices` is the budget, and guidance takes a smaller one than official
 * geometry: it is a thin dashed line drawn several times per storm, so the
 * vertices it does not need are pure cost on a phone.
 *
 * Fewer than three points is not a curve — returned as it came, never padded.
 */
export function smoothPath(pts, maxVertices = TRACK_LINE.maxVertices) {
  return smoothPathIndexed(pts, maxVertices).curve;
}

/**
 * The same curve, plus WHICH VERTEX EACH INPUT POINT LANDED ON.
 *
 * ==> `smoothPath` ALREADY COMPUTED THIS AND THREW IT AWAY. <== `spline`
 * returns an `index` beside its curve — it has to, because `smoothTracks` cuts
 * the joined path at the forecast's first fix and cannot go looking for it in
 * several hundred vertices. This export is that same map, widened to cover the
 * points the two filters below drop, and nothing here re-splines anything.
 *
 * ==> IT EXISTS FOR THE SEASON CLOCK (§57.67 slice B) AND THE POINT IS THAT
 * THE CURVE IS NOT REBUILT PER STEP. <== The archive holds one cached curve per
 * storm. Ten times a second the clock says how many recorded fixes have
 * happened, and the track has to be drawn up to there — so the one thing the
 * caller needs is the translation from a FIX number to a VERTEX number.
 * Re-smoothing to find it would be §57.35 fault 3 with extra steps.
 *
 * ==> AND SEARCHING THE CURVE FOR THE FIX INSTEAD WOULD BE A FLOAT COMPARISON,
 * WHICH IS WHY THIS IS EXPORTED RATHER THAN REDERIVED BY THE CALLER. <== A
 * centripetal Catmull-Rom passes exactly through its knots in the maths, and in
 * `double` it passes through them to within a rounding error that changes with
 * latitude — `spline` scales longitude by `cos(lat)` on the way in and divides
 * it back out on the way out. Matching a fix to a vertex on equality would work
 * on most storms and fail silently on some.
 *
 * `index` has ONE ENTRY PER INPUT POINT, always, even for the points that never
 * reached the spline:
 *
 *   - a point `isPt` rejects, and
 *   - a point `dedupe` drops for sitting on top of its predecessor.
 *
 * Both take the vertex of the last surviving point before them, which for a
 * dropped duplicate is the literal truth — it IS at that vertex. **The
 * duplicate case is not theoretical: measured across the whole shipped archive
 * on 2026-08-31, 130 of 3,266 storms have at least one fix dedupe drops, up to
 * 9 in one storm (AL061862).** An index built over the deduped list and read
 * with a raw fix number would put those storms' heads up to nine fixes behind
 * the truth, and every other storm would look right.
 *
 * The three degenerate returns hand back `pts` unchanged, exactly as
 * `smoothPath` always has, and their index is the identity — which is correct
 * rather than a fallback, because in those cases the curve IS the input.
 *
 * @param {Array<Array<number>>} pts
 * @param {number} [maxVertices]
 * @returns {{curve: Array<Array<number>>, index: Array<number>}}
 */
export function smoothPathIndexed(pts, maxVertices = TRACK_LINE.maxVertices) {
  const identity = () => (Array.isArray(pts) ? pts.map((_, i) => i) : []);
  if (!Array.isArray(pts) || pts.length < 3) return { curve: pts, index: identity() };

  /* Filtered with the original position carried along, rather than with
   * `filter`, because the whole job here is not losing track of where a point
   * came from. */
  const clean = [];
  const cleanFrom = [];
  for (let i = 0; i < pts.length; i++) {
    if (isPt(pts[i])) { clean.push(pts[i]); cleanFrom.push(i); }
  }
  if (clean.length < 3) return { curve: pts, index: identity() };

  const { pts: unique, kept } = dedupe(clean);
  if (unique.length < 3) return { curve: pts, index: identity() };

  const { curve, index } = spline(unique, maxVertices);

  /* Stamp the survivors where they belong, then fill every gap forward from
   * the last one stamped. `index[0]` seeds it so a leading unreadable point
   * cannot leave a hole at the front. */
  const full = new Array(pts.length);
  for (let k = 0; k < kept.length && k < index.length; k++) {
    full[cleanFrom[kept[k]]] = index[k];
  }
  let last = index[0];
  for (let i = 0; i < full.length; i++) {
    if (full[i] === undefined) full[i] = last; else last = full[i];
  }

  return { curve, index: full };
}
 ...
 */
export function smoothTracks(bundle, label = 'storm', endAt = null) {
  ...
  try {
    ...
    ({ past, forecast } = orient(past, forecast, label));

    /* ==> A STORM WITH NO FORECAST STILL HAS A LAST KNOWN POSITION, AND THE
     * TRACK HAS TO REACH IT. <==
     *
     * For a LIVE storm the leg to the current position comes free: the forecast
     * track begins at tau-0, `orient` turns the past around to meet it, and the
     * connector is drawn as part of the past. Empty the forecast slot for a
     * silent or ended storm and that connector goes with it — correctly, since
     * it was reaching for a forecast that is no longer drawn. What it left
     * behind was a dotted trail stopping short of the grey X, with open water
     * between them and nothing saying why. Found on glass by Aaron.
     *
     * `endAt` is the SAME coordinate the X is drawn at (`map/markers.js` builds
     * that mark from `storm.lon/lat`), passed in rather than dug out of the
     * bundle, so the two cannot drift apart. */
    if (!forecast.length) past = extendToAnchor(past, endAt, label);
...
export const __internals = { stitch, orient, spline, unwrapLons, dLon, runsFrom, unfold, turnDeg, extendToAnchor };
