...
/**
 * The same curve, and the map back to the points it was drawn through.
 *
 * ==> IT EXISTS BECAUSE `smoothPath` THROWS AWAY SOMETHING `spline` ALREADY
 * COMPUTES. <== `spline` returns an `index` giving each control point's offset
 * in the output, and two of its three callers want the curve alone, so the map
 * was dropped at this door. The season clock (§57.23) needs it: to cut a track
 * at a moment in time it has to know WHICH vertices belong to which recorded
 * fix, and the alternative — matching coordinates back to fixes by distance
 * afterwards — is arithmetic that can be subtly wrong on a storm that loops
 * back over itself, which real storms do.
 *
 * ==> `kept` INDEXES THE CALLER'S OWN ARRAY, NOT AN INTERNAL ONE. <== Two
 * filters run in here before the curve is drawn: non-points are dropped, then
 * consecutive duplicates are merged. A caller holding a timestamp per point
 * must be told which of ITS points survived, or it lines the times up against
 * the wrong vertices — silently, and only on the storms that happen to have a
 * repeated fix in them. So `kept[j]` is an index into `pts` as it was handed
 * in, and `index[j]` is where that point landed in `curve`.
 *
 * `smoothPath` is now this function with the map discarded, so the two can
 * never draw different curves.
 *
 * @param {Array<[number,number]>} pts
 * @param {number} [maxVertices]
 * @returns {{curve:Array, index:number[], kept:number[]}}
 */
export function smoothPathIndexed(pts, maxVertices = TRACK_LINE.maxVertices) {
  const identity = (arr) => arr.map((_, i) => i);
  if (!Array.isArray(pts) || pts.length < 3) {
    return { curve: pts, index: identity(pts || []), kept: identity(pts || []) };
  }

  /* The filter and the dedupe both drop points, and BOTH have to be carried
   * through to the answer. `keptClean` walks the caller's array; `kept` then
   * reads it through dedupe's own surviving indices. */
  const clean = [];
  const keptClean = [];
  for (let i = 0; i < pts.length; i++) {
    if (isPt(pts[i])) { clean.push(pts[i]); keptClean.push(i); }
  }
  if (clean.length < 3) {
    return { curve: pts, index: identity(pts), kept: identity(pts) };
  }

  const { pts: unique, kept: keptUnique } = dedupe(clean);
  if (unique.length < 3) {
    return { curve: pts, index: identity(pts), kept: identity(pts) };
  }

  const { curve, index } = spline(unique, maxVertices);
  return { curve, index, kept: keptUnique.map((k) => keptClean[k]) };
}
