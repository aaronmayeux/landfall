 ...
 *
 * ==> AND IT RETURNS NULL FOR EVERY STORM IN AN NHC BASIN, WHICH IS A KNOWN
 *     HOLE AND NOT AN OVERSIGHT. <== §51.5. `mergeStorms` (data/merge.js)
 * drops every GDACS storm whose basin NHC covers, so an Atlantic, East Pacific
 * or Central Pacific storm reaches this app as its NHC record ALONE — the
 * GDACS twin, and its event id with it, are gone before any view sees a storm.
 *
 * That is the right dedupe rule and it costs something real here: GDACS
 * modelled 47 Hawaiian towns for Lala, a Central Pacific storm, and this
 * feature cannot reach them. The American half of surge is NHC's own product
 * (§4.8, §36), which is the better forecast where it publishes — but it only
 * answers while a US surge watch is in force. Closing the gap means joining
 * an NHC storm to a GDACS event id by NAME, which is the same class of join
 * §50.12 is currently measuring for CAP alerts and is not free. Decide it
 * there, with that evidence, rather than inventing a second name-matcher here.
 */
