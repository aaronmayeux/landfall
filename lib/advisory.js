...
/**
 * The gust speed out of NHC's coded FORECAST advisory (TCM), in knots.
 *
 * ==> WHY THIS NEEDS A SECOND PRODUCT AT ALL. <== NHC publishes no gust
 * anywhere the app already looks. `CurrentStorms.json` — the list every NHC
 * storm is built from — has an `intensity` and a `pressure` and no gust field
 * at any depth (checked against the live feed, 2026-08-20). The PUBLIC
 * advisory the drawer already fetches says only "Maximum sustained winds are
 * near 60 mph (95 km/h) with higher gusts" — a phrase, not a number, in every
 * one of the nineteen archived Bertha advisories. The TCM is the one product
 * that states it:
 *
 *     MAX SUSTAINED WINDS  85 KT WITH GUSTS TO 105 KT.
 *
 * So a GDACS storm showed a Gusts row and an NHC storm did not, and the reason
 * was not that we were dropping it — it was that nobody had asked the right
 * page for it.
 *
 * KNOTS, BECAUSE THE PRODUCT IS IN KNOTS. Everything on the panel converts out
 * of knots at render time, and converting here would mean converting twice.
 *
 * `\s+` BETWEEN THE WORDS AND AROUND THE NUMBER, for the same reason
 * `isNhcFinalAdvisory` does it: the product is fixed-column teletype and pads
 * its numbers to three characters, so `GUSTS TO  40 KT` has two spaces in it
 * and `GUSTS TO 105 KT` has one.
 *
 * NULL IS A REAL ANSWER AND THE ONLY SAFE DEFAULT. A dissipating system's
 * final advisory can drop the clause entirely, and a caller that read a
 * missing gust as zero would print "Gusts 0 mph" under a live storm. The row
 * is omitted, never zeroed — same rule as every other null on that panel.
 */
export function nhcGustKt(text) {
  const m = String(text || '').match(/GUSTS\s+TO\s+(\d{1,3})\s*KT/i);
  if (!m) return null;
  const kt = Number(m[1]);
  /* A SANITY BAND, NOT A CLAMP. Anything outside it means the pattern matched
   * something that is not a gust, and a wrong number on a hurricane panel is
   * worse than an absent row. The strongest gust ever measured in a tropical
   * cyclone is comfortably under 250 kt. */
  return Number.isFinite(kt) && kt > 0 && kt < 300 ? kt : null;
}
