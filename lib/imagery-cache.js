...
/* ===========================================================================
 * IN-FLIGHT COALESCING — the half of the cache that covers the gap.
 *
 * The store above answers "have we already got these bytes". It cannot answer
 * "are these bytes already on their way", and that window is exactly where the
 * duplicates were. `map/imagery.js` guards with `rec.busy`, which is a flag on
 * a DISC RECORD — and `setMode` throws every record away and builds new ones.
 * So a toggle to radar and straight back to satellite starts a second fetch
 * for a URL whose first fetch has not landed, on a fresh record that has never
 * heard of it. Same 768×768 frame down the wire twice, then decoded and pixel-
 * walked twice, on a phone, while the user is still moving their thumb.
 *
 * Keyed on the URL for the same reason the store is: the URL already encodes
 * mode, bird, box and size, and a second spelling of that is a second thing
 * that can disagree.
 *
 * WHAT IT DOES NOT DO: it does not decide whether the answer is still wanted.
 * Every caller still runs its own request-identity check on the way out, and a
 * shared promise resolving for a superseded request is discarded there exactly
 * as an unshared one would be (see the two gates in map/imagery.js).
 * ======================================================================== */

/** url -> Promise<Blob> for a fetch currently in flight. */
const inflight = new Map();

/**
 * Run `fetcher` for this URL, or join the run already happening.
 *
 * The entry is removed when the promise settles, success or failure — a failed
 * fetch must not leave a rejected promise parked under its URL where the retry
 * path would join it and be handed the same failure without ever asking again.
 */
export function fetchFrameOnce(url, fetcher) {
  const live = inflight.get(url);
  if (live) return live;

  const run = (async () => fetcher())().finally(() => {
    /* Identity-checked: a later call that already replaced this entry owns it
     * now, and clearing it blind would strand that one. */
...
/** How many fetches are in flight. Diagnostics and tests only. */
