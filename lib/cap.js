 ...
 * outlines and basin-sized boxes — see the relay's header — so we never
 * download them. GDACS publishes each storm's affected countries WITH ISO-2
 * codes already on them (`affectedcountries[].iso2`), and a CAP row carries
 * `countryCode` in the same alphabet, so the join needs no lookup table.
 *
 * Imports: nothing.
 */
 ...
 * other 35 are out at sea, where no country has issued anything about them,
 * so an empty answer here is usually the truth rather than a gap.
 */
