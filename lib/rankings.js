...
export const RANK_STATS = Object.freeze({
  ...
  ace: Object.freeze({
    ...
    /* ==> THE LABEL IS THE PANEL'S WORDING, NOT THE ACRONYM. <== §57.58. The
     * row stopped saying `ACE` when the gloss landed, and one statistic
     * carrying two names is how a panel comes to disagree with itself. Nothing
     * renders this string today — the section supplies its own label and the
     * rank sentence uses `superlative` — so it is documentation, and a wrong
     * one would be believed by the first renderer that reached for it. */
...
export function rankStorm(facts, table, basin, system) {
  ...
  for (const [key, def] of Object.entries(RANK_STATS)) {
    ...
    /* ==> `value` IS THE FIGURE `read` ALREADY GAVE, HANDED ON RATHER THAN
     * RECOMPUTED. <== §57.56. The distribution bar needs the storm's own
     * number to place its mark, and the only safe source is the one the rank
     * beside it was taken from. A renderer calling `def.read(facts)` again
     * would be a second reader of the same rule, free to drift from this one
     * the day an entry's `read` gains a floor — which `hoursAtMajor`,
     * `ace` and both distance entries all already have. */
    if (places.length) rows.push({ key, def, places, value: mine });
  }
