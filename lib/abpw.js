/**
 * abpw.js — JTWC's Significant Tropical Weather Advisories, parsed. SPEC §45.3.
 *
 * ==> TWO BULLETINS, ONE PARSER. <== JTWC publishes `abpwweb.txt` for the
 * Pacific and `abioweb.txt` for the Indian Ocean, and between them they cover
 * every ocean NHC does not:
 *
 *   ABPW  1. WESTERN NORTH PACIFIC (180 to the Malay Peninsula)
 *         2. SOUTH PACIFIC (west coast of South America to 135E)
 *   ABIO  1. NORTH INDIAN OCEAN (Malay Peninsula west to the coast of Africa)
 *         2. SOUTH INDIAN OCEAN (135E west to the coast of Africa)
 *
 * They are the same template with the ocean names swapped, so this file reads
 * both and `GENESIS.ABPW` holds one set of patterns for the pair. THE FILE
 * KEEPS ITS NAME because §45.3 and a dozen comments cite it, the same reason a
 * spec section keeps its number when it moves.
 *
 * ==> THE INDIAN OCEAN HALF WAS BUILT WITHOUT EVER SEEING A BUSY ABIO. <==
 * JTWC reissues it once a day and the archive's window has only held quiet
 * ones. What makes that safe rather than reckless is `noneAssertion`: a
 * disturbance block must either SAY it is empty or yield numbered items, and a
 * block that does neither makes the whole bulletin `unavailable`. So if the
 * body of a real Indian Ocean disturbance turns out to be worded differently
 * from a Pacific one, the app says it cannot read the bulletin. It does not
 * say the ocean is calm.
 *
 ...
 * Both numbered regions of both bulletins carry all three letters, so a
 * bulletin yields two B blocks and both are read.
 *
 * THE SHAPE IS OURS AND THE NUMBERS ARE NOT. JTWC states a position and no
 * extent, so `circleAround` draws a circle at the mean size of NHC's real
 * areas — see the note on `geometry` below. Everything else here is
 * transcribed.
 *
...
import { normalizeRisk, circleAround } from './genesis.js';
...
export function parseAbpw(text, { now = Date.now() } = {}) {
  ...
  while ((block = blocks.exec(flat)) !== null) {
    ...
    /* ==> A BLOCK WE DID NOT UNDERSTAND FAILS THE WHOLE BULLETIN. <== See
     * `noneAssertion`. Either JTWC said `NONE.` or it wrote a numbered list;
     * anything else is a rewording, and the empty list a rewording produces is
     * indistinguishable on screen from a genuinely calm ocean. This is the one
     * check that makes the difference visible. */
    if (A.noneAssertion.test(body)) continue;

    const items = fresh(A.itemPattern);
    let item;
    let itemCount = 0;
    while ((item = items.exec(body)) !== null) {
      itemCount += 1;
    ...
    if (itemCount === 0) {
      return {
        ...
        issuedAt,
        systems: [],
        reason: 'a disturbance block was neither empty nor a numbered list',
      };
    }
  }

  /* NO BLOCK AT ALL IS THE SAME FAILURE ONE LEVEL UP. Every bulletin JTWC has
   * ever published carries `B. TROPICAL DISTURBANCE SUMMARY:` in each of its
   * two regions, so finding none means the heading moved or the document is
  ...
  if (blockCount === 0) {
    return {
      ...
      issuedAt,
      systems: [],
      reason: 'no tropical disturbance section in the bulletin',
    };
  }
...
function parseItem(body) {
  ...
  return {
    ...
    /* ==> A CIRCLE WE DREW, AND THE ONLY GEOMETRY IN §45 THAT IS NOT
     *     TRANSCRIBED FROM A SOURCE. <==
     *
     * This said "NO GEOMETRY — do not invent a radius to draw a circle with",
     * and the reasoning was sound: JTWC publishes a position and no extent, so
     * any shape here is ours. It was also, on glass, wrong — a watched system
     * in the Western Pacific drew nothing at close zoom, so tapping its row
     * flew the camera to empty ocean and the app looked broken at the exact
     * moment it was being most careful. Aaron overruled it on 2026-08-09,
     * knowing what it costs.
     *
     * `GENESIS.jtwcRadiusDeg` is what keeps it defensible: 6.04°, the MEAN
     * EQUIVALENT RADIUS of NHC's real published areas, so this circle is the
     * size a watched area actually is rather than a size that looked right.
     * The area panel also says in words that the shape is indicative.
     *
     * IF JTWC EVER PUBLISHES AN EXTENT, this is the line that goes. */
    geometry: circleAround({ lon, lat }, GENESIS.jtwcRadiusDeg),
