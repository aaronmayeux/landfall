...
/**
 * Sample a shape's radius at N evenly spaced bearings around its centroid.
 *
 * This turns an arbitrary published polygon into a radial profile, which is
 * what makes two DIFFERENT shapes blendable: same bearings, same count, so
 * intermediate shapes are a per-bearing interpolation. It is the same
 * representation the NHC sweep works in (`radiusAtBearing`), reached from a
 * polygon instead of from four quadrant numbers.
 *
 * A bearing that misses the ring entirely (possible on a wildly concave
 * shape) inherits its neighbour rather than reading zero — a zero radius is
 * exactly the pinch this whole change exists to remove.
 */
function radialProfile(rings, c, samples) {
  const ring = rings[0];
  const prof = new Array(samples).fill(0);

  /* Longest ray from the centroid to a vertex, binned by bearing. Vertex
   * binning is enough at these sample counts: published band rings carry
   * 100+ vertices, so every bin lands on several. */
  for (const [x, y] of ring) {
    const dx = x - c[0];
    const dy = y - c[1];
    ...
    const bin = Math.min(samples - 1, Math.floor((th / (Math.PI * 2)) * samples));
    if (r > prof[bin]) prof[bin] = r;
  }

  /* Fill any empty bin from its nearest filled neighbour, walking both ways.
   * Never leave a zero. */
  const filled = prof.some((v) => v > 0);
  if (!filled) return null;
  for (let i = 0; i < samples; i++) {
    if (prof[i] > 0) continue;
    let a = 0, b = 0;
    for (let k = 1; k <= samples; k++) {
      const p = prof[(i - k + samples * 2) % samples];
      if (p > 0) { a = p; break; }
    }
    for (let k = 1; k <= samples; k++) {
      const n = prof[(i + k) % samples];
      if (n > 0) { b = n; break; }
    }
    prof[i] = Math.max(a, b);
  }
...
/** A closed ring from a centroid and a radial profile. */
function ringFromProfile(c, prof) {
  const n = prof.length;
  const out = new Array(n + 1);
  for (let i = 0; i < n; i++) {
    const th = (i / n) * Math.PI * 2;
    out[i] = [c[0] + prof[i] * Math.cos(th), c[1] + prof[i] * Math.sin(th)];
  }
  out[n] = out[0].slice();
  return [out];
}
 ...
 *
 * THE FIRST FIX WAS ALSO WRONG, and Aaron caught it on glass. It joined the
 * widest left/right vertex of each shape with ONE STRAIGHT QUAD. Where two
 * shapes differ in size — which is always, since storms strengthen and
 * weaken along a forecast — a straight taper does not follow the real widths
 * between them. Measured on a weakening storm: 0.6° before the final fix the
 * corridor was 1.54 wide where the true shape is 1.04. The corridor arrived
 * at its terminal shape as a WEDGE, driven to a point, instead of stopping
 * at that shape's own round edge. Aaron described the symptom exactly — the
 * ends behaved as though a zero radius had been assigned where a band was
 * absent.
 *
 * THE CONSTRUCTION NOW — interpolated shapes, not a taper:
 *   1. Reduce both shapes to radial profiles about their centroids.
 *   2. Step between them at a spacing tied to the grid, and at each step
 *      stamp a shape whose centre and per-bearing radius are linearly
 *      blended between A and B.
 *   3. The result follows the true widths the whole way, and a band that
 *      ends simply STOPS at its last published shape — that shape's round
 *      cap is preserved because nothing is drawn past it.
 *
 * WHY THIS CANNOT OVERSTATE, which matters because these are safety colors:
 * every intermediate radius is a linear blend of two published radii at the
 * same bearing, so it is bounded by them and can never exceed the larger.
 * Same guarantee `lib/windswath.js` gives for its own interpolation, and the
 * same reason it uses a bounded blend rather than a spline.
 *
 * @returns {Array<Array<Array<[number,number]>>>} shapes to stamp.
 */
function bridgeShapes(ringsA, ringsB) {
  ...
  if (gap < 1e-9) return []; // same place: nothing to bridge

  const n = BAND_MERGE.profileSamples;
  const pa = radialProfile(ringsA, ca, n);
  const pb = radialProfile(ringsB, cb, n);
  if (!pa || !pb) return [];

  /* Step fine enough that consecutive stamps OVERLAP rather than leaving
   * their own micro-gaps, but no finer — each stamp is a full scanline fill
   * and this runs in the warm-prefetch path for every storm.
   *
   * The right scale is the shape's own size, not a fixed cell count: two
   * circles of radius r overlap whenever their centres are under 2r apart,
   * so stepping at a FRACTION of the narrowest radius is both sufficient
   * and self-tuning. A wide 60 km/h band takes a few big steps; a tight
   * 120 km/h core takes more, smaller ones, which is exactly where the
   * fidelity is needed. A fixed 4-cell step cost 257 ms per storm on
   * realistic input; this brings it back down while stamping the same
   * corridor. */
  const minRadius = Math.min(Math.min(...pa), Math.min(...pb));
  const step = Math.max(
    BAND_MERGE.cellDeg * BAND_MERGE.bridgeMinStepCells,
    minRadius * BAND_MERGE.bridgeStepFraction
  );
  const steps = Math.min(Math.ceil(gap / step), BAND_MERGE.maxBridgeSteps);
  if (steps < 2) return [];

  const out = [];
  /* Interior only: t=0 and t=1 are the published shapes themselves, already
   * stamped by the caller. */
  for (let s = 1; s < steps; s++) {
    const t = s / steps;
    const c = [ca[0] + dx * t, ca[1] + dy * t];
    const prof = new Array(n);
    for (let i = 0; i < n; i++) prof[i] = pa[i] + (pb[i] - pa[i]) * t;
    out.push(ringFromProfile(c, prof));
  }
  return out;
}
/**
  ...
  if (BAND_MERGE.bridgeGaps && polygons.length > 1) {
    ...
    if (withCentroids.length > 1) {
      ...
      for (let i = 0; i + 1 < withCentroids.length; i++) {
        const shapes = bridgeShapes(withCentroids[i].rings, withCentroids[i + 1].rings);
        for (const s of shapes) stampPolygon(grid, s, w, h, minX, minY, cell);
      }
    }
  }
