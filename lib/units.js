...
/** A signed wind CHANGE, knots → a whole number in the reader's unit, sign
 *  kept. These are the §47.4 contribution figures — always changes, never
 *  speeds, so they always carry a sign and never a unit word of their own
 *  (the paragraph states the unit once, on the headline). */
export function windDelta(kt, system) {
  if (!isNum(kt)) return null;
  const imperial = resolveSystem(system) === UNITS.IMPERIAL;
  return Math.round(kt * (imperial ? MPH_PER_KT : KMH_PER_KT));
}

/** The unit word alone — "mph" or "km/h" — for sentences that state it once
 *  over several signed figures. */
export function windUnitWord(system) {
  return resolveSystem(system) === UNITS.IMPERIAL ? 'mph' : 'km/h';
}

/**
 * Convert a headline and its named parts TOGETHER so the visible figures add
 * up to the visible total in whatever unit is on screen — SPEC §47.4, built
 * for the storm health paragraph (§47.8) and used nowhere else.
 *
 * Each SHIPS term is a whole number of knots and the terms sum exactly; round
 * each to mph independently and the sum no longer closes. The fix is a
 * CLOSING REMAINDER rather than nudging any named figure: every named term
 * converts with the same plain rounding every other wind figure in the app
 * uses (so the same term never prints two different values on two surfaces),
 * and the remainder is computed as displayed-total minus displayed-named — so
 * the parts sum to the total by construction, which is the property §47.4
 * requires. Verified against all four §47.8 acceptance cases: the remainder
 * comes out −2, −4, −2 and (Genevieve, where the named terms close on their
 * own) 0.
 *
 * @param {number} totalKt   the headline, knots
 * @param {number[]} namedKt the named terms, knots
 * @returns {{ total:number, named:number[], remainder:number }} whole numbers
 *   in the reader's unit, signed; `total === sum(named) + remainder` always.
 */
export function closeWindParts(totalKt, namedKt, system) {
  const total = windDelta(totalKt, system);
  const named = namedKt.map((kt) => windDelta(kt, system));
  const remainder = total - named.reduce((a, b) => a + b, 0);
  return { total, named, remainder };
}

/** Wind RANGE: a floor/ceiling pair in knots → one string, open-ended forms
 *  included. Exists for the GDACS band bracket (lib/wind-bracket.js), whose
 *  whole point is that the source gives a range and not a number — collapsing
 *  it to a midpoint here would undo that honesty at the last moment. Bounds
 *  render in the reader's units with the stored knots in parens, same shape
 *  as every other wind row:
 *    both bounds   → "37–56 mph (32–49 kt)"
 *    floor only    → "At least 75 mph (65 kt)"
 *    ceiling only  → "Under 37 mph (32 kt)"
 *  Null both ways → the em-dash, like every formatter above. */
export function formatWindRange(loKt, hiKt, system) {
  const lo = isNum(loKt), hi = isNum(hiKt);
  if (!lo && !hi) return MISSING;
  const imperial = resolveSystem(system) === UNITS.IMPERIAL;
  const per = imperial ? MPH_PER_KT : KMH_PER_KT;
  const unit = imperial ? 'mph' : 'km/h';
  const conv = (kt) => Math.round(kt * per);
  if (lo && hi) {
    return `${conv(loKt)}–${conv(hiKt)} ${unit} (${Math.round(loKt)}–${Math.round(hiKt)} kt)`;
  }
  if (lo) return `At least ${conv(loKt)} ${unit} (${Math.round(loKt)} kt)`;
  return `Under ${conv(hiKt)} ${unit} (${Math.round(hiKt)} kt)`;
}
 ...
 *
 *  Deliberately an ALIAS, not a copy — the math is identical today and
 *  duplicating it would mean two roundings to keep in step. What this buys is
 *  a NAMED SEAM: storm MOTION and storm WIND are different quantities that
 *  happen to share a unit, callers import the one they mean, and the day
 *  motion needs its own rounding it becomes a real function here without
 *  touching a single call site. */
export const formatSpeed = formatWind;

/**
 * How many nautical miles one DISPLAYED unit is, for the current system.
 *
 * Exists for one caller and one reason: a chart that wants gridlines at round
 * numbers IN THE READER'S UNITS. Everything in this app stores nautical miles,
 * so a grid picked at a round nm interval converts to 58 / 115 / 173 mi —
 * arithmetically right and unreadable as a scale. Picking the interval in
 * miles and converting BACK is the only way to get 50 / 100 / 150 on the axis,
 * and this is the conversion that lets a caller do that without knowing the
 * ratios, which stay private to this file.
 */
export function nmPerDisplayUnit(system) {
  return 1 / (resolveSystem(system) === UNITS.IMPERIAL ? MI_PER_NM : KM_PER_NM);
}
