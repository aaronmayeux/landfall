...
/* Where the precision steps down. Both are in DISPLAY units, checked after the
 * conversion, so a metric reader and an imperial one see the tier change at the
 * same physical distance to within a rounding step — the alternative is a
 * threshold in nm that lands mid-number on one of the two systems.
 *
 * COARSE_ABOVE is the third tier and it is the one with an argument behind it.
 * A storm 6,272 miles away is a position off an advisory that is already hours
 * old, on a system moving 5 mph, quoted against a house whose own coordinates
 * came out of a geocoder. The last digit of that number is not a measurement,
 * it is a rendering artefact, and printing it invites the reader to believe the
 * app knows something it does not. Rounded to COARSE_STEP the figure says the
 * true thing: about six thousand three hundred miles, which is "not your
 * problem" in every unit ever invented. Below the threshold the exact figure
 * starts to matter and is kept. */
const FINE_BELOW = 10;      // under this, one decimal — "0 mi" for 0.4 is wrong
const COARSE_ABOVE = 1000;  // over this, the last digit is noise
...
export function formatDistance(nm, system) {
  ...
  if (v < FINE_BELOW) return `${v.toFixed(1)} ${unit}`;
  if (v >= COARSE_ABOVE) {
    return `${(Math.round(v / COARSE_STEP) * COARSE_STEP).toLocaleString()} ${unit}`;
  }
  return `${Math.round(v).toLocaleString()} ${unit}`;
}

/**
 * A coordinate as a person reads it: `12.6°N 36.3°W`.
 *
 * ==> HERE BECAUSE TWO VIEWS NEEDED IT (§12). <== It lived in
 * ui/view-area-detail.js and the storms list wanted the same thing at a coarser
 * precision. Two copies of a coordinate formatter is how one of them ends up
 * printing 36.3°W and the other -36.3°E for the same area in the same drawer.
 *
 * `decimals` exists because the two callers are asking different questions. The
 * detail panel is stating where an area IS, and tenths of a degree is the
 * precision the centroid of a fuzzy polygon can honestly carry. The list row is
 * only telling two identically-named rows apart, and whole degrees do that with
 * less width and no implied accuracy.
 *
 * Degrees are degrees in both unit systems, so this takes no `system`.
 */
export function formatCoords(lon, lat, decimals = 1) {
  if (!isNum(lon) || !isNum(lat)) return MISSING;
  const ns = lat >= 0 ? 'N' : 'S';
  const ew = lon >= 0 ? 'E' : 'W';
