 ...
 * it never had.
 *
 * ==> AND A RING CAN NOW ARRIVE ALREADY UNWRAPPED, WHICH THE PARAGRAPH ABOVE
 * DID NOT ANTICIPATE. <== `lib/windswath.js` emits the built envelope past
 * ±180 on purpose — that is what makes MapLibre draw one shape across the seam
 * rather than two on opposite rims, and it is the same convention
 * `lib/trackline.js` and `lib/cone-sweep.js` already use. A ring running
 * -181.2 to -170.4 is perfectly continuous, spans only 10.8°, and so is NOT
 * flagged wrapped by the span test — its box is simply [-181.2, -170.4], and
 * every town near +179 falls outside it and is never counted. A silent
 * undercount on exactly the storms this section matters most for.
 *
 * So longitudes are folded back into (-180, 180] on the way in, BEFORE the box
 * is taken. A ring that straddles the seam then measures ~358° wide, trips the
 * span test, and goes down the shift path that was already here. One extra
 * step at the door rather than a second dateline mechanism beside the first.
 */
...
/**
 * Fold a ring's longitudes back into (-180, 180], or hand the ring straight
 * back if they are all already there.
 *
 * ==> THE CHEAP TEST FIRST, BECAUSE ALMOST NOTHING NEEDS THIS. <== Every ring
 * from a source is already in range and every ring from the swath builder is
 * in range too unless that storm is on the seam. The scan is one pass with no
 * allocation; the rewrite only happens for the handful of rings that need it,
 * so the common path costs a comparison per vertex and nothing else.
 *
 * WHY NOT A MODULO. `((lon + 180) % 360 + 360) % 360 - 180` puts exactly +180
 * onto -180, and NHC publishes vertices at exactly ±180 by the thousand
 * (that is where it cuts a shape). Moving them flips a closing edge from
 * zero-length to 360° long. The loop below leaves anything already in range
 * untouched, including +180 itself.
 */
function foldIntoRange(ring) {
  let out = null;
  for (let i = 0; i < ring.length; i += 1) {
    const lon = ring[i][0];
    if (lon <= 180 && lon > -180) {
      if (out) out.push(ring[i]);
      continue;
    }
    if (!out) out = ring.slice(0, i);
    let v = lon;
    while (v > 180) v -= 360;
    while (v <= -180) v += 360;
    out.push([v, ring[i][1]]);
