 ...
 *
 * ==> EXPORTED SINCE §56.6, BECAUSE THE PANEL'S "Show on the globe" NEEDS THE
 * SAME POINT THE CHIP WAS DRAWN AT. <== Flying to a bounding-box centre or to
 * the first vertex would land the reader beside the mark rather than on it, and
 * a second way of computing "where is this alert" is a second answer that can
 * disagree with the first. One function, one cache, one point.
 *
 ...
 */
export function pointForAlert(alert, cache) {
  const key = alert.id;
