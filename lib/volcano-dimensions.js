...
/**
 * ONE metre of ridge geometry, expressed in the units MapLibre's custom-layer
 * matrix actually wants. A single scalar, applied to all three axes.
 *
 ...
 * and Y get. Passing raw metres put a 3.5 km cone at 43,750, i.e. forty-three
 * thousand times the width of the planet, and every mountain became a needle
 * stretching past the far clip plane.
 *
 * ==> RETURNING ONE NUMBER RATHER THAN TWO IS THAT FIX MADE STRUCTURAL. <==
 * The old shape of this function returned a width and a height separately,
 * which is exactly the shape that let one of them be in different units from
 * the other and go unnoticed for four deploys. There is now no per-axis number
 * to get wrong: height exaggeration (`vertical`) is baked into the geometry in
 * METRES by `lib/volcano-ridge.js`, and what happens at draw time is a uniform
 * scale. `inflate`'s "may never become two curves" rule holds by construction
 * instead of by discipline.
 *
 * `unitsPerMetre` is MapLibre's `meterInMercatorCoordinateUnits()`, which
 * depends on latitude, so the caller reads it per cluster.
 *
 * @param {number} zoom          map.getZoom()
 * @param {number} unitsPerMetre one metre, expressed in mercator units
 * @returns {number} the uniform scale for a ridge mesh built in metres
 */
export function ridgeScale(zoom, unitsPerMetre) {
  return inflationAt(zoom) * unitsPerMetre;
}
