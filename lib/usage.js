 ...
 *   surge     modelled coastal flooding could not load (§51.3)
 *   flood     the national flood alert list could not load (§48.21)
 *
 * ==> THIS LIST IS AN ALLOWLIST AND A NAME MISSING FROM IT IS DROPPED IN
 * SILENCE. <== Every press of the Environment retry between §47.8 shipping and
 * 2026-08-21 was counted as nothing, because the caller passed `env_retry` and
 * this list said `env`. Adding a Retry button anywhere means adding its name
 * here in the same change.
 */
export const RETRY_BUTTONS = Object.freeze(['storms', 'geometry', 'env', 'rain', 'surge', 'flood']);
