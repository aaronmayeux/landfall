 ...
 *  Reuses the app's own list rather than keeping a second one — subtropical
 *  storms come ashore too. */
const isCyclone = (status) => SEASONS.cycloneStatuses.includes(String(status || '').toUpperCase());

/** True when the status is HURDAT2's extratropical code. Its own list rather
 *  than a bare `=== 'EX'` because ATCF and IBTrACS spell this differently and
 *  step 13 will add to it. */
const isPostTropicalCode = (status) =>
 ...
 *
 * ==> `EX` MEANS TWO DIFFERENT THINGS AND THIS IS WHERE THEY ARE SEPARATED.
 * <== §57.7c. Sandy was 70 kt six hours after transition when she crossed the
 * New Jersey coast; the same code sits on a decayed remnant wandering over
 * Newfoundland five days later. HURDAT2 gives them one letter, so the only
 * separators available are strength and sequence:
 *
 *   - **Still at least tropical-storm force.** `postTropicalLandfallMinKt`.
 *     52 of the archive's 404 extratropical crossings fall under it.
...
export function landfallNature(status, windKt, time, firstCycloneTime) {
  if (isCyclone(status)) return 'tropical';
  if (!isPostTropicalCode(status)) return null;
 ...
 *  it never did. The anchor `landfallNature` uses to tell a post-tropical
 *  landfall from a pre-tropical one. */
export function firstCycloneTime(points) {
  for (const p of points || []) {
    if (isCyclone(p?.status) && Number.isFinite(p?.time)) return p.time;
  }
  return null;
}
