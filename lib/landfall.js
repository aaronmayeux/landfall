...
/** True when the status says the system is a former cyclone rather than a
 *  current one. `EX` and `LO` — NWS Instruction 10-604 makes the extratropical
 *  cyclone and the remnant low two classes of one thing, both post-tropical,
 *  and `postTropicalStatuses` carries the whole argument. Step 13 adds
 *  IBTrACS's spellings once a probe has read them; nothing is added here from
 *  a published code table nobody has seen in a real file. */
