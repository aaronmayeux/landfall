...
/**
 * Simplify an OPEN polyline — a path with two ends, not a ring.
 *
 * EXTRACTED FOR THE PLATE BOUNDARIES (§12: any pattern used twice gets
 * extracted). `simplifyRing` cannot serve them: it treats the last point as a
 * structural repeat of the first and re-closes the result, which on an open path
 * invents a segment joining its two ends.
 *
 * ==> AND THE PLATE BOUNDARIES USE IT FOR A DIFFERENT REASON THAN THE WIND
 * BANDS DO. <== There, simplification buys frame budget and the ideal would be
 * to keep every vertex. Here it is a LOOK decision: PB2002 digitises mid-ocean
 * ridges on a grid, so a ridge arrives as a literal staircase — measured on the
 * Mid-Atlantic Ridge, the MEDIAN turn angle between consecutive segments is
 * 83.8°, with 106 of 171 turns steeper than 70°. No amount of curve-fitting
 * fixes that, because the corners are the published vertices. They have to go
 * before the spline sees them.
 *
 * Same floor as `simplifyRing` and the same reason: a path that cannot survive
 * the tolerance is returned untouched rather than as a husk.
 */
export function simplifyPath(points, tolerance) {
  if (!Array.isArray(points) || points.length <= SIMPLIFY.skipUnderPoints) return points;
  const out = douglasPeucker(points, tolerance);
  return out.length < SIMPLIFY.minPathPoints ? points : out;
}
