...
/**
 * How long that wind is on the house, in words. ONE implementation, because
 * three surfaces say it and they were saying different things.
 *
 * ==> IT SHIPPED READING "for at least about an hour". <== The hedge lived in
 * two places at once: the caller supplied "about" or "at least", and the
 * duration string supplied its own "about"/"under". Concatenating them
 * produced "about about an hour" on an ordinary window and "about under an
 * hour" on a short one — and the chart's aria-label, which is everything a
 * screen reader gets, dropped the "at least" altogether and understated an
 * open-ended window as a definite one.
 *
 * WHY "at least" IS NOT DECORATION. An open-ended window is one NHC stopped
 * publishing radii for while the house was still inside it, so its length is
 * a FLOOR, not a measurement. Understating how long dangerous wind lasts is
 * the unsafe direction to be wrong in (§5), so the floor has to be worded as
 * one — and the two words can never contradict each other again, because
 * there is now only one place to write them.
 *
 * SUB-HOUR WINDOWS GET MINUTES, NOT "under an hour". "At least under an hour"
 * is a floor and a ceiling in the same breath. Minutes are rounded to five so
 * a hedged figure is not dressed up as a stopwatch reading.
 *
 * THE BUCKETS ROUND DOWN WHERE IT MATTERS. "an hour" starts at 0.9 h rather
 * than 0.75 so a 49-minute floor is never reported as an hour — a floor may
 * always be stated lower than it is and never higher. Ida's Advisory 14 is
 * exactly that case: 0.82 h of hurricane force at the house.
 */
export function windDurationPhrase(hours, openEnded = false) {
  if (!Number.isFinite(hours) || hours <= 0) return null;
  const lead = openEnded ? 'at least' : 'about';
  const amount =
    hours >= 1.5
      ? `${Math.round(hours)} hours`
      : hours >= 0.9
        ? 'an hour'
        : `${Math.max(5, Math.round((hours * 60) / 5) * 5)} minutes`;
  return `${lead} ${amount}`;
}

/**
 * The whole clause, preposition included, so no caller can build "for null".
 *
 * ==> THE ZERO-LENGTH WINDOW CAME BACK IN A NEW SHAPE. <== A window still open
 * when the radii stop is closed at the last published hour — but when a storm
 * publishes radii at ONE hour only, and the house is already inside them, the
 * last published hour IS the first, and the window has no length at all. Ida's
 * Advisory 17 is exactly that: she is inland, the house sits 27.5 nm inside her
 * 34 kt field, and tau 0 is the only set she publishes. The chart's summary
 * rendered "reach your home for null" and the headline rendered a blank.
 *
 * The truth is neither "no time" nor "at least an hour". It is that the wind is
 * on the house and the forecast does not say for how long — a real answer, and
 * a different one from both. Understating the duration of a wind that is
 * blowing right now is the unsafe direction, so it gets said rather than
 * rounded to something sayable.
 */
export function windDurationClause(hours, openEnded = false) {
  const phrase = windDurationPhrase(hours, openEnded);
  if (phrase) return `for ${phrase}`;
  return openEnded
    ? 'and the forecast stops before saying for how long'
    : 'for no time the forecast can measure';
}
