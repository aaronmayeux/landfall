 ...
 *
 * NOT CONFIRMED LIVE. Phase 4's watch/warning code field (`tcww`) was read
 * off a real feature before it was trusted; this one has not been, because
 * the sandbox cannot reach NOAA. `radii` is the documented ArcGIS field name
 * for the wind radii layers and is the best available guess — but a guess is
 * what it is, and the fallback below is therefore load-bearing rather than
 * belt-and-braces.
 *
 * VERIFY on glass: read one wind feature's properties in the browser and
 * either confirm this name or replace it (SPEC §15).
 */
