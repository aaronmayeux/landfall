 ...
 *
 * ==> AT MODULE SCOPE BECAUSE THERE ARE TWO CALLERS NOW. <== §6: any pattern
 * used twice is extracted before the second use. `sweepConeDetail` builds caps
 * for the cone it redraws and `measureConeRibs` builds them for the cone it
 * only measured, and a second copy is how the two later disagree about which
 * width pairs with which quarter — a mistake that does not throw and shows up
 * only on an asymmetric cone.
 *
 * ==> AND IT IS WHY THE MEASURED PATH DOES NOT CUT ITS CAPS OUT OF THE
 * PUBLISHED POLYGON. <== That was tried first, and it is the obvious move when
 * the real shape is already in hand: clip the ring to the half-plane ahead of
 * the last station. It is exact on a cone that runs roughly straight and WRONG
 * on a hooked one, because the region ahead of that plane is then two
 * disconnected pieces — the nose, and a stretch of the cone's own body further
 * back that the recurve has carried across the same line. Clipping returns both
 * joined by a zero-width bridge, which is a self-intersecting polygon. Measured
 * on Ida 006A: one cap in thirty-five, 418 vertices, and it would have been a
 * hole punched through the veil at the day-5 circle.
 *
 * A parametric half-ellipse cannot do that. It is anchored to the real cone at
 * three points — both flanks and the reach dead ahead, all three ray hits on
 * the published outline — and everything between them is swept, so there is no
 * fourth crossing for it to find.
 *
 ...
 */
export function capRing(c, t, n, reach, plusWidth, minusWidth, unscale) {
