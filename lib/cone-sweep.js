...
export function ringOnBranch(ring, ref) {
 ...
 */
export function rayHit(p, d, rings, limit) {
  let best = Infinity;
...
/** How many vertices one quarter of a cap gets, for a cap reaching `r`. */
const capSteps = (r) => Math.max(CONE_SWEEP.minCapSteps,
  Math.min(CONE_SWEEP.maxCapSteps, Math.ceil((Math.PI * r) / (2 * CONE_SWEEP.stepDeg))));

/**
 * One end cap as a closed ring, in lon/lat.
 *
 * ==> IT IS A MODEL OF THE END, WHICH IS RIGHT HERE AND WAS WRONG NEXT DOOR.
 * <== This file is INVENTING a shape — it has already replaced the published
 * outline with a swept one — so an ellipse anchored at the two flanks and the
 * reach dead ahead is the same kind of answer as everything around it, and the
 * undercut guard prices it. `lib/cone-measure.js` is not inventing anything,
 * and when it briefly used this builder its caps sat up to 17.5 km off the ring
 * actually on screen; it walks the real arc instead. Two different jobs, two
 * different caps, and neither is a copy of the other.
 *
 ...
 * flank at ±90° round to dead ahead, so the `+1` call takes the flank width on
 * the `+n` side and the `-1` call the `-n` side. Swap them and the cap comes
 * out inside out, silently, and only on the storms where the two sides
 * genuinely differ.
 *
 * The flank endpoints themselves are excluded by `capQuarter` (the flank owns
 * them), so the ring is closed by adding them back. Without that a cap is a
 * lens floating just past the cone rather than a shape joined to it, and a
 * hairline of veil shows through at the seam.
 *
 * ==> DEDUPED, FOR THE SAME REASON THE BODY RING IS. <== Both quarters own the
 * point dead ahead — one ends there, the other starts there — so every cap
 * carried a repeated vertex, and a repeated vertex is a zero-length segment
 * with no direction: enough to make a self-intersection test report a crossing
 * that is not there, and enough to hand MapLibre a degenerate edge to
 * triangulate.
 *
 * @param {[number,number]} c  the station the cap sits beyond, scaled frame
 * @param {[number,number]} t  the direction the cap reaches in
 * @param {[number,number]} n  that station's normal
 * @param {number} reach       how far ahead of `c` the cap goes
 * @param {(p:[number,number]) => [number,number]} unscale
 */
function capRing(c, t, n, reach, plusWidth, minusWidth, unscale) {
  const steps = capSteps(reach);
  const raw = [
...
export function sweepConeDetail(trackLonLat, rings) {
  ...
  /* ==> THE BACKWARD RAY STOPPED MEASURING THIS CONE'S END WHEN THE TRACK
   * STOPPED STARTING AT THE PUBLISHED CONE'S APEX. <== §7.9, §7.11.
   *
   * `reachStart` casts backwards from the first station until it leaves the
   * published outline, and that WAS the right measurement: the track began at
   * tau 0, which is the apex, so the ray hit the apex a short way back and the
   * cap came out as the small rounded nose a cone actually has.
   *
   * Since §7.11 the first station is the STORM, which sits well inside the
   * published cone — ahead of its stale apex. The ray now runs all the way
   * through the leftover tail before it exits, and the cap balloons to fit.
   * Aaron saw it on Moke as a purple lobe hanging east and south of the storm,
   * far outside the cone it is supposed to be capping (§47.5 paints the caps,
   * which is what made a long-standing wrong shape suddenly obvious).
   *
   * MEASURED on the archived bytes, 2026-08-22. Reach against the flank width
   * it is capping:
   *
   *   Lala  start 0.48 / 0.52 = 0.91x     end 2.35 / 2.33 = 1.01x
   *   Moke  start 1.56 / 0.64 = 2.43x     end 2.32 / 2.26 = 1.03x
   *
   * Three of those four agree within a tenth, because the end of a corridor
   * half-width `w` IS a cap of radius about `w`. The outlier is not a longer
   * cone end; it is a different quantity being measured.
   *
   * So the flank width becomes a CEILING rather than only a fallback. In the
   * healthy case it changes nothing — Lala's start is already under it. It
   * cannot shrink a genuine apex either: at a real tau-0 the published cone is
   * a circle of that same radius, which is where the 0.91x comes from.
   *
   * ==> THE NOSE CAP IS DELIBERATELY NOT CLAMPED, AND THE ASYMMETRY IS THE
   * POINT. <== The LAST station is still the published cone's own day-5 nose —
   * the track ends where the cone ends — so `reachEnd` measures this cone's
   * end and is right to. Only the start moved. Clamping both would shave a few
   * percent off every correct nose to fix one wrong tail. */
  const aheadStart = Math.min(
    Number.isFinite(reachStart) ? reachStart : Infinity,
    Math.max(wL[0], wR[0])
  );
  ...
  /* ==> THE WALLS FOLDING IS NOT THE SAME QUESTION AS THE RING CROSSING, AND
   * TESTING THE WRONG ONE TURNED THIS REBUILD OFF ON LALA. <==
   *
   * This used to refuse whenever either EDGE stepped backwards against the
   * track's own direction (a `folds` helper, now deleted). That is a good smell
   * but it is not the harm: the harm is a RING that crosses itself, because
   * MapLibre fills the doubled-over region as a hole (§7.9).
   *
   * MEASURED on the archived bytes, 2026-08-22: Lala's flanks fold on her
   * recurve and the assembled ring does NOT cross itself. A wall can reverse
   * into a cusp without the boundary ever crossing. So the rebuild was refusing
   * a shape that was safe to draw, falling back to the published outline, and
   * taking §47.5's environment ribbon down with it.
   *
   * Now the ring is assembled first and asked directly, and any loop it does
   * carry is CUT rather than being a reason to give up — the same operation the
   * wind swath's walls get, from the same file, for the same reason
   * (lib/unloop.js, §7.12). Refusal is kept for a ring that still crosses after
   * cutting: that is a shape nobody has measured, and drawing a cone with a
   * hole in it is worse than drawing the published one. */
  ...
  for (let cuts = 0; ; cuts++) {
    ...
    if (!hit) break;
    if (cuts >= CONE_SWEEP.maxLoopCuts) return null;
    cutLoop(ring, hit);
  }
   ...
   * <== §7.11 moved the forecast track's start onto the storm's real position.
   * The published cone still starts at the ANALYSIS position, which on a lagged
   * advisory is a long way back — measured on Lala 2026-08-22, advisory 36A's
   * apex sat at 26.65N while the feed had her at 28.60N, 135 miles ahead.
   *
   * A cone redrawn along the track cannot cover ground the track no longer
   * crosses, so this guard read that as a 1.62 degree undercut against a 1.27
   * allowance and refused — on a rebuild that was otherwise fine. Before the
   * re-anchor the same measurement was 0.12 degrees.
   *
   * ==> SO THE GUARD ASKS ITS QUESTION ONLY WHERE THE TRACK ACTUALLY GOES. <==
   * A published vertex whose projection falls BEHIND the first station is
   * skipped. Everything from the storm forward is still checked exactly as it
   * was, so the guard's real job — catching a cone that does not belong to this
   * track — is untouched.
   *
   ...
   * happened — water the storm has crossed — and nobody is at risk from a claim
   * about where it might have been yesterday. Drawing it would be §7.11's lie
   * in a third place. */
  const behindStart = (v) =>
    (v[0] - track[0][0]) * tan[0][0] + (v[1] - track[0][1]) * tan[0][1] < 0;
  for (const r of ringsXY) {
    for (let i = 0; i < r.length - 1; i++) {
      ...
      for (let j = 0; j < steps; j++) {
        const t = j / steps;
        const v = [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t];
        if (behindStart(v)) { behind++; continue; }
        checked++;
        const d = outsideBy(v, ring);
        if (d > worst) worst = d;
      }
    }
  }
  ...
  /* ==> A CONE THAT IS MOSTLY BEHIND THE STORM IS NOT A FORECAST. <== The skip
   * above is right for a tail; it is not a licence to redraw a cone the track
   * barely touches. If most of the published outline sits behind the first
   * station, the advisory is old enough that the shape says almost nothing
   * about where the storm is going, and quietly drawing a sliver of it as if it
   * were the whole cone would be exactly the confident-wrong failure §5 is
   * about. Refuse and let the published outline draw with its own age stamp. */
  if (checked < (checked + behind) * CONE_SWEEP.minAheadFrac) return null;
  ...
  return {
    ...
    capStart: capRing(track[0], [-tan[0][0], -tan[0][1]], nrm[0],
      aheadStart, wL[0], wR[0], unscale),
    capEnd: capRing(track[last], tan[last], nrm[last],
      aheadEnd, wL[last], wR[last], unscale),
  };
}
