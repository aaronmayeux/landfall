...
function asideFigures(run, hr, sys) {
  ...
  /* ==> VERBS, NOT SIGNED FIGURES, AND "+0 mph" IS WHY. <== The first version
   * printed `signed()` values into a sentence and a storm whose structure
   * contributed nothing read "its own structure +0 mph", which is a number
   * pretending to be information. A figure belongs in the grid, where a column
   * of signs is the register; in a sentence it has to be said as a verb, and a
   * zero has a word of its own. */
  const verb = (v) => (v > 0 ? `adds ${v} ${unit}`
    : v < 0 ? `costs ${Math.abs(v)} ${unit}`
      : 'counts for nothing');

  /* ==> NAME THE QUANTITY, NEVER THE STATE, BECAUSE THIS IS A DIFFERENT HOUR
   * FROM THE ROOM SENTENCE. <== The room sentence is quoted at the FIX (§47.8);
   * everything under the grid is read at the VERDICT hour, which can be five
   * days later with the storm forecast far stronger. Lala caught this on glass:
   * her paragraph said "plenty of room to grow" from hour 0 while a first
   * draft of this note said "being up against its ceiling" from +120 h. Both
   * true, read as a contradiction. Naming the gap rather than asserting a state
   * is symmetric in both signs and cannot argue with the sentence above it —
   * and the opener says out loud which hour these belong to. */
  const parts = [];
  if (room != null) {
    parts.push(`the gap between its strength and the sea's ceiling ${verb(room)}`);
  }
  if (struct != null) parts.push(`the storm's own structure ${verb(struct)}`);

  const lead = parts.length === 1
    ? 'one other thing moves the forecast and is not counted above'
    : 'two other things move the forecast and are not counted above';
  return `At that same hour, ${lead}: ${parts.join(', and ')}.`;
}
...
export function envHealth(result, { system = null, stormName = null } = {}) {
  ...
  if (run.lastPositionHr != null && run.lastWindHr != null &&
    ...
    notes.push('The model publishes wind further along the track than it publishes positions, so the environment can only be shown for part of it.');
  }
