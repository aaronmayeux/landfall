...
import {
  at, cap, extremeOf, lastDrawableHr, lastForecastHr, series, signed,
} from './env-series.js';
 ...
 *  because three sentences branch on it — the room sentence, the water
 *  sentence and the bottom line's closing clause — and copies of one ratio is
 *  how they later disagree about the same storm in the same paragraph. */
...
/** Which of §47.8's three room bands a ratio falls in. Shared by the room
 *  sentence and the water sentence so the two can never describe the same
 *  storm as being in two different amounts of trouble. */
function roomBand(r) {
  if (r == null) return null;
  if (r >= ENV_HEALTH.roomNearRatio) return 'near';
  if (r >= ENV_HEALTH.roomFarRatio) return 'some';
  return 'plenty';
}

/**
 * THE WATER SENTENCE — what the sea can hold at the far end of the drawn
 * track, said only when it changes the room story.
 *
 * ==> THE COLOUR CANNOT SAY THIS AND MUST NOT BE MADE TO. <== §47.4 excludes
 * `SST POTENTIAL` from the ribbon for a measured reason that still stands: it
 * is how far below its own ceiling the storm sits, so folding it in would
 * paint the season's only major hurricane as its most hostile environment.
 * `POT. INT.` is the other number — the ceiling itself, absolute, a fact about
 * the water rather than about the storm — and it moves a long way along a
 * cone. Measured on the 2026 corpus: it falls by a median of 12 kt over the
 * drawn track, by 20 kt or more in 37.7% of runs, and **34 runs end in the
 * ramp's brightest violet over water whose ceiling has collapsed by 20 kt or
 * more.** Lala's 22 Aug run is the case in one line: her air improves from +1
 * to +34 kt, so the cone brightens the whole way, while the sea's ceiling
 * drops 130 kt to 87 and the published forecast has her falling 70 kt to 24.
 * Nothing there is wrong. The cone simply is not answering that question, and
 * a reader looking at bright violet has no way to know it.
 *
 * ==> THE GATE IS THE ROOM BAND, NOT A KNOT THRESHOLD, AND THAT IS WHY IT IS
 * QUIET. <== A ceiling falling 168 kt to 131 over a 50 kt storm is a 37 kt
 * move and means nothing — the water still holds three times what the storm
 * is, and a sentence about it is noise. So this fires only when the ratio the
 * sentence above it already uses lands in a DIFFERENT band at the far end
 * than at the fix. Measured: that is 40 of 342 runs, 11.7%, and **all forty
 * tighten** — the room never widens down a track, because storms move
 * poleward into cooler water. One gate, no new constant, and it can never
 * contradict the sentence it follows.
 *
 * ==> IT REPORTS AND DOES NOT PREDICT (§47.8). <== Two published figures, a
 * time, and no verb about what the storm will do. "The water ahead of it
 * holds less" is `POT. INT.` at two columns; "the water will stop it growing"
 * would be this file forecasting, and is banned. The name survives both signs:
 * holds less and holds more are each true read aloud.
 */
function waterSentence(run, sys) {
  const now = run.potIntNowKt;
  if (now == null || !now || run.currentWindKt == null) return '';
  const pot = run.potIntKt;
  if (!Array.isArray(pot)) return '';

  /* The far end of the DRAWN track that also publishes a ceiling — never past
   * the ribbon, for the same reason the bottom line is capped there. */
  let hr = null, end = null;
  for (let c = run.hours.length - 1; c >= 0; c--) {
    if (run.drawable[c] && pot[c] != null) { hr = run.hours[c]; end = pot[c]; break; }
  }
  if (hr == null || !end) return '';

  const bandNow = roomBand(run.currentWindKt / now);
  const bandEnd = roomBand(run.currentWindKt / end);
  if (bandNow === bandEnd) return '';

  const ceil = formatWind(end, sys);
  return end < now
    ? `The water ahead of it holds less — down to ${ceil} by ${at(run, hr)}.`
    : `The water ahead of it holds more — up to ${ceil} by ${at(run, hr)}.`;
}
...
function bottomLine(run, verdictHr, sys) {
  /* ==> THE LAST HOUR WITH A WIND, NOT THE LAST HOUR WITH A POSITION. <== See
   * `lastForecastHr` in lib/env-series.js for the bug this replaced: since
   * §47.2 made the position alone decide drawability, the two hours are
   * different in one run in six and reading the wind at the wrong one printed
   * an em dash into the sentence. The hour is named in the sentence either
   * way, so a run whose intensity forecast stops early simply quotes an
   * earlier time — which is the true statement. */
  const hr = lastForecastHr(run);
  if (hr == null) return '';
  const c = run.hours.indexOf(hr);
...
export function envHealth(result, { system = null, stormName = null } = {}) {
  ...
  /* Directly after the room sentence, because it is the same fact one
   * forecast later and it corrects that sentence's tense. It reads as a "but"
   * without needing the word, and it is absent on the 88% of runs where the
   * water does not change the story. */
  const water = waterSentence(run, system);
