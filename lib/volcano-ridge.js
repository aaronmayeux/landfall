...
export function buildRidge(cluster) {
  ...
  for (let j = 0; j < ny; j++) {
    ...
    for (let i = 0; i < nx; i++) {
      ...
      for (const m of local) {
        ...
        const frac = d / m.radius;
        const hv = m.height * heightFrac(m.table, frac);
        if (hv <= 0) continue;
        contrib.push(hv);
        if (m.erupting) contribLive.push(hv);
        if (m.height > lp) lp = m.height;
        if (1 - frac > deepest) deepest = 1 - frac;
      }
      const idx = j * nx + i;
  ...
  for (let j = 0; j < ny; j++) {
    for (let i = 0; i < nx; i++) {
      ...
      colors[idx * 4 + 3] =
        opaque * smoothstep01(h / foot) * smoothstep01(inset[idx] / R.edgeFade);
    }
  }
