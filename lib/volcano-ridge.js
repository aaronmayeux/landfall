...
const WATER_RGB = rgbOf(WATER.color);
...
export function buildRidge(cluster) {
  ...
  /** The same, restricted to SUBMARINE members. This is the sea's extent: the
   *  water is clipped to the seamounts' own footprints and fades out over the
   *  rim, so it reads as water rather than as a disc (§42.1.4c). */
  const wet = anySubmarine ? new Float64Array(count) : null;
  ...
  for (let j = 0; j < ny; j++) {
    ...
    for (let i = 0; i < nx; i++) {
      ...
      for (const m of local) {
        ...
        const rise = m.height * heightFrac(m.table, q);
        if (rise <= 0) continue;
        ...
        if (1 - q > deepest) deepest = 1 - q;
        if (m.submarine && 1 - q > deepestWet) deepestWet = 1 - q;
      }
      const idx = j * nx + i;
  ...
  return {
    ...
    colors,
    indices,
    water: wet ? buildWater(nx, ny, minE, minN, dx, dy, wet) : null,
    peak,
...
/**
 * The sea over a cluster's seamounts — a flat translucent sheet at z = 0,
 * clipped to the submarine footprints and faded out at their rims.
 *
 * ==> IT IS A SEPARATE MESH, NOT PART OF THE HEIGHTFIELD, BECAUSE TWO SURFACES
 * SIT OVER THE SAME GROUND. <== A heightfield is single-valued by definition;
 * water above a mountain is the one thing it cannot express. The renderer draws
 * this after the mountains with depth WRITING off, so the sea covers the peak
 * below it without occluding anything else.
 *
 * ==> AND IT DOES NOT END, IT FADES. <== Clipped hard to a footprint a water
 * plane reads as a puddle with a rim, which is worse than no water at all. The
 * alpha ramp is the same idea and the same magnitude as `ridge.edgeFade` on the
 * mountains themselves. Drawing the sea across the whole viewport instead was
 * considered and rejected (§42.1.4c): it would lie on top of MapLibre's own
 * water polygons, which is two renderers drawing one ocean.
 *
 * No lighting, no normals — a flat plane lit by a fixed overhead-ish sun is one
 * constant shade, so computing it per vertex would be 3,000 evaluations of the
 * same answer.
 *
 * @returns {object|null} `{positions, colors, indices}`
 */
function buildWater(nx, ny, minE, minN, dx, dy, wet) {
  const count = nx * ny;
  const positions = new Float32Array(count * 3);
  const colors = new Float32Array(count * 4);
  const indices = [];
  ...
  for (let j = 0; j < ny; j++) {
    for (let i = 0; i < nx; i++) {
      ...
      colors[idx * 4 + 3] = WATER.opacity * smoothstep01(wet[idx] / WATER.edgeFade);
    }
  }
  ...
  return indices.length ? { positions, colors, indices } : null;
}
