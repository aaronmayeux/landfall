 ...
 * reaching `water.spread` times each seamount's base radius and faded out at
 * the rim.
 *
 * ==> IT BUILDS ITS OWN GRID, AND THAT IS THE WHOLE CHANGE. <== It used to
 * borrow the mountain heightfield's grid outright — same bounds, same spacing —
 * which meant the sea could not be one metre wider than the mountain under it,
 * and a sea exactly the size of the thing it covers reads as a LID. Three times
 * the reach on the mountain's grid would also have been nine times the vertices
 * for a surface with no relief in it. Its own coarse grid gets both: wider, and
 * far cheaper than the mesh it sits over.
 *
 ...
 * (§42.1.4c): it would lie on top of MapLibre's own water polygons, which is
 * two renderers drawing one ocean.
 *
 ...
 */
function buildWater(subs, lon0, lat0) {
  ...
  /* Resolution is set by the smallest seamount's BASE radius, so a small
   * seamount sharing a sheet with a large one still gets enough samples across
   * its own fade. `ridge.maxCells` is reused as the ceiling deliberately: it is
   * this layer's one budget for "how many grid nodes may a cluster cost", and a
   * second number would let the two drift into a cluster that is cheap as
   * terrain and ruinous as water. */
