 ...
 * a cluster's seamounts. It is a flat plane, not a heightfield sampled from a
 * silhouette, and it left this file when the line ceiling bound. `buildRidge`
 * still calls it so a caller gets one object back.
 *
...
/**
 * The cluster's surface height at ONE arbitrary point, in metres, signed, with
 * zero at sea level. Same answer as the grid, asked one node at a time.
 *
 * ==> THIS IS THE OPTIMISED LOOP'S TWIN AND THE TWO MUST NOT DRIFT. <==
 * `buildRidge` cannot call this — its inner loop already has the offsets, the
 * distance and the bearing in hand and gets its cosine and sine for free, so
 * routing it through here would add a `hypot`, two divides and a function call
 * per member per node across a hundred thousand nodes. So the arithmetic is
 * written twice on purpose, and `tools/test-volcano-flow.mjs` asserts the two
 * agree to a millimetre over a sampled grid. **If you change the height
 * evaluation in either place, change it in both and run that test.**
 *
 * The flow tracer needs this because a flow does not walk on grid nodes — it
 * lands wherever its momentum puts it, and quantising that to the mesh would
 * make lava move in staircases at exactly the scale the eye is watching.
 *
 * @param {object[]} local members as built inside `buildRidge`
 * @param {number} k the cluster's smooth-max blend width, in metres
 * @param {boolean} anySubmarine whether the smooth base plane applies
 * @param {number} e metres east of the cluster centre
 * @param {number} n metres north of the cluster centre
 * @returns {number} surface height in metres, signed
 */
export function surfaceHeightAt(local, k, anySubmarine, e, n) {
  const contrib = [];
  let wSum = 0;
  let wBase = 0;
  ...
  if (contrib.length === 0) return wSum > 0 ? wBase / wSum : 0;
  return smaxAll(contrib, k);
}
...
export function ridgeMember(mark) {
  ...
  return {
    ...
    /** ==> IS LAVA COMING OUT, WHICH IS NOT THE SAME QUESTION AS `erupting`.
     *  <== Most erupting volcanoes emit ash or steam and no lava at all, and
     *  a glowing flow drawn down an ash eruption is the same class of lie as
     *  §42.1.9's smoke over Kilauea. Only the weekly report knows this, via
     *  `functions/api/volcano/_emissions.js`. */
    lava: !!mark.lava,
    /** ==> IS ASH IN THE AIR, WHICH IS A THIRD QUESTION AGAIN. <==
     *  `{m, stated, clamped}` in metres above the summit, or null — decided
     *  once in `lib/volcano-plume.js` from the advisory, never re-derived by a
     *  renderer. Carried through untouched: this file has no opinion about
     *  what is coming out of a volcano, only about the shape of the rock. */
    plume: mark.plume || null,
 ...
 */
export function buildRidge(cluster, opts = {}) {
  ...
  const local = cluster.map((m) => ({
    ...
    erupting: m.erupting,
    lava: !!m.lava,
    /* Carried through for the ash column, which anchors at this member's own
     * summit rather than at the cluster's peak — see `lib/volcano-plume.js`. */
    plume: m.plume || null,
    submarine: !!m.submarine,
    /* Carried through for the lava launch ring, which offsets itself per
     * volcano for the same reason the shape does — see `lib/volcano-flow.js`. */
    seed: m.seed,
   ...
   *
   * ==> `refine` RAISES BOTH THE RESOLUTION AND THE CAP, AND RAISING ONE
   * ALONE IS A SILENT NO-OP. <== The cap is expressed in CELLS, so tripling
   * `cellsPerRadius` against a fixed `maxCells` just trips the coarsening line
   * below and hands back the grid it started with — the exact trap `NOW.md`
   * named in the gully measurement. They move together or not at all. */
  const refine = Math.max(1, Number(opts.refine) || 1);
  let cell = smallest / (R.cellsPerRadius * refine);
  const width = maxE - minE;
  const depth = maxN - minN;
  const capCells = R.maxCells * refine * refine;
  const cells = (width / cell + 1) * (depth / cell + 1);
  if (cells > capCells) cell *= Math.sqrt(cells / capCells);
  ...
  return {
    ...
    colors,
    indices,
    water: buildWater(local.filter((m) => m.submarine), lon0, lat0),
    peak,
    extent: Math.max(width, depth),
    members: cluster.length,
    /** What this cluster was actually sampled at, 1 for normal. Reported
     *  rather than inferred from the node count: a big cluster at 1x and a
     *  small one at 3x can hold the same number of nodes, so counting them is
     *  not a test of whether refinement happened. */
    refine,
    /* ==> THE SURFACE ITSELF, SO LAVA RUNS DOWN THE MOUNTAIN THAT IS DRAWN
     * RATHER THAN A SECOND ONE BUILT TO MATCH. <== `local` carries each
     * member's varied shape already resolved, and `k` and `anySubmarine` are
     * the two other inputs `surfaceHeightAt` needs. Handing back the inputs
     * costs nothing and means the tracer cannot be looking at a different
     * mountain from the one on screen — which is the only way this feature
     * could be subtly, permanently wrong. */
    surface: { local, k, anySubmarine },
  };
}
 ...
 */
export function buildRidges(marks, opts = {}) {
  const members = (marks || []).map(ridgeMember);
  /* ==> REFINEMENT IS PER CLUSTER AND IT IS RATIONED. <== The whole reason
   * this is affordable is that it applies to the few clusters with lava in
   * them, not the drawn set — the arithmetic is on `VOLCANO.map3d.lava`. The
   * budget is spent on the clusters that ask, in the order they come, and a
   * cluster that misses out draws at normal resolution rather than not at
   * all: a coarse mountain with lava on it is worse-looking, not broken. */
  const wantsRefine = typeof opts.refineWhen === 'function' ? opts.refineWhen : null;
  const refine = Math.max(1, Number(opts.refine) || 1);
  let budget = Number.isFinite(opts.maxRefined) ? opts.maxRefined : Infinity;
  return clusterMembers(members)
    .map((cluster) => {
      let r = 1;
      if (wantsRefine && refine > 1 && budget > 0 && wantsRefine(cluster)) {
        r = refine;
        budget--;
      }
      return buildRidge(cluster, { refine: r });
    })
    .filter(Boolean);
}
