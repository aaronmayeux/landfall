  ...
  try {
    ...
    const rebuilt = features.map((f) => {
      ...
      if (track && single) {
        ...
        /* ==> THE REBUILD DECLINING IS NOT A REASON TO LOSE THE MEASUREMENT.
         * <== §47.5. `sweepConeDetail` refuses when it cannot produce a smooth
         * outline that is safe to DRAW — on a tight bend the swept edge loops
         * and MapLibre fills the loop as a hole. That bar is right for pixels
         * and has nothing to do with whether the cone can be measured. For a
         * third of Ida's advisories it took the environment ribbon down with
         * it, which is what Aaron saw come and go on Lala (2026-08-18).
         *
         * So the cone below falls back to the published outline exactly as it
         * always has — the drawing is untouched — and the stations are
         * measured off that same published polygon instead. Every point is a
         * ray hit on the shape being drawn, so the ribbon fits it by
         * construction. `measureConeRibs` marks the stations it cannot vouch
         * for and lib/cone-ribbon.js skips those slices. */
        if (!ribs) {
          const measured = measureConeRibs(track, rings);
          if (measured) {
            ribs = measured.ribs;
