...
import { stitchDatelineSplit } from './seam-stitch.js';
...
export function smoothCone(bundle, label = 'storm') {
  ...
  try {
    ...
    const rebuilt = features.map((f) => {
      /* ==> PUT THE SHAPE BACK TOGETHER BEFORE ANYTHING ELSE LOOKS AT IT.
       * <== §7.9. NHC serves geometry wrapped into (−180, 180], so a cone
       * crossing the antimeridian arrives CUT along the meridian as a
       * two-part MultiPolygon, each part carrying a straight artificial edge
       * down the seam. Every reader below then went wrong in its own way:
       * `curveGeometry` rounded the artificial edges into noses that bulged
       * across each other, the outline layer stroked them as real cone edges,
       * and the rebuild turned the shape away for having no single spine.
       * Stitched here, once, all three are simply not asked the question.
       * Anything that is not a clean two-part cut comes back untouched. */
      const geometry = stitchDatelineSplit(f.geometry);
       ...
       * IT IS WHAT THE DATE LINE DOES TO EVERY CONE THAT CROSSES IT. <== The
       * stitch above now hands most of those back as one polygon, so this
       * gate rarely decides anything on a seam cone any more. It still has to
       * be the sweep's own gate rather than a shared one: a genuinely
       * multi-part cone — one the stitch could not recognise — must still be
       * MEASURABLE even though it can never be swept. Sharing the test meant
       * `ribs: null`, and §47.6's row telling the reader "this cone could not
       * be measured" about a cone that measures fine: 236 stations out of 236
       * on Lala's, when the same rings are handed to `measureConeRibs`
       * directly. */
      const sweepable = geometry?.type === 'Polygon' && rings.length >= 1;
      ...
      const curved = curveGeometry(geometry);
