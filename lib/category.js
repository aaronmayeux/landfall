...
/**
 * ==> "NOTHING TO CLAIM" HAS A SETTLED COLOUR AND IT IS BONE. <==
 *
 * §57.7g, Aaron's call 2026-08-29: a system with no Saffir-Simpson severity to
 * claim gets `stormEnded` — the mark drained of its colour, still fully
 * present. Neither the teal `PREGENESIS_COLOR`, which reads too close to the
 * TD blue, nor the brick `CATEGORY_COLOR.GENERIC`, which reads like a strong
 * storm. Confirmed again on glass 2026-09-10: a pre-genesis fix on the home
 * rail came out brick and read as severe.
 *
 * ==> SO WHY IS THIS NOT JUST `categoryColor` RETURNING BONE? <== Because bone
 * is PALETTE-SCOPED — `#DCE4EC` on a night globe, `#3A4149` in daylight — and
 * `categoryColor` feeds MapLibre features and Three.js materials as well as
 * CSS. A palette value baked into a map feature goes stale the moment the
 * theme flips, and the legal fixes for that are expensive: `map/theme-state.js`
 * rule 1b FORBIDS an expression that reads both global state and feature data
 * (it silently evaluates to black in the worker, and there is a gate for it),
 * so the map's only route is a feature re-push on every theme change — a list
 * `main.js` has deliberately capped at three.
 *
 * ==> THIS FUNCTION IS THEREFORE FOR CSS CONTEXTS ONLY. <== A custom property
 * or an inline `style`, where `var(--storm-ended)` resolves and rethemes for
 * free because `app/theme-switch.js` already writes that variable. Anything
 * handing a colour to a rendering engine keeps calling `categoryColor`.
 *
 * ==> AND IT DELEGATES RATHER THAN RESTATING. <== Every graded answer comes
 * from `categoryColor` itself, so the two cannot drift on any reading that has
 * a colour to earn. `tools/test-category-ink.mjs` asserts that agreement across
 * the whole matrix rather than trusting this sentence.
 *
 * ==> `post-tropical` IS NOT IN SCOPE AND THAT IS DELIBERATE. <== It keeps
 * GENERIC here exactly as it does above. The archive globe does draw it in
 * bone, but there SIZE carries "was this ever a storm" (`STORM_GEO`,
 * §57.7g) — the home dashboard has no size mechanism, so boning it would drop
 * a distinction rather than unify one.
 */
export const UNGRADED_INK = 'var(--storm-ended)';

export function categoryInk(category, nature, code = null) {
  if (UNGRADED_SYSTEM.has(nature)) return UNGRADED_INK;
  if (CATEGORIZABLE.has(nature) && category == null && code !== 'HU') return UNGRADED_INK;
  return categoryColor(category, nature, code);
}
