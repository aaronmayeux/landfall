 ...
 * @param {Array<string>} args.stops     the active palette's cone ramp
 * @param {Array<string>} args.lineStops the active palette's LINE ramp — see
 *        `config/tokens.js` `envRampLine` for why it is not the same list
 * @returns {{status:string, reason:string|null, features:Array,
 *            fromHr:number|null, toHr:number|null}}
 */
export function buildRibbon({ ribs, caps, forecast, run, stops, lineStops }) {
  /* A missing line ramp falls back to the cone's rather than throwing: an
   * older palette is a reason to draw a dimmer line, never a reason to lose
   * the ribbon entirely. */
  const lineRamp = Array.isArray(lineStops) && lineStops.length ? lineStops : stops;
  ...
  for (let a = 0; a < ribs.length - 1; a += STRIDE) {
    ...
    if (spine.length >= 2) {
      features.push({
        ...
        properties: {
          _kind: 'line',
          _color: rampAt(lineRamp, rampT(mid)),
          kt: Math.round(mid),
   ...
   * color — which is already the +6 h value inherited back (§47.5). Nothing
