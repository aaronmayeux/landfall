 ...
 *
 * ==> PIECEWISE AND SYMMETRIC, AND THE MEASURED DOMAIN IS UNCHANGED (§47.4).
 * <== `-15..+15` maps onto the MIDDLE `scaleInnerFraction` of the ramp, with
 * the remainder split evenly between the two extensions. With five evenly
 * spaced stops that puts -15 / 0 / +15 / +40 on stops 1 / 2 / 3 / 4, so every
 * reading resolves to the colour it always had, byte for byte, and §47.4's
 * argument for +/-15 needs no revisiting.
 *
 * ==> THE SYMMETRY IS THE WHOLE REASON IT IS SHAPED THIS WAY (Aaron, glass,
 * 2026-08-22). <== The legend's middle label is `Balanced`, and `Balanced` is
 * 0 kt. While the scale ran -15..+40 that sat a THIRD of the way along the
 * bar, and a key that mis-states the middle of its own scale is worse than no
 * key: every colour on it is a real ramp colour, in the wrong place, silently.
 * Zero has to be the midpoint, so the scale has to be symmetric, so the
 * hostile end has to reach -40 as well.
 *
 * ==> AND THE HOSTILE EXTENSION IS ONE FLAT COLOUR, WHICH IS HONEST RATHER
 * THAN LAZY. <== `scaleLoKt`'s stop is already within three units of the night
 * ocean and IS the daylight sea in the light theme (§47.5) — there is nothing
 * darker or paler to reach for. So the palette repeats it and everything past
 * -15 kt draws exactly what it drew before. A -52 kt hour and a -23 kt hour
 * still draw identically; they always did, and now the bar shows that flat
 * quarter instead of hiding it behind an end-stop.
 *
 * The bright extension is the one that carries new colour, and it earns it:
 * that end used to flatten 2.0% of the season, which sounds small until it
 * lands on a storm that lives up there. Lala ran +18 kt to +34 kt across the
 * back half of her cone and drew it in one flat violet.
 */
export const rampT = (kt) => {
  ...
  /* Each extension gets half of what the measured domain does not occupy, so
   * the two ends are the same width and the midpoint of the domain — 0 kt,
   * since the domain is itself symmetric — is the midpoint of the ramp. */
  const edge = (1 - inner) / 2;
  if (!Number.isFinite(kt)) return 0;
  if (kt < lo) return edge * clamp01((kt + outer) / (lo + outer));
  if (kt <= hi) return edge + ((kt - lo) / (hi - lo)) * inner;
  return edge + inner + clamp01((kt - hi) / (outer - hi)) * edge;
};
