 ...
 * chip is no longer blocked on a list — §57.51 derives one and
 * `data/retired-names.js` is in the repo — but the chip itself is its own pass.
 * The names are here because they are 7 KB of the 15 and re-generating the file
 * later is a monthly-runner round trip; the chip lands when it is wired.
 */
...
export function rowLabel(row, { catLabel }) {
  ...
  return `${row.year} — ${n} storm${n === 1 ? '' : 's'}, ${strongest}${ashore}${under}`;
}
