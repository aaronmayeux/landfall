...
/** `LANDFALL` holds a COUNT of landfalls since §57.7a, not a 0/1 flag. Every
 *  reader that treats it as a boolean still works — 0 is still falsy — but a
 *  reader that counts rows rather than summing them is back to counting
 *  storms. */
...
export function stormRow(facts) {
  ...
  /* ==> A COUNT, NOT A FLAG, AND THAT CHANGED WITH §57.7a. <== It was 0 or 1
   * while the only source was NOAA's `L` marker, because the marker is missing
   * for whole decades and a count built on it would have been confidently
   * wrong rather than merely coarse. Now that `lib/landfall.js` crosses the
   * track against a real coastline, the number of landfalls is something we
   * actually know — and the wall's `Landfalls` sort key was counting STORMS
   * and calling them landfalls, which for 2005 is 16 against a real 29.
   *
   * Costs nothing: almost every value is a single digit either way. */
  const landfall = facts?.landfalls?.length || 0;
...
export function aggregate(shown) {
  ...
  for (const s of shown || []) {
    ...
    /* Summed rather than counted: the column is landfalls per storm now, so a
     * storm that came ashore three times contributes three. A `+= 1` here
     * would silently turn the row back into a count of storms — which is what
     * it used to be, and what §57.7a corrected. */
    if (Number.isFinite(s[LANDFALL])) landfalls += s[LANDFALL];
