...
export function rowsFor(wall, basin, keep = null) {
  ...
  for (const [key, list] of Object.entries(years)) {
    ...
    out.push({
      ...
      strongest,
      landfalls,
      ace: aceMeasured ? Math.round(ace * 10) / 10 : null,
      aceMeasured,
...
export function liveRow({ year, facts, running }) {
  ...
  for (const f of ended) {
    if (Number.isFinite(f?.peakCategory) && f.peakCategory > strongest) strongest = f.peakCategory;
    if (Number.isFinite(f?.ace)) { ace += f.ace; aceMeasured = true; }
  }

  return {
    year,
    shown: ended.map(stormRow),
