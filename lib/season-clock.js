...
/**
 * How far round the head's mark has turned, in degrees, at this moment.
 *
 * ==> IT IS HERE RATHER THAN IN THE LAYER BECAUSE ONLY THIS FILE KNOWS WHAT
 * TIME IT IS. <== `map/layers/season-head.js` is handed a state, not a moment —
 * that is the whole shape of the cut (§57.67e) — so an angle computed there
 * would need the clock passing the moment down a second path, which is the
 * "two calls to stay consistent" trap `setSeasonTracks` exists to avoid. An
 * angle is arithmetic over time and a sign, the same class of answer as
 * `headingDeg` next to it, and it is still not a colour.
 *
 * ==> IT IS SPENT IN STORM TIME, WHICH IS WHAT MAKES SCRUBBING TURN IT. <== The
 * period is `SEASONS.clockGlyphSpinSeconds` REAL seconds converted through
 * `toStormMs`, so at the shipped pace a revolution is six seconds of watching —
 * and dragging the scrubber to a moment gives the same angle playing to that
 * moment would. A rotation paced on `performance.now()` instead would spin a
 * paused globe and would answer differently every time the same moment was
 * reached, which is a thing a reader can catch.
 *
 * ==> AND IT IS MEASURED FROM THE STORM'S OWN FIRST FIX. <== Every storm
 * therefore starts its head at zero rather than at whatever angle 1851 or 2021
 * happens to land on, and the arithmetic stays in a range a float has plenty of
 * room in rather than dividing an epoch by six seconds.
 *
 * Positive is clockwise, which is what MapLibre's `icon-rotate` means, so the
 * sign is the NEGATIVE of `spinSign` — a northern cyclone turns
 * counter-clockwise. The southern mark is the mirrored artwork AND the opposite
 * sign, so it turns the way the arms sweep rather than against them.
 *
 * @param {number} t      the moment, epoch ms
 * @param {number} from   the storm's first fix, epoch ms
 * @param {1|-1} spin
 * @returns {number} degrees, wrapped into 0–360
 */
function spinDegAt(t, from, spin) {
  const periodMs = toStormMs(SEASONS.clockGlyphSpinSeconds * 1000);
  if (!(periodMs > 0) || !Number.isFinite(t) || !Number.isFinite(from)) return 0;
  const turns = (t - from) / periodMs;
  const deg = -(spin || 1) * turns * 360;
  /* `%` keeps the sign of the dividend in JavaScript, so a northern storm would
   * answer −0 to −360 without the second term. MapLibre accepts a negative
   * rotation happily; a test comparing two angles that mean the same thing does
   * not, and neither does anybody reading a debug dump. */
 ...
 *   headingDeg: number|null, spin: number|null, spinDeg: number|null,
 * }}
 */
export function stormStateAt(storm, t) {
  ...
  if (t >= last.time) {
    ...
    return {
      ...
      /* Answered for an ended storm as well, even though nothing draws a head
       * on one (rule 2 above). The alternative is a null that a later caller
       * has to remember to handle, for a field whose honest value is simply
       * "this is the angle at that moment". */
      spinDeg: spinDegAt(t, pts[0].time, spinSign(last.lat)),
    };
  }
  ...
  return {
    ...
    spin: spinSign(a.lat),
    spinDeg: spinDegAt(t, pts[0].time, spinSign(a.lat)),
  };
}
