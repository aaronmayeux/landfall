 ...
 * @param {object} opts  WIND_SWEEP
 * @returns {Array<Array<Array>>} one entry per shape, each a list of closed
 *   rings with the shell first — GeoJSON polygon coordinates, in the plane
 */
function mergeBands(rings, opts) {
  if (rings.length < 2 || opts.mergePieces === false) return rings.map((r) => [r]);
  ...
  for (const g of groups) {
    ...
    if (united) {
      /* Shell first, then holes — GeoJSON's own order. A hole is a patch the
       * storm circled without its wind field ever reaching the middle. */
      out.push(united.map(closeRing));
      continue;
    }

    /* ==> A GROUP THAT WILL NOT MERGE WHOLE STILL MERGES IN PAIRS. <== The
     * union refuses a shape it cannot model, and refusing the whole group over
     * one awkward pair used to leave four separate polygons where two would
     * do. Merging greedily two at a time can only reduce the count and can
     * never produce a shape the union would not have accepted on its own,
     * because every step is a full `unionRings` call with all of its guards.
     * Measured on Hanna 2008's 50 kt band: four pieces, whole-group merge
     * refused, pairwise gets it to two. */
    let pieces = g.slice();
    let merged = true;
    while (merged && pieces.length > 1) {
      merged = false;
      outer:
      for (let i = 0; i < pieces.length; i++) {
        for (let j = i + 1; j < pieces.length; j++) {
          if (!ringsOverlap(pieces[i], pieces[j])) continue;
          const pair = unionRings([pieces[i], pieces[j]], { stepBudget: opts.unionStepBudget });
          /* Only a single-ring answer can go back into the pool: a shell with
           * holes is not something the next pass could union again, and
           * pretending otherwise would drop the holes. */
          if (!pair || pair.length !== 1) continue;
          pieces = pieces.filter((_, k) => k !== i && k !== j).concat([pair[0]]);
          merged = true;
          break outer;
        }
      }
    ...
    if (pieces.length < g.length) {
      for (const r of pieces) out.push([closeRing(r)]);
      continue;
    }

    /* ==> AND IF EVEN THAT GETS NOWHERE, SAY SO. <== The band is still drawn.
     * Silence on a known-imperfect shape is the §5 failure. */
    console.warn(
  ...
  for (const t of thresholds) {
    ...
    for (const shape of rings) {
      features.push({
        ...
        geometry: { type: 'Polygon', coordinates: shape.map((r) => r.map(toLonLat)) },
      });
    }
  }
  return features;
