...
import {
  MODEL_COLOR, MODEL_COLOR_LIGHT,
  MODEL_FALLBACK_RAMP, MODEL_FALLBACK_RAMP_LIGHT,
 ...
 * model that carries a storm across 180° publishes `1795W` and then `1790E` —
 * two neighbouring positions half a degree apart whose numbers are 359 degrees
 * apart. Handed to MapLibre unchanged that is a line instructed to travel the
 * long way round the planet, and it draws exactly that: a guidance line
 * stretching across the whole map through basins the storm will never see.
 *
 * SEEN ON GLASS 2026-07-28 (Aaron, Fausto west of Hawaii). It appeared with
 * the UCAR ensemble means because they were the first models in the shortlist
 * long-range enough to carry an East Pacific storm past 180 — the flaw was
 * always here, the storm simply had never reached the seam before.
 *
 * THE FIX IS UNWRAPPING, NOT SPLITTING. Longitudes are made continuous and
 * allowed to run past ±180, which is what MapLibre wants for a line across the
 * seam — the same contract lib/trackline.js already keeps for the past and
 * forecast tracks. Cutting the line into two features instead would leave a
 * visible gap at the seam and give the layer two answers to one question.
 *
 * THE RUN IS ANCHORED TO THE STORM, not to its own first point. Everything
 * downstream — the back-half clip's half-plane test, the anchor vertex, the
 * eye reading five lines out of one dot — compares model positions against the
 * current position, and those comparisons are only meaningful if both sit in
 * the same 360° window.
 * ------------------------------------------------------------------------- */

const round2 = (v) => Math.round(v * 100) / 100;

/** Longitude difference wrapped into (−180, 180], so 179.5 and −179.5 read as
 *  one degree apart rather than 359. */
function dLon(a, b) {
  let d = a - b;
  while (d > 180) d -= 360;
  while (d <= -180) d += 360;
...
/**
 * A run of [lon, lat] made continuous, in the same 360° window as `anchorLon`.
 *
 * Every step is the WRAPPED delta added to the running value, so a crossing of
 * the seam continues past ±180 instead of jumping back. With no anchor the run
 * keeps its own first longitude and nothing translates.
 */
export function unwrapRun(pts, anchorLon = null) {
  if (!Array.isArray(pts) || !pts.length) return pts;
  const first = Number.isFinite(anchorLon)
    ? anchorLon + dLon(pts[0][0], anchorLon)
    : pts[0][0];
  const out = [[round2(first), pts[0][1]]];
  for (let i = 1; i < pts.length; i++) {
    const prev = out[i - 1][0];
    out.push([round2(prev + dLon(pts[i][0], prev)), pts[i][1]]);
  }
  return out;
}
...
export function clipBehind(pts, cur, headingDeg) {
  ...
  if (heading) {
    let i = 0;
    while (i < pts.length) {
      const east = dLon(pts[i][0], cur.lon) * cosLat;
  ...
  /* The anchor is expressed in the KEPT RUN's longitude window, not in the
   * feed's. An unwrapped run past the dateline sits at −181 while the storm's
   * own position still reads +179; prepending the raw value would put the
   * whole-globe line back in at the one vertex this function adds. */
  const anchor = [round2(kept[0][0] + dLon(cur.lon, kept[0][0])), round2(cur.lat)];
...
export function parseAdeck(text, { cur = null, headingDeg = null } = {}) {
  ...
  for (const model of MODEL_TRACKS.techs) {
    ...
    /* CONTINUOUS FIRST, THEN CLIPPED. See the dateline note above: the clip
     * and the anchor both compare model positions with the storm's, and the
     * map needs one unbroken run of longitudes to draw across 180°. */
    const run = unwrapRun(ordered, cur && Number.isFinite(cur.lon) ? cur.lon : null);
    const points = clipBehind(run, cur, headingDeg).slice(0, MODEL_TRACKS.maxPoints);
 ...
 *
 * The color is resolved per feature and baked into the properties, so the
...
/**
 * The identity color for one model, in the LIVE theme.
 *
 * ==> ASKED AT PAINT TIME, NEVER CACHED. <==
 * A `const` holding the table at module scope freezes whatever the theme was
 * when this file was first imported — the exact mistake config/theme.js warns
 * about in its own header, and it shows up as one stubbornly dark layer in an
 * otherwise light app.
 *
 * WHAT IS FIXED HERE IS THE HUE, NOT THE HEX. §6 fixes severity colors
 * outright because a Cat 3 dot must read the same everywhere; model identity is
 * a LEGEND, and the legend is on screen — the picker swatch comes from this
 * same function, so the line and its label can never disagree. The dark set is
 * measurably invisible on a daylight ocean (see the block in config/tokens.js),
 * and a line cannot carry the halo the severity marks use.
 *
 * Anything outside the shortlist takes a color from the fallback ramp by
 * position, so a model added to the manifest without a hex still draws,
 * distinctly, and never silently shares a named model's color.
 *
 * ==> THE BAKED `_color` IS RE-BAKED ON A THEME CHANGE, AND THAT IS LOAD-BEARING.
 * `tracksToFeatures` writes the hex into the feature, so nothing re-reads this
 * per frame — and `map/layers/model-tracks.js` paints `['get', '_color']`,
 * which means NO paint property holds the color and nothing MapLibre can be
 * handed will change it.
 *
 * THIS USED TO HAPPEN BY ACCIDENT AND NOW HAPPENS ON PURPOSE. A theme switch
 * rebuilt the whole basemap style, and main.js's `installOnStyle` re-ran the
 * geometry pushes on the `style.load` that followed — freshly-colored
 * features fell out of a teardown that existed for other reasons. Theming is
 * `map.setGlobalState` now (see `map/theme-state.js`) and never replaces the
 * style, so the switch calls `onRepushGuidance` explicitly instead. It is one
 * of exactly two things on the map a global-state color cannot reach.
 *
 * Remove that repush and the guidance lines keep the old theme's colors until
 * the next poll.
 */
export function modelColor(tech) {
  const light = isLight();
  const named = (light ? MODEL_COLOR_LIGHT : MODEL_COLOR)[tech];
  if (named) return named;
  const i = MODEL_TRACKS.techs.findIndex((m) => m.tech === tech);
  const ramp = light ? MODEL_FALLBACK_RAMP_LIGHT : MODEL_FALLBACK_RAMP;
