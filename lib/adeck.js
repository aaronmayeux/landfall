...
/**
 * The identity colour for one model, in the LIVE theme.
 *
 ...
 * otherwise light app.
 *
 * WHAT IS FIXED HERE IS THE HUE, NOT THE HEX. §6 fixes severity colours
 ...
 *
 * Anything outside the shortlist takes a colour from the fallback ramp by
 * position, so a model added to the manifest without a hex still draws,
 * distinctly, and never silently shares a named model's colour.
 *
 ...
 * which means NO paint property holds the colour and nothing MapLibre can be
 ...
 * geometry pushes on the `style.load` that followed — freshly-coloured
 ...
 * of exactly two things on the map a global-state colour cannot reach.
 *
 * Remove that repush and the guidance lines keep the old theme's colours until
 * the next poll.
 */
