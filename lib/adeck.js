...
/** Basins TCGP files a-decks for. JTWC warns on exactly these three, but the
 *  check is explicit rather than assumed — an unrecognised basin must become
 *  an honest "not covered", never a 404 dressed up as a dead source. */
const TCGP_BASINS = new Set(['wp', 'io', 'sh']);

/**
 * JTWC product id → TCGP a-deck storm id.
 *
 *   'wp1126'  →  'wp112026'
 *
 * TWO ID SHAPES FOR THE SAME STORM, and the difference is the year. JTWC's
 * product filename carries a TWO-digit year (`wp` + `11` + `26`); TCGP's
 * filename carries FOUR (`wp` + `11` + `2026`). Written out here rather than
 * inline at the call site because it is the one place two upstreams' naming
 * conventions meet, and a silent off-by-two-characters here fetches a
 * different storm's guidance — or, worse, a real deck for the wrong storm.
 *
 * THE CENTURY IS HARDCODED, DELIBERATELY. `20` + `yy` is wrong in 2100 and
 * correct until then; deriving it from the current date would be wrong every
 * New Year's Eve for a storm that formed in December, which is a bug that
 * actually happens.
 *
 * @param {string} product e.g. `'wp1126'`
 * @returns {string|null} the TCGP storm id, or null if the shape is unknown.
 */
export function tcgpIdFromJtwcProduct(product) {
  const m = /^([a-z]{2})(\d{2})(\d{2})$/i.exec(String(product ?? '').trim());
  if (!m) return null;
  const basin = m[1].toLowerCase();
  if (!TCGP_BASINS.has(basin)) return null;
  return `${basin}${m[2]}20${m[3]}`;
}
