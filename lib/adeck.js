 ...
 * per frame. A theme switch rebuilds the basemap style, and main.js's
 * `installOnStyle` re-runs `loadGeometry` and `repushAmbient` on every
 * `style.load` — which is what pushes freshly-coloured features. Remove that
 * repush and the guidance lines keep the old theme's colours until the next
 * poll.
 */
