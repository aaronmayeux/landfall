...
/**
 * ==> A SPELLED-OUT NUMBER IS NOT A NAME, AND MEASURING SAID SO. <== 71 storms
 * across the two HURDAT2 basins are "named" TEN, NINETEEN or TWENTY-TWO. They
 * are storms that never earned a name, wearing their storm number in words —
 * which is precisely what `UNNAMED` means one line up.
 *
 * Without this the roster shows `TEN` and `Storm 21` side by side for the same
 * kind of storm in the same season: 2005 has both, because NOAA wrote the
 * tenth system as TEN and the twenty-first as UNNAMED. One of them would look
 * like a name a person chose. §57.14 gives all of them one display form.
 *
 * ==> AND IT IS WHY THE GHOST ROSTER CAN BE TRUSTED. <== Ghosts are the names
 * a season did NOT spend (`lib/season-names.js`). A storm called TEN counted as
 * a used name would be checked against a list it can never be on, and reported
 * as off-list — the signal that means "the roster in this repo is wrong".
 * Real noise in the one place built to detect a real fault.
 *
 * 22 distinct forms are in the files, including BOTH `TWENTY-ONE` and
 * `TWENTYONE`, so this is generated from parts rather than listed: units and
 * teens, then TWENTY or THIRTY alone or joined to a unit with or without the
 * hyphen. The optional `-E` / `-C` tail is the ATCF operational form (`ONE-E`
 * for an East Pacific depression), which HURDAT2 does not use but a b-deck
 * does — and `parseBdeck` shares this function.
 *
 * No storm name in either basin's roster is a number word, so nothing real is
 * at risk of being swallowed. `tools/test-hurdat.mjs` holds this against every
 * name in both whole files and asserts the count.
 */
...
function isPlaceholderName(name) {
  ...
  if (SPELLED_NUMBER.test(n)) return true;
