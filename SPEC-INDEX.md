# SPEC-INDEX.md — where every section lives

**Generated by `tools/spec-index.mjs`. Do not hand-edit — your edit will be
overwritten. Change a heading in the spec and regenerate.**

Section numbers are permanent addresses. A section can move between files;
it is never renumbered, because code comments reference it. This table turns
`SPEC §17.7` into a file and a line range without reading 61 KB to find it.

To read one section, the Lines column is ready to paste:

```
sed -n "442,713p" SPEC-OPS.md
```

Covering 9 files, 229 numbered headings.

An arrow marks a signpost in `SPEC.md` rather than the content itself. The
real home is the other row carrying the same section number.

## Sections

| § | Title | File | Lines |
|---|---|---|---|
| `1` | What this is | `SPEC.md` | 179-210 |
| `2` | Stack (settled — don't re-litigate without new info) | `SPEC.md` | 211-368 |
| `3` | Domain, accounts, and live infrastructure | `SPEC.md` | 369-390 |
| `4` | Data architecture | `SPEC-DATA.md` | 21-22 |
| `4` | Data architecture | -> `SPEC-DATA.md` | SPEC.md:391 |
| `4.1` | &nbsp;&nbsp;Companion documents | `SPEC-DATA.md` | 23-43 |
| `4.2` | &nbsp;&nbsp;CORS ground truth | `SPEC-DATA.md` | 44-64 |
| `4.3` | &nbsp;&nbsp;The relay — keep it dumb | `SPEC-DATA.md` | 65-209 |
| `4.4` | &nbsp;&nbsp;Sources and split | `SPEC-DATA.md` | 210-245 |
| `4.5` | &nbsp;&nbsp;NHC MapServer | `SPEC-DATA.md` | 246-488 |
| `4.6` | &nbsp;&nbsp;GDACS geometry | `SPEC-DATA.md` | 489-621 |
| `4.7` | &nbsp;&nbsp;The wind field — four numbers, three tiers | `SPEC-DATA.md` | 622-676 |
| `4.8` | &nbsp;&nbsp;Surge | `SPEC-DATA.md` | 677-772 |
| `4.9` | &nbsp;&nbsp;Imagery | `SPEC-DATA.md` | 773-1135 |
| `4.10` | &nbsp;&nbsp;The normalized storm object | `SPEC-DATA.md` | 1136-1216 |
| `4.11` | &nbsp;&nbsp;Advisory identity | `SPEC-DATA.md` | 1217-1258 |
| `4.12` | &nbsp;&nbsp;Polling | `SPEC-DATA.md` | 1259-1275 |
| `4.13` | &nbsp;&nbsp;Cache TTLs | `SPEC-DATA.md` | 1276-1362 |
| `4.14` | &nbsp;&nbsp;Recovery from failure | `SPEC-DATA.md` | 1363-1377 |
| `4.15` | &nbsp;&nbsp;The town list | `SPEC-DATA.md` | 1378-1405 |
| `5` | Failure philosophy (non-negotiable) | `SPEC.md` | 396-397 |
| `6` | Fixed color contracts (not themeable — identical in light and dark) | `SPEC.md` | 1045-1125 |
| `6.0` | &nbsp;&nbsp;The glow, and the one thing that cannot be shared | `SPEC.md` | 1126-1156 |
| `6.1` | &nbsp;&nbsp;NWS watch/warning products are the second fixed contract | `SPEC.md` | 1157-1179 |
| `7` | Layer model | `SPEC-MAP.md` | 19-46 |
| `7` | Layer model | -> `SPEC-MAP.md` | SPEC.md:1180 |
| `7.1` | &nbsp;&nbsp;The layers panel | `SPEC-MAP.md` | 47-116 |
| `7.2` | &nbsp;&nbsp;Full layer inventory | `SPEC-MAP.md` | 117-164 |
| `7.3` | &nbsp;&nbsp;Tropics & equator | `SPEC-MAP.md` | 165-190 |
| `7.4` | &nbsp;&nbsp;The track line — one continuous curved path | `SPEC-MAP.md` | 191-346 |
| `7.5` | &nbsp;&nbsp;Forecast point dots, and the ring that says which way | `SPEC-MAP.md` | 347-404 |
| `7.6` | &nbsp;&nbsp;Model spaghetti tracks | `SPEC-MAP.md` | 593-777 |
| `7.7` | &nbsp;&nbsp;Watch/warning coastal paint — wide-band coast select | `SPEC-MAP.md` | 778-931 |
| `7.8` | &nbsp;&nbsp;Population heat — where people are | `SPEC-MAP.md` | 932-966 |
| `7.9` | &nbsp;&nbsp;The cone of uncertainty — measured, then redrawn along the track | `SPEC-MAP.md` | 999-1208 |
| `7.10` | &nbsp;&nbsp;When there is no coast to paint — the dashed chord and its breakpoints | `SPEC-MAP.md` | 1209-1320 |
| `8` | Home | `SPEC-UI.md` | 22-23 |
| `8` | Home | -> `SPEC-UI.md` | SPEC.md:1185 |
| `9` | Design | `SPEC-MAP.md` | 1604-1605 |
| `9` | Design | -> `SPEC-MAP.md` | SPEC.md:1190 |
| `9.1` | &nbsp;&nbsp;The visual contract | `SPEC-MAP.md` | 1606-1643 |
| `9.2` | &nbsp;&nbsp;Light mode | `SPEC-MAP.md` | 1644-1781 |
| `9.3` | &nbsp;&nbsp;Theming the map without rebuilding it | `SPEC-MAP.md` | 1782-1849 |
| `9.3` | &nbsp;&nbsp;The crossfade | `SPEC-MAP.md` | 1850-1932 |
| `9.4` | &nbsp;&nbsp;The node cage — an information surface, not decoration | `SPEC-MAP.md` | 1933-2117 |
| `9.5` | &nbsp;&nbsp;Storm-lit triangle fill | `SPEC-MAP.md` | 2118-2144 |
| `9.6` | &nbsp;&nbsp;Land, coast and atmosphere | `SPEC-MAP.md` | 2145-2176 |
| `9.7` | &nbsp;&nbsp;Idle rotation | `SPEC-MAP.md` | 2177-2250 |
| `9.8` | &nbsp;&nbsp;Opening sequence | `SPEC-MAP.md` | 2251-2265 |
| `9.9` | &nbsp;&nbsp;Zoom ladder | `SPEC-MAP.md` | 2266-2381 |
| `9.10` | &nbsp;&nbsp;The home marker | `SPEC-MAP.md` | 2382-2560 |
| `9.11` | &nbsp;&nbsp;The provisional pin | `SPEC-MAP.md` | 2561-2584 |
| `9.12` | &nbsp;&nbsp;Icons — no pack, deliberately | `SPEC-MAP.md` | 2585-2612 |
| `9.13` | &nbsp;&nbsp;The storm glyph — 3D node mesh only | `SPEC-MAP.md` | 2613-2718 |
| `9.14` | &nbsp;&nbsp;Storm light on the backdrop | `SPEC-MAP.md` | 2719-2961 |
| `9.16` | &nbsp;&nbsp;Opening the Home drawer frames the house and the storm together | `SPEC-MAP.md` | 2962-3052 |
| `9.17` | &nbsp;&nbsp;The glass rim at the horizon | `SPEC-MAP.md` | 3053-3101 |
| `10` | Input — touch, mouse, keyboard all first-class | `SPEC.md` | 1196-1267 |
| `11` | Basemap tiles — OpenFreeMap (OpenMapTiles), z8 by design | `SPEC-MAP.md` | 3182-3228 |
| `11` | Basemap tiles | -> `SPEC-MAP.md` | SPEC.md:1268 |
| `11.1` | &nbsp;&nbsp;The two schemas are not interchangeable | `SPEC-MAP.md` | 3229-3251 |
| `11.2` | &nbsp;&nbsp;Administrative furniture — borders and place names | `SPEC-MAP.md` | 3252-3348 |
| `11.3` | &nbsp;&nbsp;The name ladder — each rung overlaps the last | `SPEC-MAP.md` | 3349-3389 |
| `11.4` | &nbsp;&nbsp;`to-number` on a missing property is 0, not null | `SPEC-MAP.md` | 3390-3413 |
| `11.5` | &nbsp;&nbsp;Label collision order is free, and it is load-bearing | `SPEC-MAP.md` | 3414-3435 |
| `12` | Code structure rules (summary — full rules live in project instructions) | `SPEC.md` | 1273-1292 |
| `12.1` | &nbsp;&nbsp;The constants table of contents — the grouping, without the split | `SPEC.md` | 1680-1710 |
| `12.2` | &nbsp;&nbsp;The relay mirrors — six facts written down twice | `SPEC.md` | 1711-1745 |
| `13` | Inherited hard-won rules | `SPEC.md` | 1746-1747 |
| `14` | Roadmap — RETIRED | `SPEC.md` | 2312-2318 |
| `15` | Open decisions — RETIRED | `SPEC.md` | 2319-2325 |
| `16` | Screen architecture | `SPEC-UI.md` | 1088-1089 |
| `16` | Screen architecture | -> `SPEC-UI.md` | SPEC.md:2326 |
| `16.4` | &nbsp;&nbsp;&nbsp;&nbsp;The heading arrow — one mark, three surfaces | `SPEC-UI.md` | 1724-1904 |
| `16.5` | &nbsp;&nbsp;&nbsp;&nbsp;The storm stepper, and the header both drawers share | `SPEC-UI.md` | 1905-2252 |
| `17` | Public operation | `SPEC-OPS.md` | 23-24 |
| `17` | Public operation — hardening, scale, and the money question | -> `SPEC-OPS.md` | SPEC.md:2332 |
| `17.1` | &nbsp;&nbsp;The disclaimer | `SPEC-OPS.md` | 37-151 |
| `17.2` | &nbsp;&nbsp;The inspect routes are gated | `SPEC-OPS.md` | 152-164 |
| `17.3` | &nbsp;&nbsp;No third party on the critical path | `SPEC-OPS.md` | 165-206 |
| `17.4` | &nbsp;&nbsp;`_headers` and the CSP | `SPEC-OPS.md` | 207-276 |
| `17.5` | &nbsp;&nbsp;Telemetry | `SPEC-OPS.md` | 277-530 |
| `17.6` | &nbsp;&nbsp;The app has a failure state for its own boot | `SPEC-OPS.md` | 531-559 |
| `17.7` | &nbsp;&nbsp;The origin collapse — one fetch per feed, globally | `SPEC-OPS.md` | 560-852 |
| `17.8` | &nbsp;&nbsp;The Cloudflare setup, recorded so it can be rebuilt | `SPEC-OPS.md` | 853-913 |
| `17.9` | &nbsp;&nbsp;Money, and the two things that can bill | `SPEC-OPS.md` | 914-979 |
| `17.10` | &nbsp;&nbsp;Push notifications are the one v2 feature that needs server state | `SPEC-OPS.md` | 980-991 |
| `17.11` | &nbsp;&nbsp;The PWA — service worker, icons, and the install door | `SPEC-OPS.md` | 992-1043 |
| `17.12` | &nbsp;&nbsp;IP, copying, and monetization | `SPEC-OPS.md` | 1095-1133 |
| `18` | Research from a sandbox with no internet | `SPEC-OPS.md` | 1154-1164 |
| `18.1` | &nbsp;&nbsp;Live web search — DISCOVERY | `SPEC-OPS.md` | 1165-1175 |
| `18.2` | &nbsp;&nbsp;WebFetch — TRIAGE | `SPEC-OPS.md` | 1176-1197 |
| `18.3` | &nbsp;&nbsp;The archive runner — EXACT BYTES | `SPEC-OPS.md` | 1198-1285 |
| `18.4` | &nbsp;&nbsp;The Cloudflare MCP — OUR OWN NUMBERS | `SPEC-OPS.md` | 1286-1292 |
| `18.5` | &nbsp;&nbsp;The order, and why getting it backwards is expensive | `SPEC-OPS.md` | 1293-1304 |
| `18.6` | &nbsp;&nbsp;The telemetry pull — THE SECOND ROAD TO OUR OWN NUMBERS | `SPEC-OPS.md` | 1305-1358 |
| `27` | Snapshot conditions | `spec-parameter.md` | 24-50 |
| `28` | The headline question: does GDACS give current wind? | `spec-parameter.md` | 51-55 |
| `28.1` | &nbsp;&nbsp;The trap — `severitydata.severity` is NOT current wind | `spec-parameter.md` | 56-101 |
| `28.2` | &nbsp;&nbsp;The real current wind, source A — timestepped wind-radii polygons | `spec-parameter.md` | 102-132 |
| `28.3` | &nbsp;&nbsp;The real current wind, source B — an exact number, buried | `spec-parameter.md` | 133-183 |
| `28.4` | &nbsp;&nbsp;The real current wind, source C — the track segment label | `spec-parameter.md` | 184-190 |
| `28.5` | &nbsp;&nbsp;Recommendation | `spec-parameter.md` | 191-201 |
| `29` | NHC `CurrentStorms.json` | `spec-parameter.md` | 202-208 |
| `29.1` | &nbsp;&nbsp;Scalar fields — the whole list | `spec-parameter.md` | 209-250 |
| `29.2` | &nbsp;&nbsp;Product objects — 13 of them, five distinct shapes | `spec-parameter.md` | 251-267 |
| `29.3` | &nbsp;&nbsp;`classification` codes | `spec-parameter.md` | 268-276 |
| `29.4` | &nbsp;&nbsp;What this feed does NOT contain | `spec-parameter.md` | 277-288 |
| `30` | NHC MapServer | `spec-parameter.md` | 289-294 |
| `30.1` | &nbsp;&nbsp;Layer block arithmetic — confirmed | `spec-parameter.md` | 295-307 |
| `30.2` | &nbsp;&nbsp;The 26-layer block, as actually observed (EP1, base 134) | `spec-parameter.md` | 308-344 |
| `30.3` | &nbsp;&nbsp;`Forecast Points` (offset +2) — the richest layer in the service | `spec-parameter.md` | 345-399 |
| `30.4` | &nbsp;&nbsp;Remaining data layers — field lists | `spec-parameter.md` | 400-478 |
| `30.5` | &nbsp;&nbsp;Query forms that work — TWO OF THEM, one per layer family | `spec-parameter.md` | 479-504 |
| `31` | GDACS event list | `spec-parameter.md` | 505-539 |
| `31.1` | &nbsp;&nbsp;Complete field inventory — 44 paths, all of them | `spec-parameter.md` | 540-590 |
| `31.2` | &nbsp;&nbsp;Type traps in this feed | `spec-parameter.md` | 591-603 |
| `32` | GDACS per-event geometry | `spec-parameter.md` | 604-609 |
| `32.1` | &nbsp;&nbsp;Feature census (Fausto, 95 features) | `spec-parameter.md` | 610-624 |
| `32.2` | &nbsp;&nbsp;Disambiguating the two polygon families | `spec-parameter.md` | 625-654 |
| `32.3` | &nbsp;&nbsp;`Line_*` track segments | `spec-parameter.md` | 655-677 |
| `32.4` | &nbsp;&nbsp;`Point_Polygon_Point_N` timestep dots | `spec-parameter.md` | 678-690 |
| `32.5` | &nbsp;&nbsp;Event-level properties are stamped on every feature | `spec-parameter.md` | 691-700 |
| `32.6` | &nbsp;&nbsp;The `forecast` flag is unreliable, and the dots overrule it | `spec-parameter.md` | 701-747 |
| `33` | GDACS `geteventdata` and the impacts chain | `spec-parameter.md` | 748-769 |
| `33.1` | &nbsp;&nbsp;`impacts` — agency attribution and the buried wind number | `spec-parameter.md` | 770-801 |
| `34` | How Landfall displays this data today | `spec-parameter.md` | 802-805 |
| `34.1` | &nbsp;&nbsp;Field → display | `spec-parameter.md` | 806-871 |
| `34.2` | &nbsp;&nbsp;Units — `lib/units.js` | `spec-parameter.md` | 872-900 |
| `34.3` | &nbsp;&nbsp;Saffir-Simpson thresholds — `lib/category.js` | `spec-parameter.md` | 901-937 |
| `34.4` | &nbsp;&nbsp;What the parsers actually match | `spec-parameter.md` | 938-967 |
| `35` | Findings — what this audit changed | `spec-parameter.md` | 968-971 |
| `35.1` | &nbsp;&nbsp;GDACS current wind — derived from band containment | `spec-parameter.md` | 972-984 |
| `35.2` | &nbsp;&nbsp;NHC publishes `ssnum` and we derive category instead | `spec-parameter.md` | 985-991 |
| `35.3` | &nbsp;&nbsp;MapServer `lat`/`lon` attributes are rounded to whole degrees | `spec-parameter.md` | 992-997 |
| `35.4` | &nbsp;&nbsp;GDACS timestamps carry no timezone and are UTC | `spec-parameter.md` | 998-1007 |
| `35.5` | &nbsp;&nbsp;Six of 26 MapServer layers break the bin-prefix naming convention | `spec-parameter.md` | 1008-1015 |
| `35.6` | &nbsp;&nbsp;CLOSED 2026-07-26 — the app left `EVENTS4APP`, and not for the bytes | `spec-parameter.md` | 1016-1032 |
| `35.7` | &nbsp;&nbsp;`MH` is real and has now been seen | `spec-parameter.md` | 1033-1037 |
| `35.8` | &nbsp;&nbsp;Dead weight | `spec-parameter.md` | 1038-1053 |
| `36` | Sample payloads | `spec-parameter.md` | 1054-1057 |
| `36.1` | &nbsp;&nbsp;NHC `CurrentStorms.json` — one storm, trimmed to scalars | `spec-parameter.md` | 1058-1091 |
| `36.2` | &nbsp;&nbsp;MapServer Forecast Points — tau 0 and tau 12 | `spec-parameter.md` | 1092-1137 |
| `36.3` | &nbsp;&nbsp;GDACS event list — one TC feature, properties | `spec-parameter.md` | 1138-1183 |
| `36.4` | &nbsp;&nbsp;GDACS geometry — the two Poly_Red kinds side by side | `spec-parameter.md` | 1184-1235 |
| `37` | Quick reference — do not get these wrong | `spec-parameter.md` | 1236-1272 |
| `45` | Genesis — the areas being watched | `SPEC-DATA.md` | 1428-1429 |
| `45.1` | &nbsp;&nbsp;Why this exists | `SPEC-DATA.md` | 1430-1457 |
| `45.2` | &nbsp;&nbsp;Source — NHC, the two- and seven-day outlook | `SPEC-DATA.md` | 1458-1617 |
| `45.3` | &nbsp;&nbsp;Source — JTWC, everywhere else | `SPEC-DATA.md` | 1618-1727 |
| `45.4` | Genesis — what it draws on the globe | `SPEC-MAP.md` | 3436-3495 |
| `45.5` | &nbsp;&nbsp;Failure behaviour | `SPEC-DATA.md` | 1728-1814 |
| `45.6` | &nbsp;&nbsp;Which horizon goes on the globe | `SPEC-MAP.md` | 3624-3641 |
| `45.7` | &nbsp;&nbsp;The standing visual risk | `SPEC-MAP.md` | 3642-3650 |
| `45.8` | Genesis — the drawer section | `SPEC-UI.md` | 2553-2744 |
| `45.9` | &nbsp;&nbsp;The text outlook — the arbiter over layer 3 | `SPEC-DATA.md` | 1815-1945 |
| `47` | The environment ribbon | `SPEC-NEXT.md` | 71-72 |
| `47.1` | &nbsp;&nbsp;Why this exists | `SPEC-NEXT.md` | 73-89 |
| `47.2` | &nbsp;&nbsp;Source — SHIPS | `SPEC-NEXT.md` | 90-215 |
| `47.3` | &nbsp;&nbsp;Ocean heat for the rest of the world — investigated, not adopted | `SPEC-NEXT.md` | 216-250 |
| `47.4` | &nbsp;&nbsp;What the color means | `SPEC-NEXT.md` | 251-445 |
| `47.5` | &nbsp;&nbsp;The environment ribbon — the cone, colored by what the environment is worth | `SPEC-MAP.md` | 1321-1603 |
| `47.6` | &nbsp;&nbsp;The coverage problem, stated plainly | `SPEC-NEXT.md` | 446-471 |
| `47.7` | &nbsp;&nbsp;Performance | `SPEC-NEXT.md` | 472-508 |
| `47.8` | &nbsp;&nbsp;The storm health paragraph | `SPEC-NEXT.md` | 509-854 |
| `47.9` | &nbsp;&nbsp;The layers row | `SPEC-UI.md` | 2763-2860 |
| `47.10` | &nbsp;&nbsp;The fixtures | `SPEC-NEXT.md` | 855-878 |
| `47.11` | &nbsp;&nbsp;The legend | `SPEC-NEXT.md` | 879-974 |
| `48` | Rainfall — the sources | `SPEC-DATA.md` | 1946-1947 |
| `48.1` | &nbsp;&nbsp;Why this exists, and what it deliberately is not | `SPEC-DATA.md` | 1948-1973 |
| `48.2` | &nbsp;&nbsp;Source — the advisory paragraph | `SPEC-DATA.md` | 1974-2016 |
| `48.3` | &nbsp;&nbsp;Source — gridded rainfall at a point | `SPEC-DATA.md` | 2017-2044 |
| `48.4` | &nbsp;&nbsp;Two traps in the payload, both measured | `SPEC-DATA.md` | 2045-2066 |
| `48.5` | &nbsp;&nbsp;Coverage, and the two shapes of "no" | `SPEC-DATA.md` | 2067-2096 |
| `48.6` | &nbsp;&nbsp;Flood warnings in force | `SPEC-DATA.md` | 2097-2134 |
| `48.7` | &nbsp;&nbsp;The relay route | `SPEC-DATA.md` | 2135-2179 |
| `48.8` | Rain — the home drawer's section | `SPEC-UI.md` | 2861-2925 |
| `48.9` | &nbsp;&nbsp;Rainfall — the storm drawer's section | `SPEC-UI.md` | 2926-2955 |
| `48.10` | &nbsp;&nbsp;The two numbers that disagree, and why both are right | `SPEC-UI.md` | 2956-2987 |
| `48.11` | &nbsp;&nbsp;Acceptance cases | `SPEC-DATA.md` | 2180-2225 |
| `48.12` | &nbsp;&nbsp;The provenance line, on both paths | `SPEC-UI.md` | 2988-3012 |
| `48.13` | &nbsp;&nbsp;The rainfall probe | `SPEC-OPS.md` | 1134-1153 |
| `48.14` | &nbsp;&nbsp;Source — rainfall where NWS does not reach | `SPEC-DATA.md` | 2312-2348 |
| `48.15` | &nbsp;&nbsp;The global relay route | `SPEC-DATA.md` | 2349-2381 |
| `48.16` | &nbsp;&nbsp;Which source answered, and the two meanings of `alerts: null` | `SPEC-DATA.md` | 2382-2432 |
| `48.17` | &nbsp;&nbsp;At your house — the point forecast in the storm drawer | `SPEC-UI.md` | 3013-3088 |
| `49` | Time has a direction | `SPEC-NEXT.md` | 975-976 |
| `49.1` | &nbsp;&nbsp;Why this exists | `SPEC-NEXT.md` | 977-1032 |
| `49.2` | &nbsp;&nbsp;The rule this section adds | `SPEC-NEXT.md` | 1033-1058 |
| `49.3` | &nbsp;&nbsp;The observed track | `SPEC-DATA.md` | 2226-2311 |
| `49.4` | &nbsp;&nbsp;Relative time has a direction | `SPEC-UI.md` | 3137-3159 |
| `49.5` | &nbsp;&nbsp;The closest pass, backwards | `SPEC-NEXT.md` | 1059-1111 |
| `49.6` | &nbsp;&nbsp;Strongest means strongest, over the whole life | `SPEC-NEXT.md` | 1112-1163 |
| `49.7` | &nbsp;&nbsp;The Timeline rail keeps the past | `SPEC-NEXT.md` | 1164-1282 |
| `49.8` | &nbsp;&nbsp;The chart's left half | `SPEC-NEXT.md` | 1283-1380 |
| `49.9` | &nbsp;&nbsp;The wind that already reached you | `SPEC-NEXT.md` | 1381-1456 |
| `49.10` | &nbsp;&nbsp;Moving home is not an edge case | `SPEC-NEXT.md` | 1457-1474 |
| `49.11` | &nbsp;&nbsp;Fixtures | `SPEC-NEXT.md` | 1475-1495 |
| `49.12` | &nbsp;&nbsp;Open questions for glass | `SPEC-NEXT.md` | 1496-1521 |
| `49.13` | &nbsp;&nbsp;The passes | `SPEC-NEXT.md` | 1522-1545 |
| `49.14` | &nbsp;&nbsp;The rungs that describe the past | `SPEC-NEXT.md` | 1546-1588 |
| `49.15` | &nbsp;&nbsp;A wind band is one field, not two halves | `SPEC-NEXT.md` | 1589-1631 |
| `49.16` | &nbsp;&nbsp;A GDACS storm gets the same wind countdown | `SPEC-NEXT.md` | 1632-1711 |
| `49.17` | &nbsp;&nbsp;A quadrant drawn at zero is a measurement, not an unreadable band | `SPEC-NEXT.md` | 1712-1799 |
| `50` | National agency alerts — what the rest of the world publishes | `SPEC-DATA.md` | 2433-2439 |
| `50.1` | &nbsp;&nbsp;Why this exists, and what it deliberately is not | `SPEC-DATA.md` | 2440-2487 |
| `50.2` | &nbsp;&nbsp;The payload, and the geometry we refuse | `SPEC-DATA.md` | 2488-2507 |
| `50.3` | &nbsp;&nbsp;Matching — by country, and only for GDACS storms | `SPEC-DATA.md` | 2508-2554 |
| `50.4` | &nbsp;&nbsp;Translation — the coded fields only | `SPEC-DATA.md` | 2555-2606 |
| `50.5` | &nbsp;&nbsp;The relay route | `SPEC-DATA.md` | 2607-2639 |
| `50.6` | &nbsp;&nbsp;The three states | `SPEC-DATA.md` | 2640-2660 |
| `50.7` | &nbsp;&nbsp;Acceptance cases | `SPEC-DATA.md` | 2661-2678 |
| `50.8` | &nbsp;&nbsp;A warning, a stand-down and a drill are three different things | `SPEC-DATA.md` | 2679-2705 |
| `50.9` | &nbsp;&nbsp;Severity onto the NHC colour rungs | `SPEC-DATA.md` | 2706-2730 |
| `50.10` | &nbsp;&nbsp;The shape route | `SPEC-DATA.md` | 2731-2750 |
| `50.11` | &nbsp;&nbsp;The warning area as a coast selector | `SPEC-DATA.md` | 2751-2855 |
| `50.12` | &nbsp;&nbsp;Coverage is the storms we list, and an unattributed storm says so | `SPEC-DATA.md` | 2856-2908 |
| `51` | Storm surge outside America — modelled water at named towns | `SPEC-DATA.md` | 2909-2910 |
| `51.1` | &nbsp;&nbsp;The product, and the one that is not it | `SPEC-DATA.md` | 2911-2997 |
| `51.2` | &nbsp;&nbsp;The relay route | `SPEC-DATA.md` | 2998-3027 |
| `51.3` | &nbsp;&nbsp;Surge at the house — the home dashboard section | `SPEC-DATA.md` | 3028-3076 |
| `51.4` | &nbsp;&nbsp;The coastal layer | `SPEC-DATA.md` | 3077-3127 |
| `51.5` | &nbsp;&nbsp;The boundary with NHC — DELIBERATE, SETTLED, DO NOT CLOSE | `SPEC-DATA.md` | 3128-3166 |
| `51.6` | &nbsp;&nbsp;Coastal flooding — the home drawer's section | `SPEC-UI.md` | 3089-3136 |
| `51.7` | &nbsp;&nbsp;The reach — how far one town's number travels along the coast | `SPEC-DATA.md` | 3167-3214 |
| `51.8` | A marker on the deepest towns — approved, not built, second pass | `SPEC-NEXT.md` | 1800-1837 |
| `52` | What a boot costs on each platform | `SPEC-NEXT.md` | 1857-1889 |
| `53` | JTWC's `.tcw` — a better source than the product we parse | `SPEC-NEXT.md` | 1890-1926 |
| `54` | People in the path — still coming, or already been through | `SPEC-UI.md` | 2450-2471 |

## Files

| File | Bytes | Numbered sections |
|---|---|---|
| `SPEC-DATA.md` | 191,964 | 55 |
| `SPEC-MAP.md` | 232,926 | 39 |
| `SPEC-NEXT.md` | 121,628 | 29 |
| `SPEC-OPS.md` | 82,795 | 21 |
| `SPEC-UI.md` | 186,151 | 14 |
| `SPEC.md` | 161,903 | 14 |
| `spec-parameter.md` | 59,622 | 50 |

