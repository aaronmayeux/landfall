...
export const SEASONS = Object.freeze({
  ...
  /* --- The season clock (§57.23, §57.30 step 10) --------------------------- */

  /**
   * ==> TWO DIALS, AND MIXING THEM UP IS THE ONE MISTAKE THIS FEATURE CAN MAKE
   * THAT NOBODY WOULD SEE. <== `lib/season-clock.js` `msPerStep` multiplies
   * them, so playback pace and rendering cost are chosen separately.
   *
   * `clockDaysPerSecond` is TASTE. §57.23 asks for about a day a second, which
   * puts a full Atlantic season at roughly three minutes and a single storm's
   * life at about ten seconds. Aaron's dial.
   *
   * `clockStepsPerSecond` is COST, and it is the number §57.35 fault 3 caps.
   * The data is six-hourly: at a day a second that is four real observations
   * per second of playback, so ten steps a second already draws each storm
   * between two and three times per recorded fix. Sixty would hand MapLibre a
   * fresh parse and re-index sixty times a second for no more information, and
   * the frame rate will not survive it. **Raising this makes playback more
   * expensive, never faster.**
   */
  clockDaysPerSecond: 1,
  clockStepsPerSecond: 10,

  /**
   * ==> HOW LONG THE CLOCK HOLDS ON THE LAST FRAME BEFORE STOPPING, IN
   * MILLISECONDS. <== Running off the end and halting on the same tick reads
   * as the app dropping the season rather than finishing it — the last storm
   * completes and everything stops in the same frame the reader is still
   * watching it. A short hold makes the ending an ending.
   *
   * It does NOT rewind. A reader who wants it again presses play, and the
   * clock starts over from the beginning because it is already at the end —
   * which is one press either way and never a surprise.
   */
  clockEndHoldMs: 900,

  /**
   * ==> HOW MANY MILLISECONDS OF STORM TIME AN ARROW KEY MOVES. <== Six hours,
   * which is one recorded observation: the smallest step that is guaranteed to
   * change what is on screen. A smaller value would give a keyboard reader
   * presses that visibly do nothing, on a control whose whole job is precision.
   *
   * ==> AND IT IS THE DATA'S CADENCE RATHER THAN A ROUND NUMBER. <== HURDAT2
   * is six-hourly by construction (§57.4). If a future source is hourly this
   * is the number that moves, and it should move to that source's cadence
   * rather than to something that feels nice.
   */
  clockNudgeMs: 6 * 60 * 60 * 1000,
});
