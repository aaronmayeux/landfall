...
/* ---------------------------------------------------------------------------
 * WIND SWATH SMOOTHING (SPEC §7)
 *
 * NHC's wind swath polygons arrive RASTERIZED. They are generated by tracing
 * a grid, so every boundary is a staircase of perfect right angles at a fixed
 * cell size — confirmed on glass 2026-07-24 (Fausto, all three bands). The
 * underlying shape is correct and already merged into one envelope per
 * threshold; only the outline is quantized.
 *
 * WE SMOOTH WITH CHAIKIN'S CORNER-CUTTING, and the choice matters for a
 * safety layer. Chaikin replaces each corner with two points set in from it,
 * so the smoothed ring sits INSIDE the original — the error direction is
 * conservative. A spline through the same points draws prettier curves but
 * overshoots on tight corners, pushing the boundary OUTSIDE the true extent
 * and claiming hurricane-force wind where NHC claims none. Inward is the
 * only acceptable direction to be wrong here.
 *
 * THIS DEVIATES FROM OFFICIAL GEOMETRY, on purpose and by a stated amount.
 * Two iterations move the boundary inward by at most about half a grid cell
 * — negligible against a swath spanning hundreds of nautical miles, but real,
 * and recorded here rather than left to be discovered as a mismatch later.
 * ------------------------------------------------------------------------- */

export const WIND_SMOOTH = Object.freeze({
  /** Chaikin iterations. Each pass quadruples smoothness and doubles the
   *  point count, so this is the cost dial as much as the look dial.
   *  2 dissolves the staircase; 3 is imperceptibly rounder for twice the
   *  points; 1 leaves visible steps. */
  iterations: 2,

  /** The cut ratio. 0.25 is textbook Chaikin (quarter-point on each side of
   *  a corner) and is what makes the inward shift bounded and predictable.
   *  Not intended as a tuning knob — it is here so the number is not
   *  unexplained in the math. */
  ratio: 0.25,

  /** Rings shorter than this are passed through untouched. A triangle or a
   *  sliver has no staircase to remove, and smoothing one collapses it
   *  toward its centroid — a small band would visibly shrink rather than
   *  soften. */
  minRingPoints: 8,
   ...
   *  draws raw: jagged and honest beats smooth and dropped (§9 — feel is the
   *  overriding lens, and a stutter is worse than a staircase). */
  maxRingPoints: 4000,
});
