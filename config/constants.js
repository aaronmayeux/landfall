...
/* ==> TABLE OF CONTENTS — 68 blocks, ten groups <==
 *
...
/* ---------------------------------------------------------------------------
 * FLOOD ON THE MAP — how the alerts draw and when (SPEC §56.5)
 *
 * ==> BEHAVIOURAL THRESHOLDS LIVE HERE AND COLOURS LIVE IN tokens.js, THE SAME
 * SPLIT `GENESIS` / `GENESIS_GEO` MAKES. <== A zoom gate is a rule about when
 * something is worth drawing; a green is a rule about what it looks like. The
 * flood pair mirrors the genesis pair so there is one place to look for each
 * kind of question.
 *
 * ==> THESE ARE THE DIALS AARON MOVES ON GLASS, AND NOT ONE OF THEM HAS BEEN
 * SEEN YET. <== §56.13: nothing about this feature can be judged until a US
 * storm is near a flooding region. Every value below is derived from an
 * OFFLINE measurement — pixels on a screen, counts in a capture — which is a
 * real basis and is not the same thing as having looked at it.
 * ------------------------------------------------------------------------- */

export const FLOOD = Object.freeze({
  /** Below this zoom the polygons are not drawn at all.
   *
   *  ==> IT COMES STRAIGHT OFF §56.2's PIXEL TABLE. <== Measured in web
   *  mercator against the real twenty-five: at z5 the MEDIAN polygon is 6.1 px
   *  across and the smallest is 1.4. At z6 the median is 12.3. Under about a
   *  dozen pixels a county-sized warning is not a shape, it is a green speck
   *  that reads as dirt on the screen — and fifteen of the twenty-five sat in
   *  one line along the Wabash valley, so at planet distance they merge into a
   *  single smear that misstates how many there are.
   *
   *  ==> NOTHING IS LOST BY GATING IT, BECAUSE THE ICON IS WHAT ANSWERS AT
   *  DISTANCE. <== The icons are NOT gated. Below this zoom the layer is a
   *  scatter of clustered dots with counts on them, which is the honest read
   *  of "twenty-five alerts, most of them here" — a picture the polygons
   *  genuinely cannot draw at that size. */
  polygonMinZoom: 6,

  /** By this zoom the polygons are at full strength. Between the two the fill
   *  and the outline ramp up, so the shapes arrive rather than snapping on.
   *
   *  §56.2: the median is 24.6 px at z7 and the SMALLEST is 5.5, so this is
   *  the first zoom at which the whole set — not just the big ones — is a
   *  shape a reader can judge. A hard switch at one zoom makes a pinch feel
   *  like a bug; one zoom level of ramp is the shortest fade that reads as
   *  intentional. Motion durations live in config/motion.js; this is a zoom
   *  interpolation and belongs with the zooms. */
  polygonSolidZoom: 7,

  /** How near two icons have to be, IN PIXELS, before MapLibre merges them.
   *
   ...
   *  right on top of the cluster at z7. It DOES NOT resolve by zooming in.
   *  Three warnings with no icon and nothing on screen saying so is §5's
   *  silence with a map over it.
   *
   *  Clustering answers it because nothing is ever dropped: two alerts that
   *  cannot both be drawn become one dot reading "2". 44 is the touch target
   *  (§10) — icons closer together than a fingertip cannot be tapped apart, so
   *  merging them is not a loss of information, it is an accurate statement
   *  about what this screen can resolve. */
  clusterRadiusPx: 44,

  /** Past this zoom nothing clusters — every alert is its own dot.
   *
   ...
   *  dot would be hiding a distinction the reader can already see. This is
   *  also where the polygons have been solid for two zoom levels, so the dot
   *  is decoration over a shape rather than the only marker. */
  clusterMaxZoom: 9,

  /** The tap box, in pixels, for both a dot and a cluster.
   *
   *  §10's 44 px minimum, and the same number `map/markers.js` builds its
   *  storm hit box from. Stated here rather than reaching for `SIZE` because
   *  this is a hit-testing rule for a hazard layer, and the day somebody makes
   *  the storm dot easier to hit is not the day flood alerts should silently
   *  change too. */
  tapBoxPx: 44,
});
