...
| `config/tokens.js` | 2432 | **Exempt** — same reason as constants.js: one table, no logic. |
| `ui/panels.css` | 3360 | **Exempt, and the threshold below it was missed.** See below. Crossed 2,500 when the environment paragraph and the freshness column landed; still declarative all the way down. |
...
| `map/layers/flood.js` | 724 | **Newly over the line, by twenty-four, and the crossing is Slice B's chips.** About 78% of this file is the reason beside the code: the header alone is ninety lines, because it carries the three faults §56.15 records and the argument for how each one is designed out. Four separable concerns: the two caches and the corridor match; the CHIP (four canvas images, `chipName`, `chipImage`, `ensureChipImages`); the paint expressions (`colorExpr`, `rampTo`, `isWarningExpr`, `chipExpr`, `countInkExpr`, `countHaloExpr`); and the layer definition itself. **The chip half is the clean lift and it is named here so the next session does not have to find it** — `chipImage` and the five expressions are pure functions with no module state, reading only tokens and the theme, so they move to a chip file of their own with no behaviour change and no new state to thread. ==> **IT WAS NOT DONE IN THE PUSH THAT CROSSED THE LINE, AND THE REASON IS THE PHASE ITSELF.** <== §56.16 lands this feature in three slices precisely so each one can be felt on a phone and reverted alone; folding a behaviour-neutral split into the same commit as new behaviour is what made the first attempt unbisectable. **Slice C is the next thing to touch this file, and it does the lift first.** |
