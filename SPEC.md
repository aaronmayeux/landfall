...
| `lib/windswath.js` | 719 | **Newly over the line by nineteen, and I put it there — the count is recorded rather than absorbed.** It was 655 before §7.14 added `leadingOvertaken` (11 lines of code) and the reason beside it. It is TWO things wearing one filename, and the seam is the same shape as `lib/trackline.js`'s directly above. **The planar sweep geometry** — `radiusAtBearing`, `centreOfRadiiRing`, `fullRing`, `sweepRun`, and the despike and loop-cut passes inside it — is pure maths about quadrant roses and swept corridors, knows nothing about a storm or an advisory, and `sweepRun` alone is 240 lines. **The tier assembly** — `leadingOvertaken` and `buildFullTrack` — is entirely about what a SOURCE published, which clock each product is on, and which of its hours are still true; almost every line of it is the argument rather than the arithmetic. **The lift is the geometry**, into a new module under `lib/`, taking roughly 380 lines and leaving the assembly reading as one story. It is a clean lift because the dependency runs one way: assembly calls geometry, geometry never calls assembly — the same test that made the trackline cut obvious. **Only 357 of these lines are code.** Take the cut before the next thing lands here. |
