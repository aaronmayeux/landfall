...
| `data/nhc-mapserver.js` | 719 | **Nineteen over, and the seam repair put it there — recorded rather than absorbed.** It was 664 before §7.9's stitch was wired at the door on 2026-08-24: **11 lines of code and 44 of reason**. Recounted rather than carried forward: **249 code, 435 comment, 35 blank** — only 35% of the file is code, and the highest comment ratio of anything in this table outside `config/constants.js`. That is the right shape for it: almost every line is a fact about what NHC's MapServer actually returns, measured off archived bytes, and deleting the reason would leave twenty-two magic layer ids and a sentinel value nobody could defend. **The cut, if one is wanted, is the wind-tier assembly** — `normalizeForecastRadii`, `normalizePastRadii` and the block that builds the two envelopes are about wind radii rather than about the MapServer, and they are the only part of this file that would read as a story somewhere else. Roughly 150 lines. **Not taken here, deliberately:** this was a repair wired ahead of a fault appearing on glass, and a file move inside it would put the risk somewhere other than where the thinking was. Take it when the next thing lands here, not before. |
