...
| `data/home-dashboard.js` | 909 | **Newly over the line.** Crossed it when the class-milestone walk landed. Three concerns are already visible and already independent: the approach maths (`closestApproach` feeding `band`, `atClosest`, `nearRing`), the intensity story (`peak`, `arrivalTrend`, `peakWhen`, `milestones`), and the `stage` ladder, which reads all of them and is the only part that has to. `nearRingWindow` and the corridor call are the natural seam. Not urgent — every piece is pure and separately tested — but it does not get to grow again first. **It grew anyway, by 134 lines, for §49's pass 3**, and the growth landed inside the two concerns already named rather than making a third: the past pass beside the approach, the observed wind beside the forecast one, the peak spanning the whole life. The seam is unchanged and the debt is now explicit. |
