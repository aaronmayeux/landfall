...
| `ui/season-detail-markup.js` | 688 | **Under the line, and the cut was taken in the pass that caused the crossing rather than the pass after it.** §57.48 added 7 lines — the `comebackHtml` slot, which is a slot rather than a renderer precisely because this file had 19 lines of headroom and the sentence itself went to `ui/season-shape-markup.js` (148). §57.44's archive ranking section took it from 698 to 824. Every other row in this table records a ceiling crossed, a cut named, and the cut taken one or more passes later at a bigger size — `ui/view-seasons-board.js` did it five times and `ui/chart-home.js` is 337 over with its lift still named and untaken. **So this one was taken immediately, and that is the only reason it is a 651 instead of a fourth row promising something.** `ui/season-rank-markup.js` (202) holds `ordinal`, `seasonRankHtml` and `archiveRankHtml`. **The boundary is the SUBJECT, like the roster/furniture split above:** everything left here renders a FACT about one storm — its peak, its life, its landfalls, its wind field — and the two that moved render a COMPARISON, which is a different kind of claim and the only one on the panel that can be wrong because of what OTHER storms did. They are also the only two callers of `ordinal`, which went with them. **The dependency runs one way**: the rank file imports `rowsHtml` and `absenceHtml` from here and nothing here knows it exists, and no convenience re-export was left behind. **No behaviour changed in the move**, deliberately, so a break can only be the move — `tools/test-season-detail.mjs` (97) and `tools/test-rankings.mjs` (125) were both green before and after, and the former now imports both modules behind one handle because its subject was always "everything the panel draws" rather than one file. |
