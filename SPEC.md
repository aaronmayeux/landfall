...
| `map/layers/flood.js` | 757 | **Over the line, and the last three passes have moved it 724 up, 633 down, 757 up again.** Going national deleted the corridor match, the memo, the held storm and the held bundle; the glyph fix then added the wave path, the `drawWaves` mapping and four more chip images. **About three quarters of the file is the reason beside the code** — the header alone carries §56.15's three faults, why each is designed out, and why the per-storm reading of §56.1 was wrong. Three separable concerns: the CHIP (`chipName`, `chipImage`, `drawWaves`, `waves`, `ensureChipImages`), the paint expressions (`colorExpr`, `rampTo`, `isWarningExpr`, `chipExpr`, `countInk`, `countInkExpr`, `countHaloExpr`), and the layer definition. ==> **THE CHIP HALF IS THE LIFT AND IT IS NOW OVERDUE.** <== Those functions are pure, hold no module state beyond one lazily-built `Path2D`, and read only tokens, the theme and the icon set — they move to a file of their own with no behaviour change and nothing new to thread. Taking them also takes this file's only import from `ui/` with them, which is the tidier place for it. **The entry has now said this twice and the file has grown both times. Slice C does the split first, feature or not.** |
