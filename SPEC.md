...
**The next feature that wants a home in this file does the inventory and the cut FIRST**, with no behaviour change, so a break can only be the move. The visible seam: the five view factories and their callback bags are one concern; the home cluster — marker, provisional pin, setup and dashboard — is a second. §51.3's surge facade added ~20 lines of the first kind — one injected data path in the shape every other one here already has. **§56.5 and §56.6 added ~100 more of the same kind** — the flood-alert detail panel's construction and registration, one delegated listener for the row-to-panel path both `Flooding` sections share, and `refreshFloodRow` / `openFloodAlert` / `setFloodDraw`. All orchestration: the geometry went to `lib/interior-point.js` and the feature building to `lib/flood-features.js`, which is the pattern that has kept this file readable. **It is 1221 lines against a 700-line rule now, so the next thing that wants to live here gets asked whether it is genuinely wiring.** |
