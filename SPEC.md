...
| `app/views.js` | 836 | **Watch, and it grew again.** The composition layer that came out of `main.js` pass 3. Almost all of it is wiring — the drawer, the five views, the home marker and the provisional pin, knotted by construction order. Splitting it would put half a knot in each of two files, which is the thing it exists to prevent. The Home drawer's opening camera frame (§9.16) added ~50 lines of it; the *maths* went to `map/home-frame.js`, only the orchestration landed here. Next feature that wants a home in this file gets an inventory and a cut list first. |
