...
| `lib/windswath.js` | 964 | **THE CUT IS NOW BADLY OVERDUE AND THIS ROW SAYS SO PLAINLY.** It was 655, then 719 when §7.14 added `leadingOvertaken`, 794 when §7.12 fault 3 added the antimeridian branch discipline, and **964 since §7.12 fault 4 — the spine split that made the bands nest on a looping storm — landed on 2026-08-29.** Recounted rather than carried forward: **418 code, 491 comment, 55 blank**, so 51% of the file is the argument beside the arithmetic. **Fault 4 was a bug fix on a live wrong shape and the cut was NOT taken inside it, deliberately** — surgical strike: an error gets the targeted fix, not a rewrite of the file, and this one ships to the live globe in hurricane season and needed its own revert point. That is the second time this row has recorded that decision, which is itself the argument for taking the cut now. The structure has not changed and neither has the seam. It is TWO things wearing one filename, the same shape as `lib/trackline.js`'s directly above. **The planar sweep geometry** — `radiusAtBearing`, `centreOfRadiiRing`, `fullRing`, `cutAllLoops`, `trackSplitPoints`, `breakRun` and `sweepRun` — is pure maths about quadrant roses and swept corridors, knows nothing about a storm or an advisory, and `sweepRun` alone is 240 lines. `lonStep` and `onOneBranch` go with it: they are arithmetic about the number line, and `lib/cone-sweep.js` already holds its own copy of the same idea, which is the second caller the extraction rule names. **The tier assembly** — `leadingOvertaken` and `buildFullTrack` — is entirely about what a SOURCE published, which clock each product is on, and which of its hours are still true. **The lift is the geometry**, into a new module under `lib/`, now roughly 560 lines and leaving the assembly reading as one story. It is a clean lift because the dependency runs one way: assembly calls geometry, geometry never calls assembly. **It is the next thing that happens to this file, before any feature.** |
