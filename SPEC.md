...
| `map/layers/flood.js` | 633 | **Over the line, and it came DOWN ninety-one lines on the day it crossed.** It reached 724 when Slice B added the chips; going national on 2026-08-23 deleted the corridor match, the memo, the held storm, the held bundle and the `floodMatchRuns` seam, and turned `update()` and `clear()` into empty functions. **What is left over the ceiling is prose, not behaviour**: the header is ninety lines because it carries the three faults §56.15 records, the argument for how each is designed out, and the reason the per-storm reading of §56.1 was wrong — all of it written so nobody reverses the reversal. Three separable concerns remain: the CHIP (four canvas images, `chipName`, `chipImage`, `ensureChipImages`), the paint expressions (`colorExpr`, `rampTo`, `isWarningExpr`, `chipExpr`, `countInkExpr`, `countHaloExpr`), and the layer definition itself. **The chip half is still the clean lift** — those seven functions are pure, hold no module state, and read only tokens and the theme, so they move to a file of their own with no behaviour change and nothing new to thread. **Slice C does it first**, because folding a behaviour-neutral split into the same commit as new behaviour is what made the first attempt at this phase unbisectable. |
