...
| `map/imagery.js` | 1056 | **Watch, and the cut is now named.** Wave 6 (§4.9) added 117 lines swapping radar from NOAA to RainViewer — per-request pixel sizing, the centre-and-zoom addressing, and the branch that asks the coverage mask why a frame came back blank. All three landed inside the existing fetch/paint concern rather than making a new one, and the two genuinely separable pieces went OUT rather than in: the mask's fetch, cache and verdict to `data/radar-coverage.js`, and the wording of a blank-radar row to the same file, so the sentence lives beside the states it reads. What remains is three concerns — the MapLibre source/layer lifecycle, the fetch-and-cache loop with its retry ladder, and the pixel pass — and the layer lifecycle is the clean lift. Inventory before the next imagery pass that is more than a seam. |
