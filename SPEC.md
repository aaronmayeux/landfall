...
| `app/views.js` | 905 | **Watch, and the cut list is now owed.** The composition layer that came out of `main.js` pass 3. Almost all of it is wiring — the drawer, the five views, the home marker and the provisional pin, knotted by construction order. Splitting it would put half a knot in each of two files, which is the thing it exists to prevent. The Home drawer's opening camera frame (§9.16) added ~50 lines of it and the two-stage house tap another ~40; both times the *decision* went to its own module (`map/home-frame.js`, `map/home-tap-stage.js`) and only the orchestration landed here, which is the pattern that has kept this file readable. It has now had two features in a row on that deal. **The next one does the inventory and the cut FIRST**, with no behaviour change, so a break can only be the move. The visible seam: the five view factories and their callback bags are one concern; the home cluster — marker, tap stage, provisional pin, setup and dashboard — is a second and the larger. |
