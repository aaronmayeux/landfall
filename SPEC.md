...
| `ui/view-seasons-board.js` | 1064 | **==> THE ROW SAID FOUR TIMES THAT STEP 9 MUST TAKE AN INVENTORY BEFORE IT ADDED ITS FILTER AND ITS SLIDER. IT DID NOT. <==** Step 9 landed here first and the file went 868 -> 1064, 364 over. That is the finding, and it is worth more than the number: a ceiling row that names a cut and defers it does not stop a feature, because the feature arrives with its own reasons and the row is read after the code is written. **The two NEW files step 9 did create were real cuts** — `ui/seasons-near-home.js` took the measurement, the slider markup and the row note; `lib/near-home-words.js` took the wording — so what landed here is only what could not leave: state the reader chose (`radius`), the getters that keep the house and the units current, the bundle six callers read, and the live drag handler. **THE INVENTORY, TAKEN LATE, IS THIS.** Roughly 300 lines of event routing (`onClick`, `onChange`, `onInput`, `onKeydown`, `repaintRoster`); roughly 200 lines of selection and focus rules; roughly 180 of render; the rest is state and the returned surface. **THE NAMED SEAM IS UNCHANGED AND STILL NOT CLEAN** — every branch in the handlers reads `basin`, `year`, `filter`, `radius`, `ticked` and `focused`, so lifting them means a callback bag as wide as the state itself, which §12 calls a knot in two files rather than one. **What changed is that the knot now has a name and a shape**: the handlers do not need the state, they need six ACTIONS — choose a basin, choose a year, choose a filter, set the radius, toggle a tick, open a storm — and every one of those already exists as a function in this file. A new input module takes the routing and is handed those six. That is a real seam and it is the next session's, on its own, with no feature riding on it. **Do not let step 10's season clock land here first.** |
