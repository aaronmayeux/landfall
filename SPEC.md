...
| File | Lines | Call |
|---|---|---|
| `config/constants.js` | 7765 | **Exempt — standing** (above), and the exemption was re-measured rather than re-asserted. Was 5,509 before `VOLCANO` (1,972 lines), `PLATE_LINE` (223) and `TILT` (64) moved to their own files, all three since deleted. **Only 858 of these lines are code** — 5,191 are comment and 524 blank, so 87% of the file is the reason beside a number. Recounted 2026-08-24 rather than carried forward; the previous row said 705 and 77%, and those two figures did not agree with each other even then. `tools/module-graph.mjs` was run against all 63 blocks: every one is read by the SHIPPED app, zero are off-path. `WIND_BAND_KT` was the one dead export and is deleted. A grouped table of contents at the head of the file is generated and enforced by `tools/constants-toc.mjs` — see §12.1. §51 added `GDACS_SURGE` (64 blocks), which is the same shape as every row here: a reason per number, and every one of them read on the shipped path. §56.8 added `CAP` (67 blocks) — one string, `surgeEventMatch`, and 37 lines of which 2 are code: it decides whether a CAP row lands in `Watches and warnings` or in `Flooding`, and the alternative was a bare `'storm surge'` literal in feature code, which is the unexplained constant this section forbids. §57.30 step 2 added `SEASONS` (69 blocks), the history parser's whole rulebook — storm-number bands, ATCF column positions measured off real files, the dateline threshold — and it is the block that took this row from 6,179 to 6,572. **Recounted 2026-08-31 for §57.67: 7,765 lines, of which 953 are code, 6,207 comment and 606 blank — 80% reason beside a number.** The clock's three pace constants added 48 lines, 3 of them code. The row had drifted 346 lines behind the file before the previous pass, which is worth knowing: `doc-check` runs a 5% tolerance on this number, so a stale row goes unreported until some unrelated change tips it over — and that is exactly what happened here, the gate firing on a 48-line addition because 428 lines of drift were already sitting under it. |
| `config/tokens.js` | 2847 | **Exempt** — same reason as constants.js: one table, no logic. Grew 266 lines when `SEPIA` landed (§57.20). **That is a THIRD palette and it did NOT cost a third copy** — it spreads `DARK` and overrides only what moves, because sepia is DARK's lighting model at a different temperature rather than a different model. A full restatement would have been ~680 lines whose unchanged half would drift. The cost of a spread is that a colour added to `DARK` is inherited here in cyan, silently, on a globe nobody is currently looking at — `tools/test-sepia.mjs` fails the build on exactly that, so a themed colour is two edits and the second is not optional. |
| `ui/panels.css` | 3591 | **Exempt, and the threshold below it was missed.** See below. Crossed 2,500 when the environment paragraph and the freshness column landed; still declarative all the way down. §56.6 added the flood detail panel's rules and the pressable alert row — the row's whole block exists to make a `<button>` look like the prose it replaced, because §48.6 records a tinted row being reverted once already. |
| `functions/tiles/_pmtiles.js` | 1721 | **Exempt — vendored.** Third-party library, not our code, never edited by hand. |
| `ui/view-storm-detail.js` | 1929 | **Over the line, and coming DOWN for the first time.** The Environment section (§47.8) went in as four seams — a section row, an ensure, a wire and a repaint — with its whole controller in `ui/env-health.js`, and Rainfall (§48.9) followed the same shape. The gust (§16) went in as state, a guard, an ensure and a second section repaint, all in this file, because it feeds one `<dd>` inside a section this file owns; that was the last addition that worked, and the entry said so: THE NEXT DETAIL PASS OF ANY SIZE DOES THE SPLIT FIRST. ==> §54 WAS THAT PASS AND IT PAID THE DEBT INSTEAD OF ADDING TO IT. <== The past/ahead headcount split would have put ~114 lines into this file. Instead the whole People in the path section moved out to `ui/people-in-path.js` — state machine, wording, arithmetic and all — and what is left here is four seams. Net 100 lines off. The stamp, the section renderers, the advisory record and the stepper remain the four separable concerns, and the next one out is the stepper. **§56.7 added 81 lines and took only the seams the rule allows** — a section row, a body function, a repaint, one ensure and one wire, with the whole Flooding controller in `ui/flooding-storm.js` and every sentence it says in `ui/flood-words.js`. `wwHtml` also split into itself plus `wwBody()` so §56.8's pointer is appended once for both halves of that section rather than written into either. **That is 169 over the last recorded figure and the stepper is still uncut: the next detail pass of any size does the split first, and this entry has now said so twice.** |
| `ui/view-settings.js` | 702 | **Newly over the line, by two, and the crossing is a comment rather than a feature.** §"One text system" renamed the imagery block's heading to `Satellite imagery` and wrote down why in six lines — both sliders feed `map/imagery.js`, and radar has read neither since it became its own file in Wave 6. Six blocks build the panel (`themeBlock`, `unitsBlock`, `meshGroup`, `rotateBlock`, `imageryBlock`, `aboutBlock`), each one a heading and its controls, plus `sliderRow` shared between three of them. **The one real seam is the INSTALL half** — `INSTALL_STATE`, `installState`, `manualPlatform`, `MANUAL_STEPS`, `MANUAL_LEAD`, `installBlock` and `syncInstall` are 130 lines that share nothing with a settings control except the panel they land in, and they carry the whole argument about why absence of `beforeinstallprompt` is not absence of capability. That is the lift if anything else lands here. Watch, don't split yet: two lines over on a file whose blocks are all the same shape is not the failure §12 is watching for. |
| `config/layers.js` | 834 | **Newly over the line, and exempt for `constants.js`'s reason rather than by analogy.** It is the layer MANIFEST: frozen data, one entry per row, with the argument for each default written beside it. The only logic in it is four small helpers at the foot (`isLive`, `pairLiveOptions`, `layerGroups`, `defaultLayerState`), none of which can throw and none of which has state. Splitting it would put half the inventory in each of two files and give the panel two places to look. Crossed the ceiling when the environment row landed; grew again when §47.9's note was rewritten and the reasoning behind it recorded. |
| `ui/view-storms.js` | 1353 | **Watch, and the cut is now named.** Two row builders and the list chrome. The storm row and the watch row are deliberately the same shape and should move together when they move. |
| `main.js` | 2002 | **Cut in three passes, done.** §57.30 step 4 added ~135 lines and all of it is the fourth kind of wiring: the `liveGlobe` hide/show facade the archive is handed, the dynamic-import entry, the `?season=` boot hook, and five one-word gates that stop a poll painting live storms onto the archive globe. The facade is deliberately here rather than in `seasons/` — the archive must not reach into `map/` or `data/` (§12) — but it is also the clearest argument yet for the `warmable layer` helper below: `hide()` is a hand-written list of every globe surface, and the next layer somebody adds without adding a line to it appears ON the archive. It grew 246 lines since, of which 89 are code and all 89 are wiring. See below. The environment ribbon (§47) added ~40 more, all of it the warm-and-push pair that model guidance already had — the same shape twice is the signal that a `warmable layer` helper is the next lift out of here, not a fourth copy. **§48.21's flood layer is that third copy and it went in anyway**: `ensureFlood` is `ensurePopulation`'s shape with a status push bolted on. The helper is now the cheapest real cut available in this file. **§57.30 step 6 added ~50 more, and every line of it is the same fourth kind of wiring**: `archiveGlobe` grew a `setFocus`, the two archive layers are attached at `style.load`, the tap handler gained an archive branch, and the loaded Seasons module is stashed so a tap on a track can reach the board without a second dynamic import inside a click handler. Nothing about the archive's behaviour is in here — WHERE a track is drawn and WHEN it brightens are both somewhere else — but the row is now 106 lines above where it was last recorded and the `warmable layer` helper has been "the next cut" for four passes running. **§57.30 step 6b added 20 more and it is the fifth pass in a row** — the wind footprint's three calls folded into the `archiveGlobe` facade beside the two that were already there, which is exactly the "same shape three times" this row keeps naming. Take the helper. **§57.21c added ~158 and it is the SIXTH pass, and this time the row's own prediction came true rather than being restated.** This table has warned for five passes that `hide()` is a hand-written list of every globe surface and that the next layer added without a line in it appears ON the archive. That is exactly the bug §57.21c fixed — except it was not a new layer, it was the layer engine's whole ambient set, which had never been in the list at all. **The warning was right and it under-stated itself: the omission was there from the day the facade was written.** What went in: `ambientPrune`/`repushAmbient` in the facade, two archive-gate lines in `subscribe`, `liveRunningIds`, `archiveOffset`, two camera methods, the ridge flatten, and the `data-door` read. All of it the fourth kind of wiring, none of it archive behaviour — but the file is now 184 lines above the last recorded figure and the `warmable layer` helper has been "the next cut" for six passes. **The helper would not have prevented this one.** Ambient geometry is not a layer with a warm-and-push pair; it is the engine's own set, and the real lesson is that `hide()` should be asking the engine what it holds rather than listing it. Whichever pass opens this file next takes one of the two. **§57.21d added ~146 and it is the SEVENTH pass**: the glyph list and its two writers in the `archiveGlobe` facade, the ordered archive tap branch, `openSeasonStormNow`, the press clock, and `minimiseArchiveSheet`. 2,002 lines now, of which **623 are code** — 1,271 are comment and 109 blank, so 63% of this file is the reason beside a line. It is the composition root and every one of those 146 lines is wiring or a DOM measurement that only the composition root is allowed to take, which is the same defence the previous six passes made and is wearing thin. **The cut named above has not moved and neither candidate got any cheaper.** |
...
| `lib/trackline.js` | 833 | **OVER THE LINE BY 133, AND 75 OF THOSE LANDED AGAINST THIS NOTE ON 2026-08-31.** §57.67 slice B added `smoothPathIndexed` — the fix-to-vertex map `smoothPath` already computed internally and threw away — because the season clock has to cut a cached curve at a recorded fix and the alternative was matching floats against the curve. **It went in here rather than in a new file because it needs `dedupe` and `spline`, which are private to this file and are exactly what the cut below would lift.** Recounted rather than carried forward: **375 code, 458 comment**, one concern, and §12's ceiling targets accumulated behaviour rather than length. That is the argument for it being tolerable, not for it being right: **the next thing that touches this file takes the cut first.** The cut is obvious rather than arguable. Crossed it when the 2026-08-21 pass added `dropRepeats` and the gap-ratio override at the seam (§7.4). It is TWO things wearing one filename. **The planar-geometry primitives** — `dLon`, `sep2`, `isPt`, `turnDeg`, `runLength`, `unwrapLons`, `dedupe`, `sample`, `spline` — are pure maths about points and curves, know nothing about a storm, and are already borrowed by `lib/ringpolish.js`'s constants. **The assembly** — `runsFrom`, `dropRepeats`, `unfold`, `stitch`, `orient`, `extendToAnchor`, `smoothTracks` — is entirely about what a SOURCE published and how to make one honest path out of it, and every line of it is the reason rather than the arithmetic. **The lift is the primitives, into a new curve-maths module under `lib/`**, which takes roughly 200 lines with it and leaves the assembly reading as one story. It is a clean lift because the dependency runs one way: assembly calls maths, maths never calls assembly. Nothing landed here against this note yet — take it before the next thing does. |
| `lib/windswath.js` | 1,068 | **THE CUT IS NOW BADLY OVERDUE AND THIS ROW SAYS SO PLAINLY.** It was 655, then 719 when §7.14 added `leadingOvertaken`, 794 when §7.12 fault 3 added the antimeridian branch discipline, 964 when §7.12 fault 4 — the spine split that made the bands nest on a looping storm — landed on 2026-08-29, and **1,068 since fault 4b merged the split pieces back into one shape on 2026-08-30.** Recounted rather than carried forward: **469 code, 537 comment, 62 blank**, so 51% of the file is the argument beside the arithmetic. **Fault 4 was a bug fix on a live wrong shape and the cut was NOT taken inside it, deliberately** — surgical strike: an error gets the targeted fix, not a rewrite of the file, and this one ships to the live globe in hurricane season and needed its own revert point. That is the second time this row has recorded that decision, which is itself the argument for taking the cut now. The structure has not changed and neither has the seam. It is TWO things wearing one filename, the same shape as `lib/trackline.js`'s directly above. **The planar sweep geometry** — `radiusAtBearing`, `centreOfRadiiRing`, `fullRing`, `cutAllLoops`, `trackSplitPoints`, `breakRun`, `mergeBands` and `sweepRun` — is pure maths about quadrant roses and swept corridors, knows nothing about a storm or an advisory, and `sweepRun` alone is 240 lines. `lonStep` and `onOneBranch` go with it: they are arithmetic about the number line, and `lib/cone-sweep.js` already holds its own copy of the same idea, which is the second caller the extraction rule names. **The tier assembly** — `leadingOvertaken` and `buildFullTrack` — is entirely about what a SOURCE published, which clock each product is on, and which of its hours are still true. **The lift is the geometry**, into a new module under `lib/`, now roughly 660 lines and leaving the assembly reading as one story. **Fault 4b put its own geometry in a NEW file (`lib/polyunion.js`) rather than in here, which is the rule working — but the lift is still owed.** It is a clean lift because the dependency runs one way: assembly calls geometry, geometry never calls assembly. **It is the next thing that happens to this file, before any feature.** |
| `data/nhc-mapserver.js` | 770 | **Seventy over, and the cut this row already named is now OWED.** It was 664 before §7.9's stitch was wired at the door on 2026-08-24, 719 after, and 770 once the surge slot was wired in — a different SERVICE reached through a different relay route, so it cannot ride the layer-id loop and has to be started and joined by hand (§4.8). Recounted rather than carried forward: **267 code, 467 comment, 36 blank** — 35% code, the highest comment ratio here outside `config/constants.js`, and the right shape for a file whose lines are almost all facts about what NHC's MapServer actually returns. **The cut is unchanged and still the wind-tier assembly** — `normalizeForecastRadii`, `normalizePastRadii` and the block building the two envelopes are about wind radii rather than about the MapServer, and are the only part that would read as a story somewhere else. Roughly 150 lines, which lands this at ~620. **Not taken with the surge restore, deliberately:** that went in with a storm making landfall inside 36 hours, and a 150-line file move in the same pass would put a regression somewhere other than the change being judged. This row previously said "take it when the next thing lands here" — the next thing has landed, so the cut is now due before anything else grows this file. |
...
| `map/imagery.js` | 911 | **Watch, and it SHRANK — the first entry on this table to do so.** Wave 6 moved radar out to `map/radar-layer.js` (§4.9), and the split was not a tidy-up: a WMS and a tile pyramid want opposite shapes, and forcing radar through the disc machinery is what made it blurry. Everything radar-specific went with it — the zoom picker, the box, the rim fraction, the radar rim feather, and the coverage branch. What is left is satellite only, and is three concerns: the MapLibre source/layer lifecycle, the fetch-and-cache loop with its retry ladder, and the pixel pass. The layer lifecycle is the clean lift if it grows again. |
...
| `ui/view-home.js` | 1775 | **Over the line, and the first cut on its list is taken.** `countdownHtml` was 168 lines of CODE — the longest function in the app by more than double, which is the shape §12 actually cares about — and it moved out whole to `ui/countdown-home.js` with `headingOf` and `motionDetail`, which are its other caller's too. No behaviour changed in the move: the two things the rail used to close over, the unit system and `sectHead`, are handed in as arguments, and `tools/test-home.mjs` and `test-home-ida.mjs` cover it (557 assertions, and a reversed sort in the moved file fails five of them). Two concerns remain on the list: the strength strip and its figures, and the quiet/error/no-home states. **§49's passes 3 and 5 put 275 lines back** — pass 5 added the past-tense wind clause and the four arms the sentence now splits into — — the past-tense headline, the mid-pass line and the tense arms of the strength strip — and that is the pass the split now waits on: it is a CORRECTNESS change, and moving code in the same breath as changing its meaning means a break could be either. The split is the next pass on this file and it carries no behaviour with it. **§51.3 added ~118 lines and took the one seam the rule allows** — a section string, an ensure, a repaint and an icon, with the whole controller in `ui/surge-home.js`, exactly as Rain did. That is the shape this file's growth is now limited to until the split lands. **§56.7 replaced that section with `Flooding` for a net +20**: `ui/surge-home.js` is deleted and `ui/flooding-home.js` took its 221 lines plus the flood rows Rain gave up, so the controller grew and the seam here did not — same section string, same ensure, same repaint, one wire. **And it shrank by 35 for the first time**: the seven-shape icon set and `iconSvg` moved out to `ui/section-icon.js` when the storm panel became their second caller. `sectHead` stays here — it is the wrapper, and Home's headings are plain blocks where the storm panel's are collapse buttons. **§56.9 added +116 and took the seam the rule allows** — the corridor gate's whole rule is in `ui/home-gate.js`, and what is here is one composition (`readyBundle`), one call, and `wireHouseSections`, which the quiet path now reaches as well as the dashboard path. |
...
| `app/views.js` | 1317 | **Watch, and the cut list is written.** §57.30 step 4 added one parameter and two lines: `onOpenSeasons` threaded to both archive doors. Deliberately a callback and not an import — a static `import` of `seasons/` anywhere on this path would put the whole feature into every visitor's module graph forever (§57.35 fault 4). The composition layer that came out of `main.js` pass 3. Almost all of it is wiring — the drawer, the five views, the home marker and the provisional pin, knotted by construction order. Splitting it would put half a knot in each of two files, which is the thing it exists to prevent. The Home drawer's opening camera frame (§9.16) added ~50 lines of it and the *maths* went to `map/home-frame.js`; only the orchestration lands here, which is the pattern that has kept this file readable. **§48.21's flood facade added ~25 lines of the first kind**, and Slice A's `refreshFloodRow()` another ~20 — the flood status row now moves on SELECTION as well as on the fetch, because the layer is per-storm and "no storm selected" is one of its states — one injected data **§56.6 added ~98 and every line of it is the first kind.** Two entrances into one flood-alert panel — a chip on the globe and a row in the `Flooding` section — meet here and are normalized through `floodAlertFacts` before the view sees either, so the panel has ONE input shape and no branch asking where the reader came from. The cluster split is here for the same reason the Home drawer's camera frame is: `map/layers/flood.js` answers what zoom splits a pile and this file flies, because `map/globe.js` owns every camera travel and a layer reaching for `easeTo` would be a second place the reduce-motion contract lives. |
path plus the module-level slot the drawer reads synchronously, in the shape
§51.3's surge facade already established. That is the third facade added to this
file without a cut and the warning below is now overdue.
**The next feature that wants a home in this file does the inventory and the cut FIRST**, with no behaviour change, so a break can only be the move. The visible seam: the five view factories and their callback bags are one concern; the home cluster — marker, provisional pin, setup and dashboard — is a second. §51.3's surge facade added ~20 lines of the first kind — one injected data path in the shape every other one here already has. |
| `ui/view-seasons-board.js` | 990 | **Over the line, and the sixth cut is taken — but the seam this row named for five passes was measured and rejected.** Step 9 landed its filter and its slider here first and took the file to 1,064, which is the finding this row already carried: a ceiling row that names a cut and defers it does not stop a feature. **THE INVENTORY, TAKEN LATE, IS THIS**, and it is measured rather than estimated: 1,064 lines of which **363 are code** — 628 comment, 73 blank, so 59% of the file is the reason beside the line. Longest function is `onClick` at 48 code lines, well under the 168 that triggered `ui/view-home.js`'s cut, so by this section's own "a long function is worse than a long file" test it was never the worst thing on this table. **==> AND THE INPUT SEAM DOES NOT EXIST IN THE SHAPE THIS ROW DESCRIBED. <==** It said "roughly 300 lines of event routing" over "six ACTIONS, and every one already exists as a function in this file". Measured: the input block is 288 lines of which about **fifteen** are dispatch — the `closest()` lookups and their `if`/`return` — and the other 273 are the decisions themselves plus their reasons. And **three** of the ten actions exist as functions (`setFocus`, `pushSelection`, `onOpenStorm`); `chooseBasin`, `stepYear`, `chooseYear`, `chooseFilter`, `setRadius` and `toggleCheckAll` are inline branch bodies. Lifting the routing moves nothing; extracting the actions first is a rewrite of the handlers. **A seam named five times without being measured once — that is worth more than the line count.** **WHAT WAS TAKEN INSTEAD — TWO CUTS, OF TWO DIFFERENT KINDS, AND THE DIFFERENCE IS THE REUSABLE PART.** `ui/seasons-board-selection.js` (232) took `activeIds`, `selectedEntries`, `pushSelection`, `setFocus` and the two paint bindings **and `ticked` and `focused` with them** — the state moving with the code is what makes it a cut rather than a relocation, since nothing is left here to hand back through a getter. `ui/seasons-board-render.js` (212) took `render` and `rosterHtml` and took NO state, because it holds none: it reads fifteen getters, writes `innerHTML`, and calls the two paints. **A wide read bag is safe where a wide state bag is not** — reads have nothing to drift out of step with, which is the distinction this table should apply to the next candidate rather than counting arguments. **Neither carried behaviour**: `tools/test-seasons-board.mjs` passed untouched across both, and was then mutation-checked against the moved code, which is the only thing that makes a green suite mean anything after a move. Three of four mutations were killed; the survivor was a genuine hole in the suite and is fixed in its own commit. **823 IS STILL 123 OVER AND THIS ROW IS NOT GOING TO PRETEND OTHERWISE.** What remains is the input block (288), the drawer contract (160), the two load hooks (58), the state the reader chose (76) and the wiring that now stands the two new modules up. **There is no obvious next cut and that is the honest finding rather than a gap in the analysis** — the input block is the biggest thing left and it is the seam this row spent five passes recommending and one pass disproving. Anything taken from here next should be argued from the 363-code-lines measurement and from whether STATE moves with it, not from 823. **==> STEP 10'S SEASON CLOCK MUST LAND IN ITS OWN FILE, NOT HERE. <==** Every seasons pass has grown this file and every one has promised to cut next time. That promise has now been broken five times, and two cuts in one session do not buy a sixth. **==> AND IT WAS BROKEN A SIXTH TIME, BY §57.39a, WHICH IS RECORDED HERE RATHER THAN EXCUSED. <==** 823 to 990, across two passes. What arrived is the year stepper's construction and the drawer contract's two new answers (`titleFor` naming the basin, `backLabelFor` naming the year) — and the `[data-step]` branch left the input block, so the routing is fifteen lines lighter than the measurement above. **The growth is overwhelmingly the REASONS**, which is this row's own finding about the file: it was 59% comment at 1,064 lines and the ratio has not improved. That does not make 937 acceptable; it makes the line count the wrong instrument for this file, exactly as the measured-not-estimated inventory above concluded. **The next thing that lands here should be argued from a code-line count and from whether STATE moves with it.** |
| `ui/seasons-board-markup.js` | 472 | **Under the line, and the cut named here was taken — one pass later than this row ordered it.** The row said *"take it before step 9"*; step 9 landed first and carried the file to 785. `ui/seasons-board-furniture.js` then took the 284 lines this row had named: `scoreHtml`, `liveDownHtml`, `ghostsHtml`, `waitingHtml`, `seasonFailedHtml`, `emptyRosterHtml`, `indexFailedHtml`, `footprintSlotHtml` and `footprintNoteHtml`. **The boundary is the SUBJECT rather than the size:** this file is the ROSTER — one storm's row, the master checkbox, the filters, and the assembly that puts them together — and the other is the SEASON: the scorecard and its honesty lines, the unused-name ghosts, the footprint note, and the four sentences for the ways a season can fail to arrive. **The dependency runs one way and no re-export was left behind**: the roster imports the five sentences its assembly still needs, and nothing in the furniture knows a roster exists. A convenience re-export here would have turned the boundary back into one file with two names. **It carried no behaviour change**, deliberately — every function moved is pure and was already told rather than reading, which is what let them move at all. |
| `ui/season-detail-markup.js` | 676 | **Under the line, and the cut has now been taken twice, both times in the pass that caused the crossing rather than the pass after it.** §57.44's archive ranking section took it from 698 to 824 and `ui/season-rank-markup.js` (376) took it back to 651. §57.55a's comments then carried it to 711, and §57.57 took `movementHtml` and `windFieldHtml` out to `ui/season-track-markup.js` (219) before a line of step 3's restructuring landed here — §57.54k required that cut to be the FIRST commit of the step rather than something promised inside it. **Both cuts are the SUBJECT rather than the size, and the second seam was drawn by the plan before it was needed:** §57.54f's step 7 table already merges `How it moved` and `Wind footprint` into one `Where it went`, so the two renderers that answer where the ground went moved together. Everything left here is about how STRONG a storm was and what it did — its peak, its life, its landfalls, how it changed, what NOAA wrote — and nothing in it reads a distance or a forward speed. **The dependency runs one way and no convenience re-export was left behind** in either cut. **Neither carried behaviour**: `tools/test-season-detail.mjs` merges all three modules behind one handle because its subject was always "everything the panel draws" rather than one file, which is why 184 assertions passed across the second move without one of them being rewritten. **Every other row in this table records a ceiling crossed, a cut named, and the cut taken one or more passes later at a bigger size** — `ui/view-seasons-board.js` did it five times and `ui/chart-home.js` is 337 over with its lift still named and untaken. This one has now been taken on sight twice, and that is the only reason it is a 606 after step 4 put 43 lines back — §57.58's ACE gloss and the reasoning behind it. **Steps 5 to 7 all write into this file and it has 31 lines of headroom; whichever one crosses 700 takes the next cut in the pass that causes it**, not the pass after. **Step 7 put 70 lines back** — §57.61a's `endingHtml`, lifted out of `changeHtml` so the two halves of `How it changed` could land in two different sections, and the measured account of the 11 storms whose ending has no words. **24 lines is the tightest this file has ever been and step 7 is the last step of the rebuild, so the next thing that writes here should expect to cut rather than be surprised.** The seam after this one is `landfallsHtml`, which at 126 lines is the largest single renderer left and is the only one that reads `places` — but it is not taken today, because a cut with no caller asking for it is a change nobody can judge. |
| `seasons/index.js` | 966 | **Over the line, and it had never been COUNTED — which is the finding, not the number.** `tools/doc-check.mjs` walked ten directories and `seasons/` was not one of them, so this file and `seasons/seasons.css` have sat over the ceiling since step 5 with the gate reporting a clean table. Found 2026-08-31 when §57.67 slice C added 88 lines to it; the directory is now in the walk. **The inventory, measured rather than estimated: 966 lines of which 234 are code**, 673 comment and 59 blank — 70% of the file is the reason beside the line, which is the same shape `ui/view-seasons-board.js`'s row concluded the line count is the wrong instrument for. It is the archive's SESSION: what a visit is, what goes on screen when one starts, what comes off in what order when one ends, and which of the three rungs the reader was last on. **Slice C added 88 and took only the seams the rule allows** — the clock is built, mounted, torn down and asked for a cut, and every line of its behaviour is in `seasons/clock-control.js` (386), which is the file §57.67b required rather than bolting a fifth control onto this one. **The visible seam is the RUNG memory** — `lastRung`, `watchRung`, `restoreRung`, `openStormNow` and `focusSeasonStorm` are about where the reader was and are the only part of this file that would read as a story somewhere else, roughly 180 lines. Not taken here: slice C is a feature commit and a move inside it would mean a break could be either. **Whichever slice opens this file next takes it first.** |
| `seasons/seasons.css` | 2446 | **Exempt for `ui/panels.css`'s reason, and it had never been counted either.** See the row above for how the whole directory went unwalked. Declarative all the way down, and the cascade order is load-bearing: `seasons.css` loads after `panels.css` precisely so `html[data-seasons="on"]` rules outrank the live app's own, and splitting it would make which half loads last a thing somebody has to remember. §57.67 slice C added 133 lines — the clock's FAB, its scrubber pill, and the one rule that steps the caption pill aside — and the scrubber reuses `.slider` from `panels.css` rather than restating it, which is why it added a dozen lines of rule and a hundred of reason. |
...
| `api/jtwc/abpw.js` | the significant tropical weather advisory, Pacific |
| `api/jtwc/abio.js` | the same bulletin for the Indian Ocean; between them the two cover every ocean NHC does not |
| `api/jtwc/_area-bulletin.js` | the forward-and-cache body both of the above call. Neither route takes a parameter — that is the guard `warning.js` needs `PRODUCT_RE` for |
...
## 55. Known and accepted

**MOVED OUT OF `NOW.md`, 2026-08-22, and it must still be read.** `NOW.md` is a
list of what is IN FLIGHT; these are decisions that are FINISHED. They were
sitting in the live file only because there was nowhere else to put them, and
they were 131 of its 855 lines.

Everything here is a thing somebody will otherwise rediscover, re-report and
re-argue. A `NOW.md` reader is pointed here from its last section.
