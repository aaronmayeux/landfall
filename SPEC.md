...
| `ui/view-seasons-board.js` | 859 | **Over the line, and the first cut was taken in the pass that crossed it.** This file has grown on every seasons pass and has now been cut FOUR times: `lib/season-years.js` took the year rules, `ui/seasons-board-markup.js` took the markup twice (`liveDownHtml`, then the whole roster assembly), and §57.21b took `ui/seasons-board-paint.js` — the three functions that change rows ALREADY on screen. That last one is a real concern boundary rather than a slice: the board decides what is TRUE, the paint file decides what that looks like on existing nodes, and the dependency runs one way. **It went in at 833 and came back at 769, so the cut was 64 short of the ceiling.** ==> AND THE SECOND CUT WAS NAMED AND DELIBERATELY NOT TAKEN. <== It is the LOADING block — `loadIndexOnce`, `retryLive`, `loadSeasonNow` and the `yearDeps` bindings, roughly 170 lines — which owns every piece of fetch state (`indexState`, `index`, `liveIndex`, `seasonState`, `entries`, `score`, `roster`, `provisional`, `unreadable`, `loadToken`) and shares nothing with the roster's own state (`ticked`, `focused`, `filter`) except the render that reads both. Taking it would put the file near 600. It was not taken because §57.21b changed eight behaviours at once, and every row on this table that took a structural cut in the same breath as a behaviour change says the same thing afterwards: a break could then be either. **The loading cut was named as the next thing that happens to this file, before any feature. STEP 7 WENT IN AHEAD OF IT ANYWAY AND THAT WAS A DELIBERATE CALL, NOT AN OVERSIGHT (§57.22b).** Step 7 is a feature that had already been built, shipped and reverted once for a tap fault nobody could reproduce, and the whole point of re-landing it is a clean glass signal. A 200-line state refactor riding in the same push would have made a break either — which is the exact mistake this row records §57.21b making, arriving from the other side. **It went in at 769 and came back at 836. The cut is now OVERDUE rather than pending, it is still the same cut, and it should be its own push with no behaviour change in it. **STEP 7 WAS CONFIRMED ON GLASS ON 2026-08-25, SO THE REASON FOR DEFERRING IT HAS EXPIRED AND THIS IS NOW THE NEXT THING THAT HAPPENS TO THIS FILE.** 859 as of the tap fix. Step 9 adds a filter and a slider to this same view; it must not land on top of this.** |
