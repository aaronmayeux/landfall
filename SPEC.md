...
| `ui/view-seasons-board.js` | 950 | **Over the line, and the sixth cut is taken — but the seam this row named for five passes was measured and rejected.** Step 9 landed its filter and its slider here first and took the file to 1,064, which is the finding this row already carried: a ceiling row that names a cut and defers it does not stop a feature. **THE INVENTORY, TAKEN LATE, IS THIS**, and it is measured rather than estimated: 1,064 lines of which **363 are code** — 628 comment, 73 blank, so 59% of the file is the reason beside the line. Longest function is `onClick` at 48 code lines, well under the 168 that triggered `ui/view-home.js`'s cut, so by this section's own "a long function is worse than a long file" test it was never the worst thing on this table. **==> AND THE INPUT SEAM DOES NOT EXIST IN THE SHAPE THIS ROW DESCRIBED. <==** It said "roughly 300 lines of event routing" over "six ACTIONS, and every one already exists as a function in this file". Measured: the input block is 288 lines of which about **fifteen** are dispatch — the `closest()` lookups and their `if`/`return` — and the other 273 are the decisions themselves plus their reasons. And **three** of the ten actions exist as functions (`setFocus`, `pushSelection`, `onOpenStorm`); `chooseBasin`, `stepYear`, `chooseYear`, `chooseFilter`, `setRadius` and `toggleCheckAll` are inline branch bodies. Lifting the routing moves nothing; extracting the actions first is a rewrite of the handlers. **A seam named five times without being measured once — that is worth more than the line count.** **WHAT WAS TAKEN INSTEAD, on the test the fifth cut passed:** `ui/seasons-board-selection.js` (232) took `activeIds`, `selectedEntries`, `pushSelection`, `setFocus` and the two paint bindings **and `ticked` and `focused` with them**. The state moving with the code is what makes it a cut rather than a relocation — nothing is left here to hand back through a getter. **It carried no behaviour**: `tools/test-seasons-board.mjs` passed untouched at 174 assertions and was then mutation-checked against the moved code, which is the only thing that makes a green suite mean anything after a move. **950 is still 250 over and this row is not going to pretend otherwise.** The remaining concerns are the input block (288), the render (150), the drawer contract (160), the two load hooks (58) and the state the reader chose (76). **The render is the next cut and it is a clean one** — `render` and `rosterHtml` are pure reads of state, so nothing can drift; the cost is a read bag around fifteen wide. **Step 10's season clock must land in its own file, not here** — the pattern on this table is that every seasons pass grows this file and every one promises to cut next time. |
