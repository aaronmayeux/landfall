...
| `map/layers/points-forecast.js` | 816 | **Over the line, and it GREW by 44 in §57.7h rather than shrinking — flagged, not hidden.** The port added the per-storm genesis pass and the two-arm radius. It is three things: the MapLibre layer definitions, the per-storm projection and grouping pass, and the two publish/subscribe stores that hand placement out. The GEOMETRY of both placements is already out of it — `label-placement.js` for the time labels, `name-placement.js` for the name — which is why what remains is readable. **The layer definitions are the cut and it is now owed, not optional. Behaviour-free, before anything else lands here.** |
...
| `lib/windswath.js` | 794 | **THE CUT IS NOW OVERDUE AND THIS ROW SAYS SO PLAINLY.** It was 655, then 719 when §7.14 added `leadingOvertaken`, and 794 since §7.12 fault 3 added the antimeridian branch discipline on 2026-08-24 — **17 lines of code and 58 of reason**, recorded rather than absorbed. Recounted rather than carried forward: **374 code, 377 comment, 44 blank**, so 53% of the file is the argument beside the arithmetic. The structure has not changed and neither has the cut. It is TWO things wearing one filename, and the seam is the same shape as `lib/trackline.js`'s directly above. **The planar sweep geometry** — `radiusAtBearing`, `centreOfRadiiRing`, `fullRing`, `sweepRun`, and the despike and loop-cut passes inside it — is pure maths about quadrant roses and swept corridors, knows nothing about a storm or an advisory, and `sweepRun` alone is 240 lines. `lonStep` and `onOneBranch` go with it: they are arithmetic about the number line, and `lib/cone-sweep.js` already holds its own copy of the same idea, which is the second caller the extraction rule names. **The tier assembly** — `leadingOvertaken` and `buildFullTrack` — is entirely about what a SOURCE published, which clock each product is on, and which of its hours are still true. **The lift is the geometry**, into a new module under `lib/`, now roughly 420 lines and leaving the assembly reading as one story. It is a clean lift because the dependency runs one way: assembly calls geometry, geometry never calls assembly — the same test that made the trackline cut obvious. **This was a bug fix on a live wrong shape and the cut was not taken inside it, deliberately** (surgical strike: an error gets the targeted fix, not a rewrite of the file). It is the next thing that happens to this file, before any feature. |
