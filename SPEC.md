...
| `ui/view-seasons-board.js` | 868 | **Over the line, and the FIFTH cut has now been taken — the one this row named four passes ago.** The file has been cut five times: `lib/season-years.js` took the year rules, `ui/seasons-board-markup.js` took the markup twice (`liveDownHtml`, then the whole roster assembly), §57.21b took `ui/seasons-board-paint.js` (the three functions that change rows already on screen), and on 2026-08-25 `ui/seasons-board-loading.js` took the LOADING block. That last one is the cut this row had named and deferred three times running. **The boundary is Aaron's line: the loading file owns what has been FETCHED, the view owns what is SELECTED, and the dependency runs one way.** `loadIndexOnce`, `retryLive`, `loadSeason`, `retryIndex` and the `yearDeps` bindings went, with every piece of fetch state (`indexState`, `index`, `indexReason`, `liveIndex`, `liveReason`, `liveRetrying`, `seasonState`, `seasonReason`, `entries`, `score`, `roster`, `provisional`, `unreadable`, `loadToken`). What is left is the four choices a reader makes with a thumb — `basin`, `year`, `filter`, `ticked`, `focused` — and the render where the two halves meet. **It carried no behaviour change at all**, deliberately, so a break could only be the move: same awaits in the same order, same `Promise.all`, same token check, same render points. Two moments where a load has to touch a choice became hooks the view hands in (`onIndexReady` picks the default year, `onSeasonChanging` drops the ticks), each running at exactly the point in the sequence the single function used to. **==> AND IT DID NOT CLEAR THE CEILING, WHICH THIS ROW PREDICTED WRONGLY AND SAYS SO. <==** The previous version of this row estimated the block at ~170 lines and the file landing "near 600." That estimate was made when the file was 769. **Step 7 and its tap fix put ~90 lines back before the cut was taken, so the same cut landed 859 → 762 — 97 off, not 170, and still 62 over.** The lesson is the one this table keeps recording from the other side: a named cut deferred past a feature does not stay the same size. **THE NEXT SEAM, NAMED: the click and keydown handlers, roughly 200 lines of which two thirds is the argument for the tap-fault fix.** It is NOT a clean lift today — every branch in there reads `basin`, `year`, `filter`, `ticked` and `focused`, so moving it means a callback bag as wide as the state itself, which is the shape §12 calls a knot in two files rather than one. **Watch, do not split, and do not let step 9 land here without an inventory first**: it adds a filter and a near-home slider to this same view. One coverage gap found while taking the cut and left alone: `tools/test-seasons-board.mjs` does not fail when `loadSeason`'s stale-fetch token guard is deleted. That guard predates this pass and the gap is pre-existing, but it is now written down. **§57.21c added ~106 and the row goes to 868 — 168 over, and step 9 STILL has not landed here.** The active-storm rule is most of it: `activeIds()` now asks the live app before falling back to the clock, `selectedEntries()` drops running storms, and the master box and `showStorm` both route through the same question. It is state about what a reader may DRAW, so it belongs on this side of the loading boundary — but the file has now grown on every seasons pass without exception, which is the pattern this row has recorded four times. **The named seam is unchanged and still not clean** (the click and keydown handlers, ~200 lines, every branch reading five pieces of state). **Step 9 must take an inventory before it adds its filter and its near-home slider, and this row is now the third consecutive one saying so.**
| `ui/seasons-board-markup.js` | 730 | **Newly over the line, and the gate caught it rather than a reader.** This file is itself the product of two cuts out of `ui/view-seasons-board.js` — `liveDownHtml` first, then the whole roster assembly — so it has been growing as the destination of somebody else's ceiling problem, which is the failure mode a split can turn into if nobody watches the far side. It was 677 on `main` and §57.21c's active-storm row took it to 730: the disabled checkbox and its `aria-label`, the `– active` date cell, and the master box counting only DRAWABLE rows. **THE CUT IS NAMED AND IT IS CLEAN, WHICH IS THE UNUSUAL PART.** `scoreHtml`, `ghostsHtml`, `emptyRosterHtml`, `indexFailedHtml` and `liveDownHtml` are roughly 260 lines of SEASON-LEVEL furniture — the scorecard, the unused-names line, the two failure sentences — and they share nothing with `rowHtml`/`checkAllHtml`/`seasonRosterHtml`, which are the ROSTER. Different subject, no shared state, one direction. `ui/seasons-board-furniture.js` takes the first set and this lands near 470. **Not taken in this pass deliberately**: §57.21c changed behaviour on five surfaces at once, and a move landing in the same push means a break could be either. Take it before step 9, which adds a filter row and a near-home slider and will land on the roster half. |
